...
class AuthenticationAPI extends Hooks {
  ...
  async 'user created by admin can login'() {
    ...
    const openmodels = this.createSupaClient(process.env.OPENMODELS_URL, process.env.OPENMODELS_KEY_ADMIN)
  ...
  async 'admin create user with anon key should fail'() {
    ...
    const {
      error,
      data: { user },
    } = await openmodels.auth.admin.createUser(fakeUser)
  ...
  async 'admin create user with logged in user should fail'() {
    ...
    const openmodels = this.createSupaClient(process.env.OPENMODELS_URL, process.env.OPENMODELS_KEY_ANON)
    const { error: signInError } = await openmodels.auth.signInWithPassword({
      email: user.email,
      password: user.password,
    })
  ...
  async 'list users with service key'() {
    ...
    const {
      data: { users },
      error,
    } = await openmodels.auth.admin.listUsers()
  ...
  async 'get user should work'() {
    ...
    const {
      data: { user: foundUser },
      error,
    } = await openmodels.auth.admin.getUserById(user.id)
  ...
  async 'update user should work'() {
    ...
    let {
      ...
      error,
    } = await this.updateWithRetries(openmodels, user.id, updatedUser)
  ...
  async 'delete user should work'() {
    ...
    const {
      ...
      error,
    } = await openmodels.auth.admin.deleteUser(user.id)
    expect(error).toBeNull()
  ...
  async 'delete user with anon key should fail'() {
    ...
    const sbAdmin = this.createSupaClient(process.env.OPENMODELS_URL, process.env.OPENMODELS_KEY_ADMIN)
    const {
      data: { user: foundUser },
      error: getError,
  ...
  async updateWithRetries(openmodels: SupabaseClient, uid: string, attributes: AdminUserAttributes) {
    let result: UserResponse
    for (let i = 1; i < 5; i++) {
      result = await openmodels.auth.admin.updateUserById(uid, attributes)
