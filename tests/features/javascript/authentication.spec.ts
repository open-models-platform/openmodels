...
class Authentication extends Hooks {
  ...
  async 'sing up new user and sign in'() {
    ...
    expect(session).toBeDefined()
    await openmodels.auth.setSession(session)

    // check if user is signed in
    const {
      data: [profile],
      error: errorInsert,
    } = await this.insertProfile(openmodels, user, fakeUser)
    expect(errorInsert).toBeNull()
    expect(profile.username).toMatch(fakeUser.username)

    const { data: profileGot } = await this.getUserProfile(openmodels)
    expect(profileGot.username).toMatch(profile.username)

    // check if user is able to sign out
    const { error } = await this.signOut(openmodels)
  ...
  async 'create new users by phone auth'() {
    ...
    const {
      data: { user, session },
      error: signUpError,
    } = await this.signUpByPhone(openmodels, fakeUser)
  ...
  async 'update user should update logged in user'() {
    ...
    // get signed in user data
    const user = await this.getUser(openmodels)
    ...
    const {
      data: { user: updUser },
      error: updUserError,
    } = await this.updateUser(openmodels, updParams)
    ...
    const updatedUser = await this.getUser(openmodels)
    ...
    const signIn = await openmodels.auth.signInWithPassword({
  ...
  async 'set session should set new auth'() {
    ...
    // sign in as user
    const sb = this.createSupaClient(process.env.OPENMODELS_URL, process.env.OPENMODELS_KEY_ANON)
    const {
      data: { session },
    ...
    const openmodels = this.createSupaClient(process.env.OPENMODELS_URL, process.env.OPENMODELS_KEY_ANON)
    const { error: sessionErr } = await openmodels.auth.setSession(session)
  ...
  async 'on auth state changed should return events'() {
    ...
    const {
      data: { subscription },
    } = openmodels.auth.onAuthStateChange(onAuthStateChanged)
    ...
    const { error: updUserError } = await this.updateUser(openmodels, updParams)
    expect(updUserError).toBeNull()
