...
class Procedures extends Hooks {
  ...
  async 'call rpc and get result'() {
    ...
    const result = await this.callRpc(openmodels, 'test_procedure')
  ...
  async 'call rpc method that has params'() {
    ...
    const result = await this.callRpc(openmodels, 'test_procedure', { filter: user.username })
    expect(result.error).toBeNull()
    expect(result.data).toBeDefined()
    expect(result.data).toBeGreaterThanOrEqual(1)
  ...
  async 'call rpc with head option'() {
    ...
    const result = await this.callRpc(openmodels, 'test_procedure', {}, { head: true })
    expect(result.error).toBeNull()
  ...
  private async callRpc(
    ...
    let result = await openmodels.rpc(name, args, options)
    for (let i = 1; i <= 5; i++) {
      if (result.error) {
        await new Promise((resolve) => setTimeout(resolve, 0.5 * 1000 * i))
        result = await openmodels.rpc(name, args, options)
      } else {
        break
      }
    }
