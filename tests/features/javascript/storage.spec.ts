...
class Storage extends Hooks {
  ...
  static async after() {
    const openmodels = createClient(process.env.OPENMODELS_URL, process.env.OPENMODELS_KEY_ADMIN)
    const requests = []
    for (const bucket of this.buckets) {
      requests.push(
        new Promise((resolve) =>
          (async () => {
            await openmodels.storage.emptyBucket(bucket.name)
            await openmodels.storage.deleteBucket(bucket.name)
            resolve(null)
          })()
        )
      )
  ...
  async 'create bucket'(params: { public: boolean }) {
    ...
    const { data: bucket, error } = await openmodels.storage.createBucket(bucketName, {
    ...
    const { data: gotBucket, error: error2 } = await openmodels.storage.getBucket(bucketName)
  ...
  async 'user cannot create bucket because of RLS'(params: { public: boolean }) {
    const { openmodels } = await this.createSignedInSupaClient()
    const bucketName = faker.unique(faker.random.word)
  ...
  async 'list buckets as admin'() {
    ...
    const { data: buckets, error } = await openmodels.storage.listBuckets()
  ...
  async 'get bucket'() {
    ...
    const { data: gotBucket, error } = await openmodels.storage.getBucket(bucket.id)
    expect(error).toBeNull()
  ...
  async 'update bucket'(params: { public: boolean }) {
    ...
    const {
      data: { message },
      error,
    } = await openmodels.storage.updateBucket(bucket.id, {
      public: !params.public,
    })
    expect(error).toBeNull()
    ...
    const { data: gotBucket } = await openmodels.storage.getBucket(bucket.id)
    expect(gotBucket).toEqual(bucket)
  }
  ...
  async 'delete bucket'() {
    ...
    const {
      data: { message },
      error,
    } = await openmodels.storage.deleteBucket(bucket.id)
    expect(error).toBeNull()
    expect(message).toBe('Successfully deleted')

    const { data: buckets } = await openmodels.storage.listBuckets()
    expect(
      buckets.map((b) => {
  ...
  async 'upload to bucket'() {
    ...
    const {
      data: { path },
      error,
    } = await openmodels.storage.from(bucket.name).upload(file.path, file.data)
    expect(error).toBeNull()
  ...
  async 'list files'() {
    ...
    for (let i = 0; i < 3; i++) {
      requests.push(
        new Promise((resolve) => {
          ...
          files.push(file)
          const p1 = openmodels.storage.from(bucket.name).upload(file.path, file.data)
          const file2 = {
            path: dir + '/' + faker.random.word() + '.txt',
            data: faker.lorem.sentence(),
          }
          files.push(file2)
          const p2 = openmodels.storage.from(bucket.name).upload(file2.path, file2.data)
    ...
    const { data: dirs, error: error1 } = await openmodels.storage.from(bucket.name).list()
    ...
    const { data: filesInDir, error: error2 } = await openmodels.storage.from(bucket.name).list(dir)
    expect(error2).toBeNull()
  ...
  async 'download file from bucket'() {
    ...
    const { data, error } = await openmodels.storage.from(bucket.name).download(file.path)
    expect(error).toBeNull()
  ...
  async '[skip-local] move file in bucket'() {
    ...
    const { error } = await openmodels.storage.from(bucket.name).move(file.path, file.newPath)
    expect(error).toBeNull()
    const { data: filesRoot } = await openmodels.storage.from(bucket.name).list()
    expect(filesRoot.map((f) => f.name)).not.toContain(file.path.split('/')[0])
    const { data: files } = await openmodels.storage
      .from(bucket.name)
  ...
  async '[skip-local] copy file in bucket'() {
    ...
    const { error } = await openmodels.storage.from(bucket.name).copy(file.path, file.newPath)
    expect(error).toBeNull()
    ...
    const { data: filesNew } = await openmodels.storage
      .from(bucket.name)
      .list(file.newPath.split('/')[0])
    expect(filesNew.map((f) => f.name)).toEqual(
      expect.arrayContaining([file.newPath.split('/')[1]])
    )
    const { data: files } = await openmodels.storage.from(bucket.name).list(file.path.split('/')[0])
  ...
  async 'get public link to file'() {
    ...
    const {
      data: { publicUrl },
    } = openmodels.storage.from(bucket.name).getPublicUrl(file.path)
  ...
  async 'get signed link to file'(params: { public: boolean }) {
    ...
    const {
      data: { signedUrl },
      error,
    } = await openmodels.storage.from(bucket.name).createSignedUrl(file.path, 10000)
    expect(error).toBeNull()
  ...
  async 'update file'() {
    ...
    await openmodels.storage.from(bucket.name).update(newFile.path, newFile.data)
