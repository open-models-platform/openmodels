...
class PostgREST extends Hooks {
  ...
  async 'select row'() {
    ...
    const { data: profileGot, error } = await this.getUserProfile(openmodels)
  ...
  async 'update row'() {
    ...
    const { data: updUser, error } = await openmodels
      .from('profiles')
      .update({
        id: user.id,
        username: newName,
  ...
  async 'delete row'() {
    ...
    const { error } = await openmodels.from('profiles').delete().eq('id', user.id).select()
    expect(error).toBeNull()
