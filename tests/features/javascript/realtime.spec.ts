...
class Realtime extends Hooks {
  ...
  async '[skip-stage] connect to realtime'() {
    ...
    expect(err).toBeNull()
    const ok = await openmodels.removeChannel(channel)
    expect(ok).toBe('ok')
  }
  ...
  async '[skip-stage] receive event when connected to realtime'() {
    ...
    const { openmodels, user } = await this.createSignedInSupaClient()
    ...
    await this.insertProfile(openmodels, user, user)
  ...
  @test
  async 'get openmodels client subscriptions'() {
    const { openmodels } = await this.createSignedInSupaClient()

    const channel1 = openmodels
      .channel('profiles')
      .on('postgres_changes', { event: '*', schema: 'public' }, (payload: any) => {
        console.log(payload)
        expect('event received').toBe('should not receive event')
      })
    const channel2 = openmodels
      .channel('profiles')
      .on('postgres_changes', { event: '*', schema: 'public' }, (payload: any) => {
        console.log(payload)
        expect('event received').toBe('should not receive event')
    ...
    const channels = openmodels.getChannels()
    expect(channels).toEqual(expect.arrayContaining([channel1, channel2]))
    openmodels.removeAllChannels()
    expect(openmodels.getChannels().length).toEqual(0)
  }
  ...
  async '[skip-stage] unsubscribe from table'() {
    ...
    await new Promise((resolve) => setTimeout(resolve, 1000))
    expect(channel._isClosed).toBeTruthy()
    await openmodels.removeChannel(channel)
  }
  ...
  async '[skip-stage] remove all subscriptions from client'() {
    ...
    // wait for subscription to postgres
    await new Promise((resolve) => setTimeout(resolve, 8000))
    const ok = await openmodels.removeAllChannels()
