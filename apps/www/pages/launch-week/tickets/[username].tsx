...
export default function UsernamePage({ user, ogImageUrl }: Props) {
  ...
  const DESCRIPTION = `Claim and share your OpenModels Launch Week X ticket for a chance to win OpenModels swag.`
...
export const getStaticProps: GetStaticProps = async ({ params }) => {
  ...
  fetch(
    `https://dev.openmodels.co/functions/v1/lwx-og?username=${encodeURIComponent(
      username ?? ''
    )}`
  )
  fetch(
    `https://dev.openmodels.co/functions/v1/lwx-ticket?username=${encodeURIComponent(
      username ?? ''
    )}`
  )
  ...
  const ogImageUrl = `https://dev.openmodels.co/storage/v1/object/public/images/lwx/og/${ticketType}/${username}.png`
