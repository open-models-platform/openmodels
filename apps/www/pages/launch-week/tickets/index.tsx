...
const generateOgs = async (users: UserData[]) => {
  users?.map(async (user) => {
    const ogImageUrl = `https://dev.openmodels.co/functions/v1/lwx-ticket?username=${encodeURIComponent(
      user.username ?? ''
...
export default function TicketsPage({ users }: Props) {
  ...
  const DESCRIPTION = 'OpenModels Launch Week X | 11-15 December 2023'
