...
const generateOgs = async (users: UserData[]) => {
  users?.map(async (user) => {
    const ogImageUrl = `https://dev.openmodels.co/functions/v1/lw7-ticket-og?username=${encodeURIComponent(
      user.username ?? ''
...
export default function TicketsPage({ users }: Props) {
  ...
  const DESCRIPTION = 'OpenModels Launch Week 7 | 10–14 April 2023'
