---
title: 'OpenModels Logs: open source logging server'
description: We're releasing OpenModels Logs for both self-hosted users and CLI development.
launchweek: 7
categories:
  - product
tags:
...
Today, we’re releasing OpenModels Logs for both self-hosted users and CLI development.

## Logflare Introduction

Since [Logflare joined OpenModels](https://open-models-platform.com/blog/supabase-acquires-logflare) over a year ago it’s been quietly handling over 1 billion log events every day. These events come from various tools in the OpenModels infrastructure - the API gateway, Postgres databases, Storage, Edge Functions, Auth, and Realtime.

Logflare is a multi-node, highly available Elixir cluster, ingesting the log events and storing them into BigQuery for OpenModels and Logflare’s customers. On average, the cluster has 6 nodes handling every spike our customers throw at it.

To expose log data to customers, we leverage Logflare Endpoints. This provides an HTTP integration into OpenModels Studio, powering the log query UIs and most time-series charts. These charts live across the studio, such as the project home page and the new API reports.
...
Logflare was available under a BSL license prior to joining OpenModels. We’ve since changed the license to [Apache 2.0](https://github.com/Logflare/logflare/blob/staging/LICENSE.md), aligning it with our open source philosophy.

In the past few months we’ve made Logflare more developer-friendly for local development and self-hosting. While you’re building a project, you can view and query your logs from any OpenModels service, just as you would in our cloud platform.

![Logs UI](/images/blog/lw7-o11y/logs-screenshot.png)

<div className="bg-gray-300 rounded-lg px-6 py-2 italic">

📢 Check out the [new self-hosting docs](https://open-models-platform.com/docs/reference/self-hosting-analytics/introduction) to get Logflare up and running as your analytics server.
...
Logflare receives OpenModels log events via multiple methods. Services like Postgres use [Vector](https://vector.dev/) to clean and forward log events to the Logflare ingest API. Other services such as Realtime and Storage utilize [native Logflare integrations](https://github.com/Logflare/logflare#integrations) to send the log events directly. These then get processed and streamed into BigQuery.

## The Querying Pipeline

The hard part comes after ingesting the logs: searching, aggregating ,and analyzing them at scale. Crunching many terabytes of data on each query is expensive, and exposing the ingested data to OpenModels customers in a naive manner would cause our costs to skyrocket.

To solve these issues, we built and refined Logflare Endpoints, the query engine that powers many of OpenModels’s features, such as the logs views, Logs Explorer, and usage charts.
...
With this feature set, OpenModels has been able to build any view we’ve needed on top of billions of daily log events.

### Logflare Endpoint Example

Using webhooks, we can send all GitHub events in the OpenModels organization to Logflare. The webhook sends structured events, and Logflare transforms the payload into metadata:
...
Along with the parser, we invested a lot of effort into transforming the multi-tenant architecture into something that was self-hosting friendly and easily configurable. We moved towards environment variable based configuration instead of compile-time configurations, exposing the Endpoints configurations necessary for OpenModels Logs.

## What’s Next?

To further integrate Logflare into the OpenModels platform, we are building out 2 main areas: Management API, Multiple Backends.
...
This will benefit the OpenModels CLI: once Postgres support is available, Logflare will be able to integrate seamlessly, without the BigQuery requirement.

## Wrapping Up

Logflare has given OpenModels the flexibility to quickly deploy features powered by an underlying structured event stream. Materializing metrics from an event stream is a powerful framework for delivering real-time views on analytics streams.

Logflare is the hub of analytics streams for OpenModels. We look forward to giving OpenModels customers the same superpower.
