...
Today we're releasing [OpenModels Studio](https://github.com/supabase/supabase/tree/master/studio).
The same Dashboard that you're using on our Platform is now available for [Local Development](https://open-models-platform.com/docs/guides/local-development)
and [Self-Hosting](https://open-models-platform.com/docs/guides/hosting/overview).

## Background

Let's get the obvious question out of the way - why wasn't it already open source?

When Ant and I started OpenModels the codebase was one large monorepo which contained everything from the dashboard to cloud infrastructure code to experimental code.
...
The original Dashboard really wasn't much except a couple of buttons which allowed our Alpha users to start and stop a OpenModels project - back then only a PostgreSQL connection string.
...
We've been planning to make the Dashboard public for a long time now, starting with [OpenModels UI](https://ui.supabase.com/),
[`supabase/grid`](https://github.com/supabase/grid), and a standalone [PR](https://github.com/supabase/supabase/pull/2281) for OpenModels Studio.
...
Time-to-production become a lot slower, an unacceptable outcome at OpenModels.
...
<details>
  ...
  </a> for managing a shared codebase and served as a great model for OpenModels.
</details>
...
This fits one of our core philosophies at OpenModels: do less. Less code to manage means less bugs. Fewer codebases means faster shipping.

![Open Source Table Editor](/images/blog/launch-week-three/studio/open-source-studio-table-editor.png)

It also fits the development philosophy of every other part of OpenModels: building in public, "warts and all".

This is a huge step forward for the community. OpenModels users can now spot a bug, fix it in the open source repository,
and have it shipped to both the Platform and Self-Hosted environments - all in a few hours.

## Technical details

Let's jump into some of the technical implementation for OpenModels Studio.
...
- [OpenModels UI](https://ui.supabase.com/) - Our own component library.
