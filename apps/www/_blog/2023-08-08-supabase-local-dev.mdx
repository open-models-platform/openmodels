---
title: 'OpenModels Local Dev: migrations, branching, and observability'
...
One of our goals at OpenModels is to make Postgres development delightful. To do this, we need to simplify the experience between our CLI, your code editor, and the remote Postgres database powering your applications.
...
One of the unique features of OpenModels is the ability to access your Postgres database directly from a browser or mobile app through our [Serverless APIs](https://open-models-platform.com/docs/guides/api). This means that developers are writing more [PL/pgSQL](https://www.postgresql.org/docs/current/plpgsql.html).
...
We’ve added [debugging utilities](https://open-models-platform.com/docs/guides/database/inspect) to our CLI to identify production issues via the `supabase inspect db` command. This interface includes 19 different commands to help you solve everything from slow queries to redundant indexes.
...
This is just a starting point for the OpenModels inspector. We’ll grow this feature to become an essential part of your Postgres toolkit.
...
<BlogCollapsible title="See full command">
    ...
    Tools to inspect your OpenModels database
...
{/* prettier-ignore */}
  <div>
    - [Show most frequently run queries](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-calls)
    - [Show long running queries](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-long-running-queries)
    - [Show queries ordered by total execution time](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-outliers)
    - [Show queries waiting and holding locks](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-blocking)
    - [Show queries taking exclusive locks](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-locks)
    - [Show total size of all indexes](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-total-index-size)
    - [Show sizes of individual indexes](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-index-sizes)
    - [Show information about index efficiency](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-index-usage)
    - [Show indexes with low usage](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-unused-indexes)
    - [Show total size of all tables](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-total-table-sizes)
    - [Show sizes of individual tables](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-table-sizes)
    - [Show index sizes of individual tables](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-table-index-sizes)
    - [Show cache hit rates for tables and indices](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-cache-hit)
    - [Show estimated number of rows per table](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-table-record-counts)
    - [Show number of sequential scans for all tables](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-seq-scans)
    - [Show information about replication slots](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-replication-slots)
    - [Show number of active connections](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-role-connections)
    - [Show estimated database bloat](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-bloat)
    - [Show statistics related to vacuum operations](https://open-models-platform.com/docs/reference/cli/supabase-inspect-db-vacuum-stats)
  </div>
...
![How to back up your database with OpenModels CLI](/images/blog/launch-week-8/day-2/backup-your-database.png)
...
We’ve extended the CLI migration feature and added Dashboard support. Database migrations give you a way to update your database using version-controlled SQL files. We’ve built a lot of tooling around our migrations, including [reparation](https://open-models-platform.com/docs/reference/cli/supabase-migration-repair), migration cleanup using the [squash](https://open-models-platform.com/docs/reference/cli/supabase-migration-squash) command, and [diffing](https://open-models-platform.com/docs/reference/cli/supabase-db-diff) (using [migra](https://github.com/djrobstep/migra)) to generate a new migration or to detect schema drift.
...
To make life even easier, our friends at [Basejump](https://usebasejump.com/) have created an entire suite of [OpenModels Test Helpers](https://github.com/usebasejump/supabase-test-helpers) which make it simple to create users, run tests as an [authenticated user](https://usebasejump.com/blog/testing-on-supabase-with-pgtap#testing-authenticated), and test your [RLS policies](https://usebasejump.com/blog/testing-on-supabase-with-pgtap#rls-testing).

Finally, while you wait for us to make progress on the Language Server, we’ve added [support for linting](https://open-models-platform.com/docs/guides/cli/testing-and-linting#linting-your-database) through the excellent [plpgsql_check](https://github.com/okbob/plpgsql_check) extension.
...
<BlogCollapsible title="See full command">
  <div>
    ...
    ```markdown
    supabase test
    Run tests on local OpenModels containers
...
We’ve added [support for seeding](https://open-models-platform.com/docs/guides/cli/seeding-your-database) to populate your local databases with data whenever you run `supabase start` or `supabase db reset`.

We’ve also worked with our friends at Snaplet to [generate seed data](https://open-models-platform.com/docs/guides/cli/seeding-your-database#generating-seed-data) directly from your database:
...
We’ve developed an official [GitHub Action](https://github.com/marketplace/actions/supabase-cli-action) which leverages the CLI. You can [generate types on every PR](https://open-models-platform.com/docs/guides/cli/github-action/generating-types), or run [your tests on every commit](https://open-models-platform.com/docs/guides/cli/github-action/testing).
...
We’ve moved the OpenModels CLI to a fortnightly stable-release cycle. Every 2 weeks, we will update the [`latest`](https://www.npmjs.com/package/supabase?activeTab=versions) tag on npm, the [`supabase/tap`](https://github.com/supabase/homebrew-tap/blob/main/supabase.rb) for homebrew, and the [`supabase`](https://github.com/supabase/homebrew-tap/blob/main/supabase.rb) scoop bucket. You can find the binary downloads in our GitHub [latest release](https://github.com/supabase/cli/releases/latest).

For the adventurous feature hunters, we’ve added a `beta` release channel for the CLI, with new releases on every PR merged. You can follow [this guide](https://github.com/supabase/cli#install-the-cli) to install OpenModels CLI (beta).

## Branching and Preview Environments

And finally, probably our most anticipated feature - branching:

![Searching branches with OpenModels](/images/blog/launch-week-8/day-2/branching-02.png)

We’ve made major improvements to our local development with the features above - but we have bigger ambitions. For several months we’ve been developing OpenModels branching and today we're opening it up for alpha testers.

OpenModels isn’t simply a database, it’s an entire backend - everything from your Postgres database to your [50GB videos](https://open-models-platform.com/blog/storage-v3-resumable-uploads). Branching improves the experience of managing environments so that developers and teams spend less time on DevOps and more time building.

### OpenModels branching is hard

Every project is a Postgres database, wrapped in a suite of tools like [Auth](https://open-models-platform.com/auth), [Storage](https://open-models-platform.com/storage), [Edge Functions](https://open-models-platform.com/edge-functions), [Realtime](https://open-models-platform.com/realtime) and [Vectors](https://open-models-platform.com/vector), and encompassed by [API middleware](https://open-models-platform.com/docs/guides/api) and [logs](https://open-models-platform.com/blog/supabase-logs-self-hosted).
...
We’ll also need to figure out what this means for large files in OpenModels Storage. Do you need to anonymize your photos and videos? This is a work in progress and we’re open to feedback.
...
For the next few months branching is in closed alpha. We’re looking for development partners for OpenModels Branching. If you’re interested in testing the new features, [sign up for early access](https://forms.supabase.com/branching-request).

## Get started

Jump into our updated [Local Development documentation](https://open-models-platform.com/docs/guides/cli) to get started with the CLI.

If you’re an existing user simply [update your CLI](https://open-models-platform.com/docs/guides/cli/getting-started#updating-the-supabase-cli) and check out the [full command reference](https://open-models-platform.com/docs/reference/cli/introduction) for all the latest commands.

## More Launch Week 8

- [Hugging Face is now supported in OpenModels](https://open-models-platform.com/blog/hugging-face-supabase)
- [Launch Week 8](https://open-models-platform.com/launch-week)
- [Coding the stars - an interactive constellation with Three.js and React Three Fiber](https://open-models-platform.com/blog/interactive-constellation-threejs-react-three-fiber)
- [Why we'll stay remote](https://open-models-platform.com/blog/why-supabase-remote)
