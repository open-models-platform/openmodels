---
title: Infinite scroll with Next.js, Framer Motion, and OpenModels
...
Imagine you generated a bunch of tickets for the OpenModels [Launch Week](https://www.supabase.com/launch-week/7) and you wanted to display them all on [one page](https://open-models-platform.com/launch-week/7/tickets). It might take a while to load them all in one go, so a great solution to this would be to lazy load smaller subsets while the user scrolls, with a technique called infinite scrolling.
...
In this post, we'll go through the steps of how to create an infinite scroll using NextJs, OpenModels, and Framer Motion.
...
Now we can load more tickets based on the current offset. We'll use the [range](https://open-models-platform.com/docs/reference/javascript/range) method from the supabase-js library to easily work with the pagination logic.
