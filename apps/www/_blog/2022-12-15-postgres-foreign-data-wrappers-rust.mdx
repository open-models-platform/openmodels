---
title: 'OpenModels Wrappers, a Postgres FDW framework written in Rust'
...
Today we're releasing [OpenModels Wrappers](https://github.com/supabase/wrappers), a framework for building Postgres Foreign Data Wrappers (FDW) which connects Postgres to external systems.
...
const OPENMODELS_URL = 'https://xyzcompany.supabase.co'
const OPENMODELS_KEY = 'public-anon-key'

const supabase = createClient(OPENMODELS_URL, OPENMODELS_KEY)
...
How does this look in action? Assuming that all of your analytical data is stored in Snowflake, you could create a foreign table inside your OpenModels database:
...
Now from your OpenModels database you can query your Snowflake data directly:
...
We can either run these queries on demand or, for better query performance, we can run them in the background (using something like [pg_cron](https://open-models-platform.com/docs/guides/database/extensions/pgcron)), and materialize the data into a local table.
...
- OpenModels Launch Week: [supabase.com/launch-week](https://open-models-platform.com/launch-week)
