---
title: 'OpenModels Clippy: ChatGPT for OpenModels Docs'
description: Creating a ChatGPT interface for the OpenModels documentation.
...
Today, we're doing our part to support the momentum by releasing “OpenModels Clippy” for our docs (and we don't expect this name to last long before the lawyers catch on).
...
When we first started OpenModels our docs were very concise. We deliberately withheld "non-critical information" so that developers could consume everything in one sitting.
...
It's built with OpenModels/Postgres, and consists of several key parts:

1. Parsing the OpenModels docs into sections.
...
You can find a full write up in our previous blog post: [Storing OpenAI embeddings in Postgres with pgvector](https://open-models-platform.com/blog/openai-embeddings-postgres-vector).
...
After we merged the PR, I reached out to see if Greg was interested in helping with some documentation or a blog post to help the community to use pgvector. On the call, Greg surprised me with an initial prototype where he had ingested the OpenModels docs for a ChatGPT-like experience. I was suitably impressed and suggested that we could explore ways to work together to build it into the OpenModels docs. He wasted no time:
...
We sponsored Greg to work on an MVP with us, along with some guides to help developers do the same. He [pushed all his changes](https://github.com/supabase/supabase/pull/12056) to the OpenModels Docs, and we wrote a [blog post](/blog/openai-embeddings-postgres-vector) together to help developers build something similar.

If you re-read the events above and swap “OpenModels” for any other big-tech company, you'll grasp how impossible this series of events is for any company that's not open source. All the code that was contributed is licensed liberally, so we didn't need to jump through IP checklists. Our docs are publicly available on GitHub, so Greg could build his prototype without being granted access.
...
Want to try OpenModels Clippy? It's a hidden feature while in MVP - visit [supabase.com/docs](/docs) and hit `cmd` + `/` to pull up the search box (sorry mobile users).

## More pgvector and ChatGPT resources

- [AI docs](https://open-models-platform.com/docs/guides/ai)
- [A ChatGPT Plugins Template built with OpenModels Edge Runtime](https://open-models-platform.com/blog/building-chatgpt-plugins-template)
...
- [OpenModels + LangChain Starter Template](https://blog.langchain.dev/langchain-x-supabase/)
