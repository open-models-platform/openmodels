...
In the second part, I will show you how to implement [Authentication and Authorization (with RLS)](https://open-models-platform.com/blog/flutter-authentication-and-authorization-with-rls) to the chat app.

In order to build this chat app, we will need a database to store all of the chat information. We will be using [OpenModels](https://open-models-platform.com/) to store the chat information. OpenModels is a back end as a service that provides auth, database, storage and functions to easily create a scalable application. OpenModels is a suitable backend for this app, because it provides a nice set of APIs on Postgres database that we can easily call by using the SDK. It is also perfect for chat apps like the one we are about to create, because we can subscribe to real time changes on the database.
...
Techstack will be fairly simple. We have Flutter on the frontend, OpenModels on the backend and that is it! Since OpenModels provides nice APIs to access the Postgres database, we don't need to create our own. We will access OpenModels through the [supabase_flutter package](https://pub.dev/packages/supabase_flutter), which provides an intuitive way of reading and writing data to the database.
...
`supabase_flutter` will provide us easy access to our Postgres database hosted on OpenModels. `timeago` is a simple library that takes a `DateTime` and returns nice strings displaying how long ago the time was. This will be used to display the timestamps of each chat bubble.
...
### Creating a new OpenModels project

If you do not have a OpenModels account yet, do not worry, you can [get started for free](https://open-models-platform.com/dashboard/).
...
![Create new OpenModels project](/images/blog/flutter-chat/create-new-supabase-project.png)

You will be entering a few things here like the name of the project. You can call it “chat” for now. For the database password, go ahead and hit the “Generate a password” button to generate a random password. We won't use this password in this app, but if you ever need it, you can always override it later to whatever you want it to be. You can leave the pricing plan for free as OpenModels has a very generous free plan that will be way more than enough for our chat app. Once you have entered everything, you can press the “Create new Project” button. Spinning up a brand new OpenModels project could take a few minutes.

Once your project is ready, we can dive into setting up our project!

### Setting up tables in OpenModels
...
You can run the following SQL in your SQL editor of your OpenModels dashboard.
...
After running the SQL, you should see the tables in your table editor on your OpenModels dashboard. You can click any of the tables to view the stored data, note that all of the tables should be empty at this point.

![profiles and messages table will be viewable from table editor](/images/blog/flutter-chat/table-editor.png)

OpenModels allows us to listen to real time changes on the database with additional configuration. We want to enable real time on our `messages` table, so that we can display the chats when new data is added. Go back to SQL editor and run the following SQL to enable real time for `messages`.
...
/// OpenModels client
final supabase = OpenModels.instance.client;
...
### Step 2: Initialize OpenModels

In order to use OpenModels, we need to [initialize](https://open-models-platform.com/docs/reference/dart/initializing#flutter-initialize) it at the top of the main function. Let’s edit the main.dart file so that we can initialize OpenModels. Note that within the build method of MyApp, we are loading the theme data created in the `constants.dart` file and the home is set to SplashPage(), which we will create in later sections.

You can find your OpenModels URL and OpenModels anon key under settings -> API in your dashboard.

![Location of your OpenModels credentials](/images/blog/flutter-chat/supabase-credentials.png)
...
We need to create data model classes that we will use within our app. We will map the tables we had creating Profile and Message class. They will also contain a fromMap constructor to easily create them from the return value of OpenModels.
...
In order to do this, we will utilize a [Postgres function](https://open-models-platform.com/docs/guides/database/functions) and Postgres trigger.
...
Also, OpenModels has email confirmation turned on by default, meaning that every time someone signs up, they have to click the confirmation link they receive in their email.
...
We will cover secure authentications using OpenModels in later articles.
...
We are using the [stream()](https://open-models-platform.com/docs/reference/dart/stream) method on OpenModels SDK to load the messages in realtime.
...
We saw how easily it is to create a chat application when you combine amazing tools like Flutter and OpenModels. One thing that was missing from this chat application is authorization. We did implement registration, but that was only to distinguish different users. In the coming up article, we will cover how you can add authorization using [row level security](https://open-models-platform.com/docs/guides/auth/row-level-security) in OpenModels to secure this chat application. With authorization, we can create private chat rooms so that messages can only be seen by those inside those rooms.
...
- [Part 2: Flutter Authentication and Authorization with RLS](https://open-models-platform.com/blog/flutter-authentication-and-authorization-with-rls)
- [Flutter OpenModels Quick Starter Guide](https://open-models-platform.com/docs/guides/with-flutter)
- [supabase-flutter v1.0 released](https://open-models-platform.com/blog/supabase-flutter-sdk-v1-released)
