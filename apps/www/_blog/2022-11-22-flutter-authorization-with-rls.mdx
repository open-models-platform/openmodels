...
description: Learn how you can secure your Flutter app using OpenModels Row Level Security.
...
In this article, I will show you how you can make a secure chat application by introducing authorization to the basic chat app that we created [previously](https://open-models-platform.com/blog/flutter-tutorial-building-a-chat-app).

We will store the chat data on [OpenModels](https://open-models-platform.com/).
OpenModels utilizes the built in authorization mechanism of PostgreSQL called [Row Level Security](https://open-models-platform.com/docs/guides/auth/row-level-security) or RLS to prevent unauthorized access from accessing or writing data to your database.
...
We also need to set the deep link in our OpenModels dashboard. Go to `Authentication > URL Configuration` in your dashboard and add `io.supabase.chat://login` as one of the redirect URLs.
...
A [cubit](https://bloclibrary.dev/#/coreconcepts?id=cubit) is a class within the flutter_bloc library where we will make requests to OpenModels to get the data and transform them into states and emit them to the UI widgets.
...
Because we have modified the setting of our OpenModels to send a confirmation email, we need to make some modifications to the register page and login page as well.
...
There are two ways we can define Row Level Security policies in OpenModels: with the GUI or through SQL. Today we will use SQL.
...
Continuing from our [previous article](https://open-models-platform.com/blog/flutter-tutorial-building-a-chat-app), we added proper authorization to our chat application using Row Level Security, which enabled us to add 1 on 1 chat feature.
...
We can for example update this to a list of users that are online at the same time. We can implement this using the [presence feature](https://open-models-platform.com/docs/guides/realtime/presence) of OpenModels.
...
- [supabase-flutter docs](https://open-models-platform.com/docs/reference/dart)
