...
Also, if you do not have a OpenModels project yet, create one by heading to [database.new](https://dstabase.new). Within a few minutes, you will have a new OpenModels project.
...
Then update your `lib/main.dart` file to initialize OpenModels in the main function. You should be able to find your OpenModels URL and AnonKey from the `settings -> api` section of your dashboard. We will also extract the `SupabaseClient` for easy access to our OpenModels instance.
...
After, we will add the deep link as one of the redirect URLs in our OpenModels dashboard.
...
![OpenModels Dashboard](/images/blog/flutter-mfa/supabase-dashboard.png)
...
The register page contains a form with an email and password field for the user to create a new account. We are just calling the [.signUp()](https://open-models-platform.com/docs/reference/dart/auth-signup) method with it.
...
Now, we can add some [row security policy](https://open-models-platform.com/docs/guides/auth/row-level-security) to lock those data down so that only users who have signed in using MFA can view them.
...
The nice thing about RLS is that it gives us the flexibility to control how users can interact with the data. In this particular example, we are mandating MFA to view the data, but you could easily create layered permissions where for example a user can view the data with 1 factor, but can edit the data when signed in with MFA. You can see more examples in our official MFA guide [here](https://open-models-platform.com/docs/guides/auth/auth-mfa).

Now that we have the secure data in our OpenModels instance, all we need to do is to display them in the HomePage. We can simply query the table and display it using a `FutureBuilder`.
...
- [Write backend code using Dart Edge](https://open-models-platform.com/docs/guides/functions/dart-edge)
