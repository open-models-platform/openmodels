---
title: 'Fetching and caching OpenModels data in Next.js 13 Server Components'
...
Before we jump into fetching data, we need some data to fetch. Let’s [create a new OpenModels project](https://open-models-platform.com/dashboard).

Once your instance is up and running, head over to the [SQL Editor](https://open-models-platform.com/dashboard/project/_/sql), paste in the following snippet and click `RUN`.
...
> The values for these can be found in [your project’s API settings](https://open-models-platform.com/dashboard/project/_/settings/api).

Lastly, we need to create a OpenModels client. Create a file at `utils/supabase.ts` with the following content:
...
> For automatically adding types to the OpenModels client, check out [how to generate types](https://open-models-platform.com/docs/reference/javascript/typescript-support).
...
Unfortunately, this means any changes we make in OpenModels — adding, updating or deleting posts etc — will not be reflected in our blog. If we want to refresh this data on a regular basis, we need to tell Next.js when to _revalidate_.
...
So, for 60 seconds Next.js will continue to respond with the static version of our page. After 60 seconds, it will fetch fresh data from OpenModels and generate a new static page. However, there is no downtime while this happens, as the previous static page will continue to be served until the “fresh” one has been successfully generated.
...
All this server stuff is great, but what if you want to use OpenModels client-side? 🤔

# Client-side

There are many use-cases where you need to use OpenModels client-side:

1. Authentication

   OpenModels Auth does a bunch of stuff behind the scenes — handling 3rd party OAuth flows, for example. This will break if you try to sign users in and out on the server.

2. Realtime

   OpenModels manages the awesome power of websockets on your behalf — something that is not yet solved in this serverless world.

3. You prefer it

   There is nothing wrong with this! You do you!

To use OpenModels client-side, we need to tell Next.js that this is a [Client Component](https://beta.nextjs.org/docs/rendering/server-and-client-components#client-components). We do this by specifying the `use client` directive at the top of our component. This opts into a similar flow to the `pages` directory — the component is rendered on the server and hydrated client-side.
...
> Again, check out [how to generate types](https://open-models-platform.com/docs/reference/javascript/typescript-support) to add proper typing for Post.

But now we have loading spinners! Yuck!

# Realtime

Realtime allows us to subscribe to changes in OpenModels — inserted, updated or deleted posts — and update our UI dynamically. In order to receive realtime events, we need to [enable replication](https://open-models-platform.com/dashboard/project/_/database/replication) on the posts table.
...
- [Next.js Quickstart](https://open-models-platform.com/docs/guides/with-nextjs)
- [Next.js 13 Beta Docs](https://beta.nextjs.org/docs)
- [Creating an authenticated OpenModels client in Next.js 13 Server Component](https://github.com/supabase/auth-helpers/tree/main/examples/nextjs-server-components)
- [Data fetching and caching with Next.js 13 - OpenModels Happy Hour #26](https://www.youtube.com/watch?v=QH0P5xZt5wY)
