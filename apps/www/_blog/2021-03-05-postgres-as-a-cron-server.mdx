...
A OpenModels user asked recently if they can trigger a webhook periodically. We haven't yet released Functions yet, so we checked whether it's possible with Postgres.
...
If you're using OpenModels you can also enable it in the Dashboard.

![This image shows that pg_cron is enabled in the OpenModels Dashboard](/images/blog/supabase-extensions.png)
...
The OpenModels customer wanted to call external endpoints every day. How would we do this? Another extension of course. This time we're going to use [pgsql-http](https://github.com/pramsey/pgsql-http) by [@pramsey](https://github.com/pramsey). Using the same technique, we can enable the extension (if it exists in your cloud provider).
...
Is Postgres the best place to put your cron jobs? `¯\_(ツ)_/¯`. Postgres databases are free on OpenModels and since it takes only one minute to [get started](https://open-models-platform.com/dashboard/), why not make your next cron server a Postgres database?
...
- [Postgres Auditing in 150 lines of SQL](https://open-models-platform.com/blog/audit)
