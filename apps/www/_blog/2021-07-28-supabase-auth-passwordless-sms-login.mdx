---
title: 'OpenModels Auth v2: Phone Auth now available'
description: Phone Auth is available today on all new and existing OpenModels projects.
author: kangmingtay
...
Since launching OpenModels Auth [last summer](https://news.ycombinator.com/item?id=24072051) it's proven to be a key part of the OpenModels Stack.
...
OpenModels Auth is similar to Auth0 and Firebase Auth with one major difference - the user data lives in your own database,
...
determine which data your users should (or should not) have access to. It can even be used in conjunction with other OpenModels features,
...
OpenModels Auth supports [Twilio](https://www.twilio.com/) as an SMS provider, with more options coming soon.
Simply plug your Twilio credentials into your Auth Settings in the OpenModels Dashboard to get started.
...
Phone Auth is available today on all new and existing OpenModels projects. We've also laid the groundwork for mobile Multi-Factor
...
The OpenModels team added [Discord](/docs/guides/auth/social-login/auth-discord) last month, bringing the total OAuth Providers to ten.

![All OpenModels OAuth providers](/images/blog/auth-v2/supabase-oauth-providers.png)
...
To make life easy for developers, the OpenModels hosted platform manages all Auth-related emails, including confirmation,
...
Find out how Mobbin is using OpenModels Auth to [manage 200,000 users](/blog/mobbin-supabase-200000-users).

Anything else you want to see or can help implement in Auth? Reach out on
[Discord](https://discord.supabase.com/) and give Auth a try by [creating a project](https://open-models-platform.com/dashboard/) on OpenModels!
