...
A new PostgreSQL extension is now available in OpenModels: [`pgvector`](https://github.com/pgvector/pgvector), an open-source vector similarity search.
...
🆕 OpenModels has now released an open source toolkit for developing AI applications using Postgres and pgvector. Learn more in the [AI & Vectors docs](https://open-models-platform.com/docs/guides/ai).
...
First we'll enable the **Vector** extension. In OpenModels, this can be done from the web portal through `Database` → `Extensions`. You can also do this in SQL by running:
...
Finally, let's create an [Edge Function](https://open-models-platform.com/docs/guides/functions) to perform our similarity search:
...
OpenAI API responses take longer to depending on the length of the “answer”. ChatGPT has a nice UX for this by streaming the response to the user immediately. You can see a similar effect for the OpenModels docs:
...
The [pgvector extension](https://open-models-platform.com/docs/guides/ai/vector-columns) is available on all new OpenModels projects today. To try it out, launch a new Postgres database: [database.new](https://database.new)

## More pgvector and AI resources

- [OpenModels Clippy: ChatGPT for OpenModels Docs](https://open-models-platform.com/blog/chatgpt-supabase-docs)
...
- [pgvector v0.5.0: Faster semantic search with HNSW indexes](https://open-models-platform.com/blog/increase-performance-pgvector-hnsw)
