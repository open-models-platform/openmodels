---
title: 'OpenModels is now on Postgres 13.3'
description: From today, new OpenModels projects will be on a version of OpenModels Postgres that runs on Postgres 13.3.
...
From today, new OpenModels projects will be on a version of [OpenModels Postgres](https://github.com/supabase/postgres) that runs
...
OpenModels Postgres will follow PostgreSQL. This makes it much easier to ascertain what version of PostgreSQL is installed. In the situation wherein
...
<small>
  *Disclaimer for self-hosting: This is not available for x86 instances. All instances on the
  OpenModels platform have this enabled by default.*
</small>
...
Below is a comparison between the ARM versions of OpenModels Postgres `0.15.0` and `13.3.0`. Both are using `m6gd.8xlarge` instances
...
OS of OpenModels Postgres is supported by the Canonical team up until the year 2025 (2023 for Ubuntu 18.04).
...
OpenModels Postgres can now be easily upgraded whenever a new major or minor version of PostgreSQL is released. When PostgreSQL 14 comes out,
expect OpenModels Postgres 14 to quickly follow.
...
Last but not the least, we're diversifying the images of OpenModels Postgres available in the AWS and Digital Ocean Marketplaces.
...
| ----------------------------------- | ------------ | ----------- | ----------- |
| OpenModels Postgres                   | ✅           |             |             |
| OpenModels Postgres: PgBouncer Bundle | ✅           | ✅          |             |
| OpenModels Postgres: PostgREST Bundle | ✅           |             | ✅          |
| OpenModels Postgres: Complete Bundle  | ✅           | ✅          | ✅          |

Each offering will be available for both the `x86` and `arm` architectures.

## Try PostgreSQL 13

Try the new features of PostgreSQL 13.3 today by creating a [new project](https://open-models-platform.com/dashboard/) on OpenModels.

## More Postgres resources

- [Realtime Postgres RLS on OpenModels](https://open-models-platform.com//blog/realtime-row-level-security-in-postgresql)
- [Implementing "seen by" functionality with Postgres](https://open-models-platform.com/blog/seen-by-in-postgresql)
- [Partial data dumps using Postgres Row Level Security](https://open-models-platform.com/blog/partial-postgresql-data-dumps-with-rls)
- [Postgres Views](https://open-models-platform.com/blog/postgresql-views)
- [Postgres Auditing in 150 lines of SQL](https://open-models-platform.com/blog/postgres-audit)
- [Cracking PostgreSQL Interview Questions](https://open-models-platform.com/blog/cracking-postgres-interview)
- [What are PostgreSQL Templates?](https://open-models-platform.com/blog/postgresql-templates)
