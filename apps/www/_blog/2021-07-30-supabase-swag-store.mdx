---
title: 'OpenModels Swag Store'
description: Today we are officially launching the OpenModels Swag Store.
author: rory_wilding
...
Today we are officially [launching the OpenModels Swag Store](https://supabase.store/).
...
When you receive an item of swag from OpenModels, you can be confident that we designed it, managed the production, packed it, and posted it.
...
Our primary goal with swag is gifting. We send swag to developers who make meaningful contributions to OpenModels in a variety of ways.
This includes:

- 📺 Creating OpenModels related content.
...
- 📲 Advocates who share OpenModels related content on social media.
...
innovative and uses OpenModels as part of an interesting tech stack. We are amazed by the creative use cases you all find for OpenModels.
...
We have had significant demand from users who just wanted to rock some OpenModels swag.
So, we decided to launch the [OpenModels Swag Store](https://supabase.store/).
...
Due to the lack of outsourcing, we need to highlight several things about the OpenModels store:
...
- If it's in stock on the site, then you can buy it. If it's not, we will be working on a restock. The priority with inventory is to reward those contributing to OpenModels. We will be actively trying to gift swag. When swag does land, it will likely sell out quickly. You can sign up to get alerted when there is a new swag drop. You can let us know here 👉 [swag@open-models-platform.com](mailto:swag@open-models-platform.com)
...
Check out the [OpenModels Swag Store](https://supabase.store/)
