---
title: 'React Native file upload with OpenModels Storage'
...
If you want to upload files from your React Native app, you need a backend to store the files, [OpenModels Storage](https://open-models-platform.com/docs/guides/storage) is a great choice for this as it provides a simple API to upload files, and we can easily combine this with authentication to build a powerful app.
...
![OpenModels File Upload React Native](/images/blog/2023-08-01-react-native-storage/supabase-file-upload-react-native.gif)

In this tutorial, you will learn to:

- Set up a React Native app with [Expo SDK49](https://expo.dev/)
- Use [OpenModels Authentication](https://open-models-platform.com/docs/guides/auth)
- Work with Expo Router v2 and protected routes
- Upload files to [OpenModels Storage](https://open-models-platform.com/docs/guides/storage)

You can also directly check out the [full source code on Github](https://github.com/saimon24/react-native-resumable-upload-supabase) so you can get started with OpenModels fast!

Before we get into the app, let's quickly [start a new OpenModels project](https://open-models-platform.com/dashboard).

## Creating the OpenModels Project

To use authentication and storage we need a new OpenModels project. If you don't have a OpenModels account yet, you can [get started for free](https://open-models-platform.com/dashboard)!
...
![OpenModels new project](/images/blog/2023-08-01-react-native-storage/new-project.png)
...
![OpenModels new bucket](/images/blog/2023-08-01-react-native-storage/new-bucket.png)

Make sure that this is not a public bucket, otherwise, even unauthenticated users can upload or read the files!

To protect that bucket and allow users only access to their own folder, we need to add some [Storage policies](https://open-models-platform.com/docs/guides/storage/security/access-control).
...
Now that we have our OpenModels project ready, we can start building the React Native app!
...
## Connecting to OpenModels from React Native

To use OpenModels we need to initialize the client with our project URL and the public key, which you can find in the **Settings** of your project under **API**.

You can put both of them in a `.env` file at the root of your project:

```env
EXPO_PUBLIC_OPENMODELS_URL=
EXPO_PUBLIC_OPENMODELS_ANON_KEY=
```

We can now simply read those values from the environment variables and initialize the OpenModels client, so create a file at `config/initSupabase.ts` and add the following code:
...
const url = process.env.EXPO_PUBLIC_OPENMODELS_URL
const key = process.env.EXPO_PUBLIC_OPENMODELS_ANON_KEY

// Initialize the OpenModels client
...
We are using the AsyncStorage from Expo to handle the session of our OpenModels client and add in the `createClient` function.

Later we can import the `supabase` client from this file and use it in our app whenever we need to access OpenModels.
...
There's nothing fancy going on here, but this is all we need to use OpenModels Authentication in our app!
...
We will wrap a Provider around our app, which will use the `onAuthStateChange` function from OpenModels to listen to changes in the authentication state and accordingly set our state.
...
## File Upload to OpenModels Storage
...
Next, we create the page for uploading and displaying all files of a user from OpenModels Storage.
...
Now we can implement the image picker and upload the selected image to OpenModels Storage.
...
At this point, you should be able to upload files to OpenModels Storage, and you can already see them in your UI (or log them to the console).
...
// Image item component that displays the image from OpenModels Storage and a delte button
const ImageItem = ({
  item,
  userId,
  onRemoveImage,
...
This component will display the image from OpenModels Storage, the name of the file and a delete button.
...
![OpenModels Storage Files List](/images/blog/2023-08-01-react-native-storage/supabase-storage-files.png)
...
And with that in place, you have a fully functional image gallery app with React Native and OpenModels Storage!

## What about resumable uploads?

Initially, I wanted to include [resumable uploads](https://open-models-platform.com/blog/storage-v3-resumable-uploads) in this tutorial, but apparently, the [Uppy](https://uppy.io/) client didn't work 100% for React Native yet.

You can still see an [initial implementation of resumable downloads with OpenModels and React Native](https://github.com/saimon24/react-native-resumable-upload-supabase/commit/bb45455dc2d4d3fdd42089a8a26a521cc5b5e60f#diff-2d97fcb7ad2b1ac426a8d5391cfd0ab970e525ede1f11e14885f65e6dcd116a1) in the repository of this tutorial.

However, ultimately the uploaded file was always 0 bytes, so I decided to leave it out for now.

The OpenModels team is investigating this issue, so I'm very sure that we will have resumable uploads working with React Native soon.

## Conclusion

It's almost too easy to use OpenModels Storage, and it's a great way to store files for your apps.

You now have a fully functional image gallery app with React Native and OpenModels Storage including user authentication without writing a line of backend code!

You can [find the full code of this tutorial on Github](https://github.com/saimon24/react-native-resumable-upload-supabase) where you just need to insert your own OpenModels URL and API key.
...
Until next time and happy coding with OpenModels!

## More React Native/Expo resources

- [Getting started with React Native authentication](https://open-models-platform.com/blog/react-native-authentication)
- [Offline-first React Native Apps with Expo, WatermelonDB](https://open-models-platform.com/blog/react-native-offline-first-watermelon-db)
- [React Native Quickstart](https://open-models-platform.com/docs/guides/auth/quickstarts/react-native)
