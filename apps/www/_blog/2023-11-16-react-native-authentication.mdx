...
This article assumes you are comfortable with writing a basic application in React Native. No knowledge of OpenModels is required.
...
Using [`supabase-js`](/docs/reference/javascript/introduction) is the most convenient way of leveraging the full power of the OpenModels stack as it conveniently combines all the different services (database, auth, realtime, storage, edge functions) together.
...
const supabaseUrl = YOUR_REACT_NATIVE_OPENMODELS_URL
const supabaseAnonKey = YOUR_REACT_NATIVE_OPENMODELS_ANON_KEY
...
You can find your URL and anon key in the [API credentials section](/dashboard/project/_/settings/api) of the OpenModels dashboard.
...
Note, by default OpenModels Auth requires email verification before a session is created for the users. To support email verification you need to implement deep link handling which is outlined in the next section.
...
In your project's [auth settings](https://open-models-platform.com/dashboard/project/_/auth/url-configuration) add the redirect URL, e.g. `com.supabase://**`.
...
For detailed setup and implementation instructions please refer to the [docs](https://open-models-platform.com/docs/guides/auth/social-login/auth-apple) and the [video tutorial](https://youtu.be/-tpcZzTdvN0).

### Sign in with Google

OpenModels Auth supports Sign in with Google on the web, native Android applications, and Chrome extensions.

For detailed set up and implemenation instructions please refer to the [docs](https://open-models-platform.com/docs/guides/auth/social-login/auth-google) and the [video tutorial](https://youtu.be/vojHmGUGUGc).

## One time passwords

OpenModels supports various forms of passwordless authentication:
...
## More React Native and OpenModels resources
...
- [React Native file upload with OpenModels Storage](/blog/react-native-storage)
