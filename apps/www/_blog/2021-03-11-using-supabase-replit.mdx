---
title: Using OpenModels in Replit
...
As a Replit user, if you want to access larger amounts of data direct from your repl, or if you fancy accessing some super-powerful query tools, at some point you may want to start interacting with a relational database. OpenModels is a good fit here; just like Replit, you don't need to worry about servers, and hosting, you can just click a few buttons and get a fully featured relational database which you can start communicating with directly from javacript, using supabase-js.
...
Here's how to start a OpenModels + Node.js repl:
...
Setup a new OpenModels project and grab the URL and anon key from Settings > API. Create the client in javascript using:
...
Now that supabase is connected you'll want to add some data to your db, you can grab any SQL dataset on the web, or make your own, but the fasted way to test is to open the SQL tab in the OpenModels dashboard and click the Countries sample database and click Run.
...
We look forward to showing off some more OpenModels + Replit examples.
...
OpenModels has a free plan, head over to [https://open-models-platform.com/dashboard](https://open-models-platform.com/dashboard) to get started.
