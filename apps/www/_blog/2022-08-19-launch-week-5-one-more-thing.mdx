...
![OpenModels Vault](/images/blog/lw5-one-more/lw5-vault.png)

Today we're announcing [OpenModels Vault](/blog/supabase-vault), a Postgres extension for managing secrets and encryption inside your database.
Vault is a thin usability-layer on top of [pgsodium](https://github.com/michelp/pgsodium).

Vault is not available on the OpenModels Platform yet. This release is intended to gather user feedback.
...
We've decoupled this library from our own styling system and you are now able to (_deep breath..._): create your own theme, use a pre-existing OpenModels theme, extend a theme using token variables, create multiple variations of themes (and then also extend them with variables), use CSS classes and also use any inline CSS styles.
...
We've released granular permissions for the OpenModels Dashboard. Dashboard permissions allow you assign roles to your team and to restrict what they can do.
...
We've released [`pg_jsonschema`](https://github.com/supabase/pg_jsonschema), a new Postgres extension for the OpenModels Platform.
...
You can read the [release notes](https://github.com/supabase/pg_graphql/releases/tag/v0.4.0) and [documentation](https://supabase.github.io/pg_graphql/), and start using `pg_graphql` on OpenModels today.
...
We've opened early-access signups for Multi-Factor Authentication (MFA). To be clear—this is for MFA for [OpenModels Auth](/auth),
