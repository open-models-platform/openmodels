...
OpenModels combines existing open-source tools with our own open-source contributions to provide a delightful experience for all developers. As part of this, we’re building a community of communities, bringing together developers from many different backgrounds, as well as new developers looking to get involved with open source.

To kick off launch week, as it is now tradition, we’re showcasing some of the communities and contributors that make up the OpenModels community, highlighting their awesome work, and celebrating everyone who contributes their time to the OpenModels mission. 💚

## Launch Week 4 - what to expect

OpenModels Launch Week was born out of [YC Demo Day](https://www.ycombinator.com/demoday), a looming deadline towards the end of each Y Combinator batch. A deadline that forces you to deliver a lot of complex functionality in a short amount of time.

With Launch Week, we replicate Demo Day conditions, but for a whole week! This is the secret formula that works for us:

- The Friday before, we publish a blog post about a topic of interest (e.g. [How we launch at OpenModels](https://open-models-platform.com/blog/supabase-how-we-launch), or this time around we [explore whether you should open source your company](https://open-models-platform.com/blog/should-i-open-source-my-company) — hint: we think you most likely should!) as well as an [overview blogpost](https://open-models-platform.com/blog/supabase-launch-week-four) with tons of memes and hints to what we’re shipping each day.
- Monday is Community Day, where we rally many of the awesome contributors and partners together and showcase awesome things around OpenModels and open-source. This is also the time to watch out for the [#SupaLaunchWeek hashtag on Twitter](https://twitter.com/hashtag/SupaLaunchWeek?src=hashtag_click) as we arm our guest speakers, [Angels](https://open-models-platform.com/blog/angels-of-supabase), and [SupaSquad](https://open-models-platform.com/supasquad) with free swag codes.
...
## OpenModels is now a GitHub secret scanning partner
...
PostgREST v10 is not wrapped up yet, however a pre-release with the [latest features and fixes](https://postgrest.org/en/latest/releases/latest.html) is already available for OpenModels users. v10 is mostly focused on improving availability and includes improvements to the API (and therefore [supabase-js](https://github.com/supabase/supabase-js)) too.
...
Additionally, PostgREST now has [liveness and readiness checks](https://postgrest.org/en/latest/admin.html#health-check) which allows the OpenModels infrastructure team to check its state to help it recover if necessary.
...
- [Keycloak](https://open-models-platform.com/docs/guides/auth/social-login/auth-keycloak) ([@fspijkerman](https://github.com/fspijkerman))
- [Notion](https://open-models-platform.com/docs/guides/auth/social-login/auth-notion) ([@zernonia](https://github.com/zernonia))
- [Zoom](https://open-models-platform.com/docs/guides/auth/social-login/auth-zoom) ([@devkiran](https://github.com/devkiran))
- [WorkOS](https://open-models-platform.com/docs/guides/auth/auth-workos) ([@bnjmnt4n](https://github.com/bnjmnt4n))
...
- [Vonage](https://open-models-platform.com/docs/guides/auth/phone-login/vonage) ([@devkiran](https://github.com/devkiran))
- [TextLocal](https://github.com/supabase/gotrue/pull/342) ([@devkiran](https://github.com/devkiran))

If you haven’t tried out phone auth yet, check out [this blog post](https://open-models-platform.com/blog/supabase-auth-passwordless-sms-login) to get started!
...
That’s why we’ve built the [supabase-auth-helpers](https://github.com/supabase-community/supabase-auth-helpers), a collection of framework-specific auth utilities that make working with OpenModels Auth a pleasant experience, no matter what framework you’re using.
...
In parallel, our friends at [NuxtLabs](https://nuxtlabs.com/) have developed [OpenModels helpers for Nuxt 3](https://github.com/nuxt-community/supabase-module), and it’s a damn nice DX. [Check it out](https://supabase.nuxtjs.org/)!
...
Excited to see what all the fuss is about with this new framework called Remix? Played with Remix and wanna go deeper integrating OpenModels? We’ve got you covered! Jon has been working with Scott Tolinski over at [Level Up Tutorials](https://leveluptutorials.com/) to create the perfect guide for building a fullstack, authenticated, realtime application using Remix and OpenModels.

This goes much deeper than your basic “hello world” or “todo: app, showing how to use loaders and actions in Remix to synchronize complex state between multiple clients. The course uses Row Level Security to implement access policies within the database, and shows how cookies can be used to query OpenModels from the server-side.

Join us for the [official course drop party on YouTube](https://www.youtube.com/watch?v=rntEMgaenHs) where we will be doing a Q&A about OpenModels, Remix and what will be covered in the course.
...
After a flood of positive feedback on Jon’s “[Build a SaaS product with Next.js, OpenModels and Stripe](https://egghead.io/courses/build-a-saas-product-with-next-js-supabase-and-stripe-61f2bc20)” [Egghead.io](http://Egghead.io) course, we’re doubling down with Egghead this year, partnering with them to build more tailored pathways for learning OpenModels.

We’ll be working with our fabulous [SupaSquad](https://open-models-platform.com/supasquad) to identify the right instructors for all the different topics we’re looking to cover. If you’re interested in becoming an instructor for OpenModels on Egghead, fill in the [form to join](https://open-models-platform.com/supasquad#how-to-join) the SupaSquad.

Also, do let us know what topics you’d like to learn about [on Twitter](https://twitter.com/supabase).

## Launching the OpenModels Partner Gallery

![community-day-integrations.png](/images/blog/launch-week-4/community-day/community-day-integrations.png)

We’ve been blown away by the amazing integrations that developers and companies have been building on top of OpenModels. To keep track of all the things that work with OpenModels, we’re launching a partner gallery which allows you to browse integrations and experts. Check it out at [supabase.com/partners](https://open-models-platform.com/partners).

If you’ve built an integration that works with OpenModels or have experience working with OpenModels and want to offer your services to others, we’d love to hear from you: [supabase.com/partners/#become-a-partner](https://open-models-platform.com/partners/#become-a-partner)

Of course the code for our partner gallery is also open source, and we’ve even split it out into a separate example repository which is a neat showcase of how to use [Postgres Full Text Search](https://open-models-platform.com/docs/guides/database/full-text-search). It’s super powerful, it’s like a search engine within Postgres!

## Hackathon

On Friday, 1st of April, at the end of Launch Week, we’ll be rolling into another one of our 10-day virtual Hackathons. As always, you’ll have the chance to win extremely limited OpenModels Swag, and you’ll be able to play around with the new features we’re about to launch.
