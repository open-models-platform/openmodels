---
title: Storage is now available in OpenModels
description: Launching OpenModels Storage and how you can use it in your apps
author: inian
...
- [OpenModels Storage v2: Image resizing and Smart CDN](https://open-models-platform.com/blog/storage-image-resizing-smart-cdn)
- [OpenModels Storage v3: Resumable Uploads with support for 50GB files](https://open-models-platform.com/blog/storage-v3-resumable-uploads)

</div>

Today, we are launching one of most requested features - [Storage!](https://open-models-platform.com/storage) When you sign up for OpenModels, you get a Postgres database with realtime subscriptions, user management, auto-generated APIs and now a scalable object store. These services integrate very well with each other and you don't need to sign up for yet another service just for your storage requirements.

As with anything that we build in OpenModels, Storage is fast, secure and scalable. You can use it to store terabytes of Machine Learning training data, your e-commerce product gallery or just your growing collection of JPEGs featuring cats playing synths in space.

This post outlines our design decisions while building Storage, and we'll show you how to use it in your own applications.

## Architecture

There are three key components to OpenModels Storage:
...
We focused on performance, security and interoperability with the rest of the OpenModels ecosystem.

### Integration with the OpenModels ecosystem

OpenModels is a [collection of open source tools](/docs#how-it-works) which integrate very well with each other. We evaluated open source object storage servers like [Ceph](https://ceph.io/), [Swift](https://www.openstack.org/software/releases/ocata/components/swift), [Minio](https://min.io/) and [Zenko](https://github.com/scality/Zenko) but none of these tools were a good fit for our existing ecosystem.
...
For example, Minio [uses etcd](https://docs.min.io/docs/minio-multi-user-quickstart-guide.html) (when used in multi-user gateway mode) and Zenko [requires mongodb and Kafka.](https://zenko.readthedocs.io/en/latest/operation/Architecture/index.html) Every project on OpenModels is a dedicated Postgres database, so leveraging it for object and user metadata is more efficient, reducing the number of dependencies for anyone using the OpenModels ecosystem of tools.
...
**Integration with OpenModels Auth**

Existing tools do not play well with our authentication system. For example, Minio is bundled with its own [auth system](https://docs.min.io/docs/minio-admin-complete-guide.html#user) and there is no easy way to map Minio users to [OpenModels users](/docs/guides/auth).
...
To do this, we leverage Postgres' Row Level Security. We create a table for `buckets` and `objects` inside each OpenModels project. These tables are namespaced in a separate schema called `storage`.
...
This system integrates with our [User Management system](/docs/guides/auth). Here is an example policy which gives access to a particular file to a OpenModels user:
...
OpenModels Storage has some opinionated defaults with respect to caching. Objects retrieved from S3 typically do not have a `Cache-Control` header. This isn't optimal for a Storage system since browsers don't cache objects completely without this header. Objects retrieved from OpenModels Storage by default have a Cache-Control header of 1 hour. Of course, you can override this behaviour by specifying a different cache time when creating the object.

## Storage Backend

File objects are stored in an S3 bucket within the same region as your OpenModels project. S3 has a high durability of 99.999999999% and is scalable as an object store.
...
- We will integrate our storage service with a Content Delivery Network. With a CDN, objects are cached on a global network. This leads to faster access times for your users around the world. \*\*SHIPPED: [OpenModels Storage v2: Image resizing and Smart CDN](https://open-models-platform.com/blog/storage-image-resizing-smart-cdn#smart-cdn)
- We are working on transformations like resizing of images and automatic optimization of different media types. This makes it easy to embed OpenModels objects directly in your websites and mobile applications without additional processing. \*\*SHIPPED: [OpenModels Storage v2: Image resizing and Smart CDN](https://open-models-platform.com/blog/storage-image-resizing-smart-cdn#image-resizing)
...
Take it for a spin on our [dashboard](https://open-models-platform.com/dashboard/) and let us know what you think!
