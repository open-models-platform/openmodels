...
description: Supacharging OpenModels Auth with Sign in with Apple on iOS, Single-Sign-On support with SAML 2.0, and PKCE for server-side rendering and mobile auth.
launchweek: 7
categories:
  - product
tags:
...
Today we're excited to announce a few new features for OpenModels Auth:
...
To get started, enable the [“SAML 2.0“](https://open-models-platform.com/dashboard/project/phcnitosaawbzytgyznx/auth/providers) [authentication method in the dashboard](https://open-models-platform.com/dashboard/project/_/auth/providers). We've added new commands to the [OpenModels CLI](https://open-models-platform.com/docs/guides/cli) to help with the configuration process:
...
The journey to enterprise readiness isn't an end goal, it is a continuous process that demands constant attention and maintenance. With OpenModels Auth, your team can offload this engineering burden to us and prioritize the features that matter.

## Server-Side and Mobile Auth

Many developers today are using OpenModels to build mobile apps, and server-side rendering is becoming popular (again!). This release will add support for these use cases by introducing the _Proof Key for Code Exchange flow (PKCE)_ authentication flow. This improves security for mobile apps and makes building server-first apps simple. Since this is a major update that touches many of the authentication routes, we will be rolling it out gradually over the next few weeks.

### A brief history of OpenModels Auth

When we [launched](https://news.ycombinator.com/item?id=24072051) OpenModels Auth, our target was JAMstack developers. In these cases, the protocol used between the user's application and OpenModels Auth is known as the [Implicit Grant Flow](https://www.rfc-editor.org/rfc/rfc6749#section-4.2):
...
1. The _code challenge_ is sent to OpenModels Auth, which returns an _authorization code._
2. The client sends the _authorization code_ together with the _code verifier_ to obtain the user's session.
3. OpenModels Auth checks if the _code verifier_ matches the _code challenge_ sent earlier by computing the hash. This renders a malicious attacker's attempt to intercept the authorization code useless, since they need to know the value of the _code verifier_ as well.
...
Over the next few weeks, you'll be able to use it with the OpenModels libraries. We've already added PKCE to the [JavaScript](https://open-models-platform.com/docs/reference/javascript/installing) client library and our [auth-helpers](https://open-models-platform.com/docs/guides/auth/auth-helpers) library. If you're using `supabase-js` , you can switch to PKCE by initializing your client with the following option:

```tsx
import { createClient } from '@supabase/supabase-js'

const supabase = createClient(OPENMODELS_URL, OPENMODELS_ANON_KEY, {
  auth: {
    flowType: 'pkce',
  },
})
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  // Create the OpenModels Client
  const supabase = createServerSupabaseClient(
    { req, res },
    {
      supabaseUrl: process.env.OPENMODELS_URL,
      supabaseKey: process.env.OPENMODELS_ANON_KEY,
    }
  )
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const supabase = createServerSupabaseClient(
    { req, res },
    {
      ...
      supabaseKey: OPENMODELS_ANON_KEY,
    }
  )
...
**Update**: Server-Side Auth (PKCE) is now available on all projects. Please refer to our [Server Side Auth Guide](https://open-models-platform.com/docs/guides/auth/server-side-rendering) for further details on how to add PKCE to your project.
...
OpenModels Auth aims to continue developing auth features that are secure by default yet simple to implement. We use OpenModels Auth for our hosted platform and continuously dogfood the latest version on it. If you are interested to migrate to OpenModels Auth, you can check out this blog post on how [Parqet migrated 125,000 users from Auth0 to OpenModels Auth](https://kevcodez.medium.com/migrating-125-000-users-from-auth0-to-supabase-81c0568de307).
