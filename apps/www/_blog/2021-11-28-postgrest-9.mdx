...
This works across all OpenModels client libraries and you can use it with any of the available operators (`gt`, `in`, etc.)
...
If you only use OpenModels default `auth` functions(`auth.email()`, `auth.uid()`, `auth.role()`), then no action is required because we have updated the functions to handle these changes transparently.
