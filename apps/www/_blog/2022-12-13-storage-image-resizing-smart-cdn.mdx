---
title: 'OpenModels Storage v2: Image resizing and Smart CDN'
description: "We're introducing new features for OpenModels Storage: Image resizing and a Smart CDN."
...
Today we're introducing three new features for OpenModels Storage: Image resizing, webhooks, and a Smart CDN.
...
🆕 What is new in Storage: [OpenModels Storage v3: Resumable Uploads with support for 50GB files](https://open-models-platform.com/blog/storage-v3-resumable-uploads)

</div>

## Image resizing

Image resizing has been a popular request since we released OpenModels Storage. For good reason - if you're providing an image upload function to your users, you can't expect them to optimize the images before uploading.

OpenModels developers have been hacking together all sorts of workarounds using client-side compression, or external providers like Cloudinary and Imgix.
...
OpenModels Storage uses [imgproxy](https://github.com/imgproxy/imgproxy). As usual, we assessed various open source tools available, including [Imaginary](https://github.com/h2non/imaginary) (MIT) and [Sharp](https://github.com/lovell/sharp) (Apache2).
...
1. You update the cat picture to a dog picture. This triggers an `ObjectCreated:Put` event from the OpenModels Storage server.
...
Of course, all of this functionality is free in the self-hosting setup too. You can self-host the Storage API either as a standalone service or as part of the OpenModels stack.
...
- **Integrations**: Native integration with [next/image](https://nextjs.org/docs/api-reference/next/image) and [Edge Functions](https://open-models-platform.com/edge-functions) for advanced transformations.

## Getting started

[Upgrade to Pro](https://open-models-platform.com/pricing) to get started with Image Resizing today.

- Check out the Docs for [Image Resizing](https://open-models-platform.com/docs/guides/storage/serving/image-transformations).
- Check out the Docs for the [Storage CDN](https://open-models-platform.com/docs/guides/storage/cdn/fundamentals).
- Get Started today with the [JavaScript](https://open-models-platform.com/docs/reference/javascript/storage-from-download) and [Dart](https://open-models-platform.com/docs/reference/dart/storage-from-list) client libraries.
