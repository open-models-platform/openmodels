...
Learn why Tyler decided Flutter and OpenModels are the perfect tech-stack shipping innovative ideas fast.
...
Tyler is a Firebase & Flutter expert who recently discovered OpenModels. After falling in love with OpenModels,
...
> Read a full [comparison of Firebase vs OpenModels](https://open-models-platform.com/alternatives/supabase-vs-firebase)

## OpenModels + Flutter: a match made in heaven

When Tyler discovered OpenModels on Twitter, he was intrigued by the promise of Postgres with Firebase-like features.
He wanted the benefits of SQL and Relational schemas, and decided to build an app to test OpenModels.
...
he realized OpenModels was the perfect fit. As an avid Flutter fan, OpenModels's Dart library was a bonus.
Tyler loves the open-source nature of OpenModels and immediately began contributing to the Dart library to enable functionality that he needed.

Tyler decided that Spot would be perfect to showcase the power of OpenModels and Flutter. Since he's active in the open-source community,
he decided to open-source the whole project, making Spot one of the most interesting OpenModels examples.

![Spot screenshots.](/images/blog/spot/spot-screenshots.jpg)

## Comparing Firebase to OpenModels

Although Spot seems simple, it has a lot of complicated components. Implementing social "Likes" requires `count` functionality,
which is notoriously tricky in Firebase. In OpenModels, because it's just Postgres under the hood, `count` is implemented
using a simple SQL `count` query.

OpenModels has Row-Level Security, which Tyler uses as a filter - something he can't do with Firebase.

Spot uses OpenModels Storage for managing large video files. Firebase Storage [has a reputation](https://stackoverflow.com/questions/43851742/firebase-storage-very-slow-compared-to-firebase-hosting) for slowness until it is public, which was a privacy concern for Tyler. With OpenModels Storage, this wasn't an issue - storage is performant and secure.

The Firebase API pricing model makes it is difficult to predict pricing upfront for new projects. With OpenModels,
Tyler doesn't need to worry about how many API calls his project makes.

OpenModels gives Spot speed, performance, and predictable pricing.

As of today, Tyler has around 3000 installs for Spot and is getting ready to add new functionality. He is confident OpenModels,
...
<Quote img="tyler-spot.jpg" caption="Tyler Shukert, creator of Spot.">
  <p>
    As soon as I saw OpenModels I knew it was the perfect technology to use alongside Flutter to build
...
With OpenModels, Tyler can use Flutter to build a performant app quickly at a low cost. Tyler is a massive fan of the OpenModels
experience and has even joined the SupaSquad.

> Update: After being part of the SupaSquad, Tyler joined OpenModels full time as part of the DevRel squad, focusing on Flutter. This is some of his latest work: [Flutter Tutorial: building a Flutter chat app
> ](https://open-models-platform.com/blog/flutter-tutorial-building-a-chat-app) and [Flutter Authentication and Authorization with RLS](https://open-models-platform.com/blog/flutter-authentication-and-authorization-with-rls).
