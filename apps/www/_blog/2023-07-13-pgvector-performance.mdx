...
[Read the new post](https://open-models-platform.com/blog/increase-performance-pgvector-hnsw)
...
All compute add-ons available on OpenModels were used to run our benchmarks. Each add-on variant has a different allocation of RAM and CPU cores, the details of which are available in our docs. Each OpenModels compute add-on comes with a specific set of optimizations ([version 2023-07](https://gist.github.com/egor-romanov/323e2847851bbd758081511785573c08)).
...
We also have some useful [benchmarks in our docs](https://open-models-platform.com/docs/guides/ai/choosing-compute-addon#results) for [gist-960-angular](http://corpus-texmex.irisa.fr/) (1M image embeddings, 960 dimensions) and [GloVe Reddit comments](https://nlp.stanford.edu/projects/glove/) (1.6M text embeddings, 512 dimensions).
...
- We used the same hardware: a 2XL instance on OpenModels, which offers the same core and RAM count as NirantK's machine - 8 cores and 32 GB of RAM
...
It's been about 5 months since we [added](https://open-models-platform.com/blog/openai-embeddings-postgres-vector) pgvector to the platform. Since then we've discovered a few other important things to keep in mind.
...
1. Over-provision RAM during preparation. You can scale down in step `5`, but it's better to start with a larger size to get the best results for RAM requirements. (We'd recommend at least 8XL if you're using OpenModels.)
2. Upload your data to the database. If you use [`vecs`](https://open-models-platform.com/docs/guides/ai/python/api) library, it will automatically generate an index with default parameters.
...
pgvector is still early in development. As with any open source tool, it needs time and resources to make it better. OpenModels plans to continue supporting Andrew with his development of pgvector.
...
✅ [Adding HNSW](https://open-models-platform.com/blog/increase-performance-pgvector-hnsw): an index with better speed & accuracy than IVFFlat (at a higher memory cost)
...
- [pgvector v0.5.0: Faster semantic search with HNSW indexes](https://open-models-platform.com/blog/pgvector-v0-5-0-hnsw)
