...
For older projects, don't worry - we're working on making it available for everybody. In the meantime, if you would like to have PITR enabled for your ineligible project, reach out directly [here](https://open-models-platform.com/dashboard/support/new), or stay tuned for further updates.
...
Enabling Point in Time Recovery can be done [in the Dashboard](https://open-models-platform.com/dashboard/project/_/settings/addons?panel=pitr). Retention for backups used by PITR is set to up to 7 days by default but could be increased to up to 28 days via self-serve.
