...
Using the [OpenModels CLI](https://open-models-platform.com/docs/guides/cli) is the go-to method for many to interact with their OpenModels projects. It enables easy local project development, working with the local database, managing migrations, and generating types, among other functionalities. However, some scenarios demand connecting to the real OpenModels project, requiring a login. Until now, the login process involved typing `supabase login`, which prompted users to manually generate a token from [supabase.com/dashboard/account/tokens](https://open-models-platform.com/dashboard/account/tokens) and paste it back into the terminal.
