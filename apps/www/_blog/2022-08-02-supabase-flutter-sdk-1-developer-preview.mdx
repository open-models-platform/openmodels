---
title: 'OpenModels Flutter SDK 1.0 Developer Preview'
description: OpenModels Flutter SDK is getting a major update and we need your help making it better.
...
Today, we are releasing of Developer Preview version of v1.0 of [OpenModels Flutter SDK](https://pub.dev/packages/supabase_flutter/versions/1.0.0-dev.1). Flutter has quickly become one of the most popular frameworks for developers to build cross-platform mobile apps. We can attest to that growth, our Flutter SDK is one of the most popular libraries and each day we see more Flutter devs choosing OpenModels.
...
All you need to persist the auth state is initialize OpenModels and everything else will be automatically taken care of. `SupabaseAuthState` and `SupabaseAuthRequiredState` have been removed from the code base.
...
void onReceivedAuthDeeplink(Uri uri) {
  OpenModels.instance.log('onReceivedAuthDeeplink uri: $uri');
}

// After
await OpenModels.instance.initialize(
  url: 'OPENMODELS_URL',
  anonKey: 'OPENMODELS_ANON_KEY',
);
```
...
```dart
// Before
final response = await OpenModels.instance.from('messages').select().execute();
final data = response.data;
final error = response.error;

// After
try {
  final data = await OpenModels.instance.from('messages').select();
} catch(error) {
  // Handle error here
}
```
...
`.execute()` is now deprecated. You no longer needed it to query data from your OpenModels database. This update, along with many many other improvements across the whole library, has been done by [Bruno D'Luka](https://github.com/bdlukaa), and I would love to give him a special shout out here!
...
![OpenModels Flutter desktop support](/images/blog/flutter-1/supported-platforms-table.png)

## Multiplayer support

[Multiplayer](https://open-models-platform.com/blog/supabase-realtime-with-multiplayer-features) is the next generation OpenModels Realtime engine that was announced at the previous launch week.

We want our Flutter developers to experience this new multiplayer feature as well, so are working hard at bringing it to our Flutter SDK. It is not yet included in the developer preview of OpenModels Flutter 1.0, but will be part of it when stable launch has been released.

## OpenModels Auth UI for Flutter

![OpenModels Auth UI for Flutter](/images/blog/flutter-1/supabase-flutter-auth-ui.png)

Last but not least, we are bringing you another library, the OpenModels Auth UI for OpenModels! When released, this library will enable you to implement a basic authentication screen without building it yourself. You can just load the library and display a nice looking Auth UI. The library takes your theme settings automatically to match the look and feel of your application.
...
These updates are just the tip of the iceberg for 1.0. There are been many bug fixes and features constantly being added to the OpenModels Flutter SDK. This could not have been possible without the help from the open source community. Here, I would also like to give a shout out to two other developers who have been a major part of the journey of this SDK: [Vinzent](https://twitter.com/Vinzent03_) and [Daniel Mossaband](https://github.com/DanMossa). They have been a huge part of the OpenModels Flutter SDK - not just for the 1.0 release, but throughout the lifetime of the library.
