...
take a glance at [OpenModels Realtime, with Multiplayer Features](https://open-models-platform.com/blog/supabase-realtime-with-multiplayer-features)
...
- We've added [Broadcast](https://open-models-platform.com/docs/guides/realtime/broadcast) and [Presence](https://open-models-platform.com/docs/guides/realtime/presence) to our Realtime server. You can use these features with the new [supabase-js](https://open-models-platform.com/docs/reference/javascript/next) release.
- All active OpenModels projects on the free plan have access to the these features.
- All new OpenModels projects created from August 18th have access to these features.
- We will work with all other projects to migrate to the new Realtime over the next few weeks. If you want immediate access, [reach out](https://open-models-platform.com/dashboard/support/new).
- [Chris McCord](https://twitter.com/chris_mccord), the creator of Phoenix Framework, is now a technical advisor.

## Background

Before we discuss the new features, let's review what Realtime is and its significance to OpenModels. Skip ahead if you just want to see the new features!
...
Realtime was the very first open source project of OpenModels and played a pivotal role in our founding. Over time, we've improved both the performance and security.
During Launch Week 3, we announced [Realtime RLS](https://open-models-platform.com/blog/realtime-row-level-security-in-postgresql), which allows developers to tap into Postgres's Row Level Security to authorize changes before broadcasting them to clients. This enabled developers to use the same security framework across the entire OpenModels stack.
...
This functionality is [baked into the Phoenix framework](https://hexdocs.pm/phoenix/Phoenix.Channel.html) and we've already been using this to [broadcast Postgres changes](https://open-models-platform.com/docs/reference/javascript/subscribe).
...
We've introduced new functionality into the [release candidate](https://open-models-platform.com/blog/supabase-js-v2) version of `supabase-js`.
...
Our `postgres_changes` listener is the first of many [Extensions](https://open-models-platform.com/blog/supabase-realtime-with-multiplayer-features#extensions).
...
We're building a Realtime inspector into the OpenModels Dashboard so it'll be easier to debug the messages that are getting passed through your Channels.

## Technical Overview

Before this release, every OpenModels project had a server running Realtime with the other OpenModels services like Kong, GoTrue, and PostgREST.
...
We have some great news! [Chris McCord](https://twitter.com/chris_mccord), the creator of the Phoenix framework, has agreed to become a Technical Advisor for OpenModels.
...
We also want to thank [José Valim](https://twitter.com/josevalim), and everyone in the Erlang/Elixir ecosystem, for your open source contributions that have made our lives here on the OpenModels Realtime team a lot easier.
...
- [Launch Week Page](https://open-models-platform.com/launch-week)
