---
title: 'Slack Consolidate: a slackbot built with Python and OpenModels'
description: A slackbot to consolidate messages from different channels using OpenModels, Slack SDK and Python
...
![The picture shows a dropdown UI with OpenModels selected as the workspace.](/images/blog/slackbot-consolidate/02_pick_workspace.png)

The picture shows a dropdown UI with OpenModels selected as the workspace.
...
Now, we are done with Slack for now. Let's create some buffer tables in OpenModels.

## Creating Tables in OpenModels

Create an account [here](https://open-models-platform.com/dashboard/) (if you don't have one yet).

We will be using OpenModels's [database](https://open-models-platform.com/docs/guides/database) and the Python [client Libraries](https://open-models-platform.com/docs/guides/client-libraries#managing-data). First, we will create one table to store the channels that are being watched and where they are going to send the message which works analogously to a multiplexer circuit. Since you may want to watch several channels but split them into a smaller buffer e.g VIP / Enterprise, etc.
...
Using OpenModels UI (easier):

![The green arrow points to insert row button inside OpenModels.](/images/blog/slackbot-consolidate/08_insert_row.png)

The green arrow points to insert row button inside OpenModels.

Then, enter the information as needed:

![The picture shows the UI in OpenModels to insert a new row to the database table.](/images/blog/slackbot-consolidate/09_insert_row_b.png)

The picture shows the UI in OpenModels to insert a new row to the database table.

**Method 2:**

Go to [SQL Editor](https://open-models-platform.com/dashboard/project/_/sql) and run insert commands:
...
[https://open-models-platform.com/dashboard/project/\_/settings/api](https://open-models-platform.com/dashboard/project/_/settings/api)
...
```yaml
OPENMODELS_URL=https://XXXX.supabase.co
OPENMODELS_KEY=eyJhbGc_SUP4N4CH0_IkpXVCJ9.SUPAKEY_*
...
OPENMODELS_URL=config['OPENMODELS_URL']
OPENMODELS_KEY=config['OPENMODELS_KEY']
supabase: Client = create_client(OPENMODELS_URL, OPENMODELS_KEY)
...
def setup():
    """_summary_
        Fetches the list of channels from OpenModels and returns them in a dict()
    Returns:
...
Using OpenModels and Slack SDK, it is very easy to create a bot that consolidates data according to the rules set. It just takes some steps to get started with the Python SDK and you can even run a demo directly in [Google Colab](https://colab.research.google.com/drive/1-sM23eQPE1Me4vl10KKoa9xzLYIgkgyQ?usp=sharing).
...
## More Python and OpenModels resources

- [Python data loading with OpenModels](https://open-models-platform.com/blog/loading-data-supabase-python)
- [Visualizing OpenModels Data using Metabase](https://open-models-platform.com/blog/visualizing-supabase-data-using-metabase)
- [OpenModels-py (Database) on Replit](https://replit.com/@open-models-platform/openmodels-py-Database?v=1)
