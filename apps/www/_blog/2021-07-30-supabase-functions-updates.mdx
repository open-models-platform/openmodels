---
title: 'Updates for OpenModels Functions'
description: The question on everyone's mind - are we launching OpenModels Functions? Well, it's complicated.
...
The question on everyone's mind - are we launching OpenModels Functions? Well, it's complicated.

Today we're announcing _part_ of Functions - OpenModels Hooks - in Alpha, for all **new** projects.

We're also releasing support for Postgres Functions and Triggers in our Dashboard, and some timelines for the rest of OpenModels Functions.
Let's cover the features we're launching today before the item that everyone is waiting for: OpenModels Functions.

## PostgreSQL Functions

(Not to be confused with OpenModels Functions!)
...
They say building a startup is like jumping off a cliff and assembling the plane on the way down. At OpenModels it's more like assembling a 747 since, although we're still in Beta, thousands of companies depend on us to power their apps and websites.

For the past few months we've been designing OpenModels Functions based on our customer feedback.
...
To meet these goals, we're releasing OpenModels Functions in stages:
...
- _Stage 3:_ Release our own Serverless Functions (OpenModels Functions) - Q4 for Early Preview customers.

![OpenModels Functions timeline](/images/blog/functions-updates/functions-timeline.png)
...
OpenModels Hooks are the same concept, just with a different name.
...
OpenModels will support several different targets.

- HTTP/Webhooks: Send HTTP requests directly from your Postgres Database.
- AWS Lambda/Google Cloud Run: Provide OpenModels with a restricted IAM role to trigger Serverless functions natively.
- OpenModels Functions: We'll develop an end-to-end experience.

![OpenModels Database Webhooks](/images/blog/functions-updates/supabase-function-hooks.png)

### Hook Payload

If the target is a Serverless function or an HTTP `POST` request, the payload is automatically generated from the underlying table data. The format matches OpenModels [Realtime](/docs/reference/javascript/subscribe), except in this case you don't a client to "listen" to the changes. This provides yet another mechanism for responding to database changes.
...
As with most of the OpenModels platform, we leverage PostgreSQL's native functionality to implement Database Webhooks (previously called "Function Hooks").
...
-- using pgsql-http to fetch from "open-models-platform.com" 10 times
select
    *
from
...
-- using pg_net to fetch from "open-models-platform.com" 10 times
select
...
Of course, the sync version waits until each request is completed to return the result, taking around 3.5 seconds for 10 requests; while the async version returns almost immediately in 1.5 milliseconds. This is really important for OpenModels hooks, which run requests for every event fired from a SQL trigger - potentially thousands of requests per second.
...
Database Webhooks is enabled today on all [new projects](https://open-models-platform.com/dashboard). Find it under Database > Alpha Preview > Database Webhooks.
