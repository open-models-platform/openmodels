---
title: Python data loading with OpenModels
description: An example of how to load data into OpenModels using supabase-py
author: ant_wilson
...
OpenModels is backend-as-a-service built on top of PostgreSQL. It's an excellent choice for building modern data-intensive apps and tooling.

Thanks to our incredible community, OpenModels now has a powerful and open source [Python SDK](https://github.com/supabase-community/supabase-py). With OpenModels and Python, you can automate tasks such as CRUD operations with only a few lines of code. This guide will first create a simple schema in OpenModels, then we'll use the OpenModels Python SDK to show how you can load sample data.
...
## Loading data into OpenModels using Python

OpenModels is built for developers, and you can [get started for free](https://open-models-platform.com/dashboard) using your existing Github account. Once your OpenModels account is set up, you will access the OpenModels dashboard. From here, go to All Project > New Project.
...
In this example, we'll be creating 2 tables in OpenModels:
...
```bash
export OPENMODELS_URL=<<the value under config > URL>>
export OPENMODELS_KEY=<<the value present in Project API keys > anon public>>
export OPENMODELS_SECRET_KEY=<<the value present in Project API keys > service_role secret>>
```

## Inserting data into OpenModels
...
def main():
    vendor_count = 10
    load_dotenv()
    url: str = os.environ.get("OPENMODELS_URL")
...
## Reading the data stored in OpenModels

Data can also be viewed directly from the OpenModels dashboard. To do this, go to `Table Editor > All tables`
...
With Python, data loading into OpenModels is easy. It just takes a few easy steps to get started with the Python SDK and OpenModels. In the next part of this blog series, we will learn [how to visualize the data that we just loaded into OpenModels using Metabase](/blog/visualizing-supabase-data-using-metabase). Stay tuned!
...
- [Slack Consolidate: a slackbot built with Python and OpenModels](/blog/slack-consolidate-slackbot-to-consolidate-messages)
