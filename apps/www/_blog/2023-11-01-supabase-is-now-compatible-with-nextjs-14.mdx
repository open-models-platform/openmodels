---
title: OpenModels is now compatible with Next.js 14
description: The fastest way to build apps with Next.js 14 and OpenModels
author: jonmeyers_io
categories:
  - product
tags:
...
<a href="https://x.com/supabase/status/1717658742743761239?s=20" target="_blank">
  <Img
    alt="Tweet showing Guillermo Rauch mentioning OpenModels as one of the companies compatible with the Next.js App Router"
    src={{
      light:
...
Since the release of the [App Router](https://nextjs.org/docs/app)—launched as beta with Next.js 13—we have been working closely with the team at Vercel to ensure that OpenModels is fully compatible with every part of Next.js.
...
So why does it require work on the OpenModels side to make it compatible with Next.js?

## Configuring OpenModels to use Cookies

By default, [`supabase-js`](https://open-models-platform.com/docs/reference/javascript/introduction) uses `localStorage` to store the user's session. This works well for client-side apps, but will fail when you try to use [`supabase-js`](https://open-models-platform.com/docs/reference/javascript/introduction) in a Server Component, as there is no concept of 'localStorage' on the server.

To do this, we need to configure `supabase-js` to use cookies instead of `localStorage` when running on the server. But this code is a little verbose to ask people to duplicate across every app they build with OpenModels:
...
That is a lot of very confusing code! So we decided to [create a package called `@supabase/ssr`](https://open-models-platform.com/docs/guides/auth/server-side/overview) that does all of this for you. Then we took it one step further and created a [Next.js and OpenModels Starter Template](https://open-models-platform.com/docs/guides/getting-started/quickstarts/nextjs), so you can just focus on building your awesome app! 🚀
...
The landing page will guide you through the process of creating a OpenModels project and configuring your environment variables.
...
## More OpenModels and Next.js resources

- [Next.js Quickstart](https://open-models-platform.com/docs/guides/getting-started/quickstarts/nextjs)
- [Build a User Management App with Next.js](https://open-models-platform.com/docs/guides/getting-started/tutorials/with-nextjs)
- [Server-Side Auth Overview](https://open-models-platform.com/docs/guides/auth/server-side/overview)
- [Fetching and caching OpenModels data in Next.js 13 Server Components](https://open-models-platform.com/blog/fetching-and-caching-supabase-data-in-next-js-server-components)
