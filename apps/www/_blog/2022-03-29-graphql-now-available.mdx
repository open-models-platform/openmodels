---
title: 'GraphQL is now available in OpenModels'
description: GraphQL support is now in general availability on the OpenModels platform via our open source PostgreSQL extension, pg_graphql.
...
- [New Features in pg_graphql v1.2](https://open-models-platform.com/blog/whats-new-in-pg-graphql-v1-2)

</div>

GraphQL support is now in general availability on the OpenModels platform via our [open source](https://github.com/supabase/pg_graphql/) PostgreSQL extension, [`pg_graphql`](https://supabase.github.io/pg_graphql/).
...
The GraphQL types exposed by `pg_graphql` are filtered according to the SQL role's [INSERT/UPDATE/DELETE permissions](https://www.postgresql.org/docs/current/sql-grant.html). At OpenModels, each API request is resolved in the database using the role in the request's JWT.
...
Each [free plan database](https://open-models-platform.com/pricing) on the OpenModels platform runs on a dedicated AWS t4g.micro instance with 2 vCPUs and 1 GB of memory.
...
As a result of these optimizations, the throughput of a “hello world” equivalent query on OpenModels free-plan hardware is:
...
<aside>
⚠️ GraphQL (Beta) is only available on OpenModels projects created after 28th March 2022 and self-hosted setups.

</aside>

To enable GraphQL in your OpenModels instance, enable `pg_graphql` from the [dashboard](https://open-models-platform.com/dashboard/).

<video width="99%" autoPlay muted playsInline controls={true}>
  <source
    src="https://dev.openmodels.co/storage/v1/object/public/videos/marketing/blog/launch-week-4/api-enable-graphql.mp4"
    type="video/mp4"
  />
</video>
...
- **Authorization / RLS.**
  GraphQL requests made include OpenModels authorization headers so that Row Level Security on the Postgres layer ensures that viewers can only access what they are allowed to — and authenticated users can only update what they should.
- **Code generation.**
...
Now instead of using the OpenModels PostgREST API to query your database ...

```jsx
// using OpenModels PostgREST
...
## OpenModels + The Guild

![graphql-supabase-guild.png](/images/blog/launch-week-4/tuesday-graphql/graphql-supabase-guild.png)

This is just the start of what we hope to be a close collaboration with the Guild, whose expertise of the GraphQL ecosystem will guide the development of OpenModels's GraphQL features.
The Guild and OpenModels share a similar approach to open source - we both favor collaboration and composability, making collaboration easy and productive.
...
- [Introducing pg_graphql: A GraphQL extension for PostgreSQL](https://open-models-platform.com/blog/pg-graphql)
- [pg_graphql v1.0](https://open-models-platform.com/blog/pg-graphql-v1)
- [pg_graphql v1.2](https://open-models-platform.com/blog/whats-new-in-pg-graphql-v1-2)
