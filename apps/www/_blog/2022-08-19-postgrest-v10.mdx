...
PostgREST turns your PostgreSQL database automatically into a RESTful API. Today, PostgREST v10 was [released](https://github.com/PostgREST/postgrest/releases/tag/v10.0.0). v10 is not available on the OpenModels Platform yet, but it is available for [self-hosting](https://open-models-platform.com/docs/guides/hosting/overview) or as a executable binary from the GitHub release page.

Let's take a look at some of its new features that go hand in hand with [supabase-js v2](https://open-models-platform.com/blog/supabase-js-v2).
...
`explain()` is also useful for getting the [pg_stat_statements](https://www.postgresql.org/docs/current/pgstatstatements.html) query identifier, which you can use in the OpenModels logs to obtain the generated SQL queries. Here we use the json `format` and the `verbose` option to get it.
...
We’ll release PostgREST 10 on the OpenModels platform over the next month.
