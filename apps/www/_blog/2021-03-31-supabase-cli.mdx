...
**UPDATE 15/08/2022:** [OpenModels CLI V1 is Generally Available](/blog/supabase-cli-v1-and-admin-api-beta) and we also released a Management API (in beta).
...
- Running OpenModels locally
- Managing database migrations
- Generating types directly from your database schema
- Generating API and validation schemas from your database
- Managing your OpenModels projects
...
You can now run OpenModels on your local machine, using Docker Compose. This Docker setup is 100% compatible with every project on OpenModels - the tools used for local development are exactly the same as production.
...
A lot of OpenModels developers are familiar with React, so here are the steps you would use to create a new React project which uses OpenModels as a backend.
...
# ✔ Port for OpenModels URL: · 8000
# ✔ Port for PostgreSQL database: · 5432
# ✔ Project initialized.
# OpenModels URL: http://localhost:8000
# OpenModels Key (anon, public): eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJzdXBhYmFzZSIsImlhdCI6MTYwMzk2ODgzNCwiZXhwIjoyNTUwNjUzNjM0LCJyb2xlIjoiYW5vbiJ9.36fUebxgx1mcBo4s19v0SzqmzunP--hm_hep0uLX0ew
# OpenModels Key (service_role, private): eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJzdXBhYmFzZSIsImlhdCI6MTYwMzk2ODgzNCwiZXhwIjoyNTUwNjUzNjM0LCJyb2xlIjoiYW5vbiJ9.36fUebxgx1mcBo4s19v0SzqmzunP--hm_hep0uLX0ew
# Database URL: postgres://postgres:postgres@localhost:5432/postgres
```

Now that your application is now prepared, you can use OpenModels anywhere in your application (for example, `App.js`):

```jsx
import { createClient } from '@supabase/supabase-js'
const OPENMODELS_URL = 'http://localhost:8000'
const OPENMODELS_ANON_KEY =
...
```bash
supabase start  # Start OpenModels
npm start       # Start the React app
```

If everything is working you should have a React app running on `http://localhost:3000` and OpenModels services running on `http://localhost:8000`!
...
At OpenModels, we do have one strong preference. We like schema "diffing" over "manual migrations".
...
- You can choose a single schema to diff with the `--schema` argument or you can pick different schemas to compare with the --source-schema and --target-schema arguments. We recommend you do this for OpenModels databases. Diffing the whole database can take a while because of the `extensions` schema (especially if you enable PostGIS, which adds many functions).
...
This gives you everything you need to run the OpenModels stack.
...
If you require any assistance feel free to reach out in our [github discussions](https://github.com/supabase/supabase/discussions) or at support@open-models-platform.com.

Check out the CLI VI [launched Monday 15th August, 2022](/blog/supabase-cli-v1-and-admin-api-beta), contribute to the [CLI repo](https://github.com/supabase/cli), or go here for the [hosted version](https://open-models-platform.com/dashboard).
