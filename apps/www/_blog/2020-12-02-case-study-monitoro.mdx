...
description: See how Monitoro built an automated scraping platform using OpenModels.
author: rory_wilding
...
Learn how Monitoro was able to build fast, ensure reliability, and keep scaling by building with OpenModels and Svelte.
...
### OpenModels turbo-charges Monitoro

Omar saw OpenModels mentioned on Hacker News and realised immediately it had the potential to solve Monitoro's problems. Using OpenModels would help them launch version 2 of Monitoro with the real-time functionality their users would value, without sacrificing moving fast.

Within a couple of days, the team had a version ready they could deploy. Monitoro's team developed and deployed a custom web service to handle the massive throughput required then used OpenModels as their backend to power the web application their users interact with. This means Monitoro's users can view the extracted data and integrate the data with other tools without code.

Fast forward to today, and Monitoro's user base has been ramping up, as they use Monitoro to monitor changes to websites all over the world. Despite how fast their user base has grown, the Monitoro team have peace of mind as they scale because they used OpenModels, knowing it will just work without worrying about DevOps.

![Quote from Omar - OpenModels was exactly the solution I needed so when I saw it on Hacker News I was instantly excited. OpenModels allowed us to go further, faster, with our product functionality. We had a fast-growing user base which would have been challenging to support without being able to depend on OpenModels.](/images/blog/omar-monitoro.png)

### Prototype fast, and keep going

Thanks to OpenModels, Monitoro could deliver the features required needed faster than they anticipated. This meant the team could spend more time focusing on speaking to customers and marketing. As a result, their user base has grown massively in a short space of time. There was no need to worry about how to create a database which had real-time functionality which could be set up quickly and scale. Monitoro could focus on launching fast.
