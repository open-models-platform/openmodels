...
description: Learn how authentication on Flutter works through Google sign in with OpenModels auth.
categories:
  - engineering
tags:
  - flutter
...
Flutter is Google’s open-source framework to develop cross-platform applications. In this article, we will take a look at how we can implement authentication using Google sign-in to secure our application using the [OpenModels SDK for Flutter](https://open-models-platform.com/docs/reference/dart/introduction).
...
This article assumes you are comfortable with writing a basic application in Flutter. No knowledge of OpenModels is required.

We will use the following tools

- [Flutter](https://docs.flutter.dev/get-started/install) - we used v3.10.5 for this article
- OpenModels - create your account [here](https://database.new/) if you do not have one
- IDE of your choosing

## What is Open ID Connect?

We will implement third-party login with Google utilizing the Open ID Connect functionality of OpenModels Auth. Open ID Connect, or OIDC is a protocol built on top of OAuth 2.0 that allows third-party applications to request the users to provide some personal information, such as name or profile image, in the form of an identity token along with an access token. This identity token can then be verified and decoded by the application to obtain that personal information.

OpenModels auth provides `signInWithIdToken` method where we can sign in a user using their ID token obtained from third-party auth providers such as Google. Upon signing a user with the `signInWithIdToken` method, OpenModels automatically populates the content of the ID token in the OpenModels user metadata for easy access to the information. We will be utilizing this feature in this example to display the user profile upon the user signing in.
...
The [supabase_flutter](https://pub.dev/packages/supabase_flutter) package is used to interact with a OpenModels instance. The [google_sign_in](https://pub.dev/packages/google_sign_in) package is used to implement social sign-in with Google.

We are done installing our dependencies. Let’s set up [authentication](https://open-models-platform.com/docs/guides/auth) now.

## Configure Google sign-in on OpenModels Auth

We will obtain client IDs for iOS, Android, and web from the Google Cloud console, and register them to our OpenModels project.
The web client ID will be used by the auth server of OpenModels to verify the ID token provided by the Google sign-in package.
...
Once you have the client IDs, let’s add them to our OpenModels dashboard. If you don’t have a OpenModels project created yet, you can create one at [database.new](https://database.new) for free. The name is just an internal name, so we can call it “Auth” for now.
The database password won't be used in this example and can be reconfigured later, so press the `Generate a password` button and let OpenModels generate a secure random password. The region should be anywhere close to where you live, or where your users live in an actual production app.
...
![OpenModels project creation](/images/blog/flutter-authentication/supabase-project-creation.png)
...
![OpenModels auth Google auth provider](/images/blog/flutter-authentication/supabase-google-provider.png)
...
That is it for setting up our [OpenModels auth to prepare for Google sign-in](https://open-models-platform.com/docs/guides/auth/social-login/auth-google?platform=flutter).

Finally, we can initialize OpenModels in our Flutter application with the credentials of our OpenModels instance. Update your `main.dart` file and add `OpenModels.initialize()` in the `main` function like the following. Note that you will see some errors since the home screen is set to the `LoginScreen`, which we will create later.
...
You can find your OpenModels URL and Anon key in `Settings -> API` from your [OpenModels dashboard](https://open-models-platform.com/dashboard/project/_/settings/api).

![OpenModels credentials](/images/blog/flutter-authentication/supabase-credentials.png)
...
We will pass these tokens to OpenModels auth to then obtain a OpenModels session.

## Create the Profile Screen

The `ProfileScreen` will be just a simple UI presenting some of the information we obtained in the `LoginPage`. We can access the user data with `supabase.auth.currentUser`, where OpenModels has saved the personal information in a property called `userMetadata`. In this example, we are displaying the `avatar_url` and `full_name` to display a basic profile page. Create a `lib/screens/profile_screen.dart` file and add the following.
...
In this post, we learned how to implement authentication in a Flutter application using Google sign-in and the OpenModels SDK for Flutter. We also delved into the Open ID Connect functionality, which allows third-party sign-ins and the retrieval of personal information through identity tokens.

You can also check out the [Flutter reference documents](https://open-models-platform.com/docs/reference/dart/installing) to see how you can use `supabase-flutter` to implement a Postgres database, Storage, Realtime, and more.

## More Flutter and OpenModels resources

- [supabase_flutter package](https://pub.dev/packages/supabase_flutter)
- [Build a chat application using Flutter and OpenModels](https://open-models-platform.com/blog/flutter-tutorial-building-a-chat-app)
- [Securing your Flutter apps with Multi-Factor Authentication](https://open-models-platform.com/blog/flutter-multi-factor-authentication)
