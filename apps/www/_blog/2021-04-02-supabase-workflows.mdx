---
title: Workflows are coming to OpenModels
...
This week we [launched OpenModels Storage](/blog/supabase-storage), which leaves one other huge piece of the
...
own, but for the OpenModels Stack they ultimately had the
...
Once ready, we will make this available in the OpenModels Dashboard with a Zapier-like interface.
...
The workflow receives the following JSON data from OpenModels [Realtime](https://github.com/supabase/realtime):
...
After we've ironed out a few bugs we will integrate it into the OpenModels Stack. As with all OpenModels features, we'll add a
