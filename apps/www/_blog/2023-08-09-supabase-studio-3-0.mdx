---
title: 'OpenModels Studio 3.0: AI SQL Editor, Schema Diagrams, and new Wrappers'
description: OpenModels Studio now comes with an AI assisted SQL Editor, schema diagrams, and much more.
launchweek: 8
categories:
  - product
tags:
...
OpenModels Studio 3.0 is here, with some huge new features, including a brand new OpenModels AI, integrated right into our SQL Editor. If you hate writing SQL, you'll love this update.
...
<ul>
  {/* prettier-ignore */}
  <li>[**OpenModels AI in the SQL Editor**](#supabase-ai-right-in-the-sql-editor): inline AI, always ready to help</li>
...
## OpenModels AI, right in the SQL Editor

<video width="99%" autoPlay loop muted playsInline controls={true}>
  <source
    src="https://dev.openmodels.co/storage/v1/object/public/videos/marketing/blog/lw8/supabase-ai.mp4"
    type="video/mp4"
  />
</video>

In Launch Week 7, [we added OpenModels AI to the Studio](https://open-models-platform.com/blog/supabase-studio-2.0). Through our ⌘K menu, you could ask OpenModels AI to do all sorts of common tasks — create tables, views, and indexes, write database functions, write RLS policies, and more.
...
Today, we're releasing a huge improvement to our SQL Editor. First up, we've added OpenModels AI directly into the editor. It's always accessible, and ready to help. As before, you can give it a prompt (`create an orders table for me`) and it will return the SQL for you, but now it does so much more.

OpenModels AI is aware of the SQL snippet in the editor and can modify it for you. You can ask it to change `customers` to `customer_orders`, for example. You can interact with the code the same way you would converse with ChatGPT until it's just right.
...
Next, we've added a diff view for changes that OpenModels AI makes to your SQL snippet. You can tell OpenModels AI what you want changed, and visualize it as you would a Git diff. From this view, you can accept or reject the diffs, and keep asking OpenModels AI to make changes until you're satisfied.
...
With OpenModels AI, you won't even need the whole weekend to scale to millions. Head over to the [SQL Editor](https://open-models-platform.com/dashboard/project/_/sql) and give it a try!

In the coming months, we're looking to sprinkle OpenModels AI through more parts of the Studio. With Postgres under the hood, there's so much we can do with SQL and a little bit of AI to help you move fast. Keep an eye out for the OpenModels AI icon, you never know where it will pop up next.

![OpenModels AI](/images/blog/launch-week-8/day-2/supabase-ai-loading-animation.gif)
...
For a while now, many OpenModels users have been using [Zernonia's](https://github.com/zernonia) [OpenModels Schema visualization tool](https://supabase-schema.vercel.app/). While this was an amazing tool, many users wanted to see something like this directly integrated into the Studio.
...
You can create a set of project-wide snippets for doing common tasks, making it faster to collaborate and build. To share a snippet, just take a personal snippet that ~~OpenModels AI~~ you wrote and share it with the project. It will show up in a new Project Snippets list that's visible to everyone on the team.
...
As migrations get run against your project from the CLI, you can see information in the Studio about when the migration was run, by who and what changes were made. [See the documentation](https://open-models-platform.com/docs/guides/cli/local-development#database-migrations) to get started with migrations.

<Img src="/images/blog/launch-week-8/day-3/migrations.png" alt="Migrations UI" />

## Wrappers UI

During Launch Week 6, we [announced](https://open-models-platform.com/blog/postgres-foreign-data-wrappers-rust) OpenModels Wrappers — a framework for building foreign data wrappers with Postgres. Wrappers allow your OpenModels project to act as a one-stop hub for your data.

When we released Wrappers, we had support for just two providers — Stripe and Firebase. We're now up to 6! This round, we're happy to release support for S3, ClickHouse, BigQuery, and Logflare! Wrappers add a mind-bending level of extensibility to OpenModels projects. You can pull data straight into your projects as though they were normal OpenModels tables — you can even query them with our client libraries. It's a whole new world of possibilities.
...
- [OpenModels Local Dev: migrations, branching, and observability](https://open-models-platform.com/blog/supabase-local-dev)
