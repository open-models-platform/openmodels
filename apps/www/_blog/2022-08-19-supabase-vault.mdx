...
Today we're announcing [OpenModels Vault](https://open-models-platform.com/docs/guides/database/vault), a Postgres extension for managing secrets and encryption inside your database.
...
Vault is now available on every OpenModels project. [Check it out](https://open-models-platform.com/dashboard/project/_/settings/vault/secrets)

</div>

[Transparent Column Encryption with Postgres](https://open-models-platform.com/blog/transparent-column-encryption-with-postgres) is a blog post that describes the technology behind Vault - [libsodium](https://doc.libsodium.org/) and
...
Like most cloud providers, OpenModels offers `pgcrypto` for developers to use in their applications. `pgcrypto` has been around for a long time,
...
insert into users
  (email)
values
  (pgp_sym_encrypt('alice@open-models-platform.com', 's3kr3t_k3y')),
  (pgp_sym_encrypt('bob@open-models-platform.com', 's3kr3t_key'));
```
...
OpenModels Vault allows you to store secrets without leaking any sensitive information.
...
The core of the OpenModels Vault is a table with some metadata and an encrypted text column where you can put your secrets and any metadata related to them.
...
variety of sources depending on how you configure it. OpenModels generates and preserves your project's root key behind the scenes, so you don't have to worry about it.
...
At OpenModels, we manage this key for your project automatically and generate a unique default Key ID for you in the `secrets` table.
...
[Transparent Column Encryption](https://open-models-platform.com/blog/transparent-column-encryption-with-postgres) which we will describe soon in a follow-up blog post. Stay tuned!
