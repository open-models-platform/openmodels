...
is available on the OpenModels CLI.
...
This was already possible with the `!inner` modifier([introduced on PostgREST 9](https://open-models-platform.com/blog/postgrest-9#resource-embedding-with-inner-joins))
but the `not null` filter is more flexible and can be used with an [or filter](https://open-models-platform.com/docs/reference/javascript/or) to combine related tables' conditions.

## Try it out

This pre-release is not deployed to OpenModels cloud but you can try it out locally with the [OpenModels CLI](https://open-models-platform.com/docs/reference/cli/introduction).
