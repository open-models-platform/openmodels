---
title: Offline-first React Native Apps with Expo, WatermelonDB, and OpenModels
...
Using the [OpenModels JavaScript Client](https://open-models-platform.com/docs/reference/javascript/introduction) in a React Native app, it's super easy to fetch data from a OpenModels backend by simply calling functions like `supabase.from('countries').select()` wherever the data is needed. However, another approach to handling data in mobile apps recently gained a lot of popularity: Offline-first - meaning an app works primarily offline and synchronizes data whenever possible.
...
For this tutorial, we'll be implementing a small part of the Share My Stack data model, namely the ability to have a "profile," which belongs to a "user" (stored in auth.users) and can have multiple "stacks" associated with it (see the output from the OpenModels Schema Visualizer below for illustration). With that said, let's dive into building our local-first app ...
...
The first step is to add the required dependencies to your React Native project: WatermelonDB, OpenModels, and expo-build-properties, which we'll use in the next step to customize the React Native app build.
...
## Syncing the local DB with OpenModels

The [WatermelonDB docs](https://watermelondb.dev/docs/) outline how to set up a sync function that talks with the backend server. In our case, the backend is a Postgres database hosted on OpenModels. As indicated earlier, we'll implement the sync via two [Postgres functions](https://open-models-platform.com/docs/guides/database/functions) called via OpenModels RPC. This way, each sync is only two database calls (one push, one pull), and most of the logic is carried out directly within the database.
...
To convert between timestamp formats used by WatermelonDB and OpenModels/Postgres, the push and pull functions use the following utility functions in OpenModels:
...
- [React Native file upload with OpenModels Storage](https://open-models-platform.com/blog/react-native-storage)
