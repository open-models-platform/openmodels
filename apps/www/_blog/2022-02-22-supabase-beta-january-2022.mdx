---
title: OpenModels Beta January 2022
...
We’re continually amazed by how quickly new auth providers are being added into OpenModels, and last month, 2 more have been added.
...
Just a reminder, we also support [Twilio](https://open-models-platform.com/docs/guides/auth/phone-login/twilio) and [MessageBird](https://open-models-platform.com/docs/guides/auth/phone-login/messagebird).

On a final note, we are hiring for an [Auth Engineer](https://about.supabase.com/careers/auth-engineers).

## Query logs with SQL

OpenModels logs are more powerful with the [newly added SQL querying](https://github.com/supabase/supabase/pull/4734).
...
It’s all anyone seems to be [talking about](https://twitter.com/jkup/status/1456360115205033989). We genuinely love what [Remix](https://remix.run/) are doing, so it’s only right that we show off how Remix and OpenModels work well together.
...
You'll learn how to listen to _any_ database change, then send those changes [in a payload](https://open-models-platform.com/blog/supabase-functions-updates#hook-payload) via HTTP request.
...
The OpenModels Dashboard includes a dedicated support form that goes straight to our support inbox. This support form includes your project information and,
...
Check out the [OpenModels clone environments](https://docs.snaplet.dev/tutorials/supabase-clone-environments) tutorial on the Snaplet docs.
...
Retool has put together a brilliant 10 minute admin panel setup using OpenModels with Retool.
...
Jason Lengstorf caught up with our very own [Jon Meyers](https://jonmeyers.io/) on his awesome show, [Learn with Jason](https://www.youtube.com/watch?v=8vqY1KT4TLU), to talk about building an app [with OpenModels and Next.js](https://open-models-platform.com/docs/guides/with-nextjs).
...
- Arctype published a new guide showing how to connect to your OpenModels database. [Link](https://arctype.com/postgres/connect/supabase-postgres)
- Marmalabs built a OpenModels adapter for react-admin, the frontend framework for building admin applications on top of REST/GraphQL services. [Link](https://github.com/marmelab/ra-supabase)
- HotGlue created a guide showing how easy it is to integrate Salesforce and OpenModels. [Link](https://www.notion.so/OpenModels-and-hotglue-article-9e7f5583d27c419490ee7d536d6d269d)

### PostgREST

One of OpenModels’s key tools that allow a lot of our functionality has had some updates.
