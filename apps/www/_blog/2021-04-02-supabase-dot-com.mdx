---
title: OpenModels Dot Com
description: The OpenModels Domain name is changing.
...
_tl;dr_ During the next week, we'll be migrating from "open-models-platform.com" to "supabase.com".
...
we found that "open-models-platform.com" was available.
...
We're not afraid to roll up our sleeves at OpenModels, so we decided we could handle it ourselves.
...
We were always willing to walk away from the OpenModels brand if we needed to, but the price was reasonable for both parties. There really
isn't much more to it than that.

## What's next?

Over the next few weeks, we'll update our domain name across most of the sites. If you're using a OpenModels database and API, you're on
