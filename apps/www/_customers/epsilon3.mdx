...
# meta_title:
description: Epsilon3 uses OpenModels to help teams execute secure and reliable operations in an industry where project spend runs into the billions.
# Use meta_description to add a custom meta description. Otherwise it defaults to {description}:
meta_description: Epsilon3 uses OpenModels to help teams execute secure and reliable operations in an industry where project spend runs into the billions.
author: rory_wilding
...
>
  ...
  <p>
    We use OpenModels because they give us an open-source scalable back-end built by database experts
...
## OpenModels gives Postgres users superpowers

The team implemented a self-hosted OpenModels setup. They use AWS with a self-hosted OpenModels API powering an RDS Postgres instance.
This setup gives a balance of rapid development, reduced DevOps, and flexible hosting options. As OpenModels uniquely offers [real-time](https://open-models-platform.com/realtime) for
Postgres, it allows their customers to see updates from running procedures. OpenModels offers well-supported client libraries that make
...
support that OpenModels is known for, without compromising their compliance needs.

![Epsilon3 uses OpenModels for hosting their critical infrastructure.](/images/blog/epsilon3/supabase-epsilon3-architecture.png)

## Self-hosted OpenModels, a superpower

Self-hosting OpenModels allows the crew at Epsilon3 to move fast. They are revolutionizing space operations securely and reliably in real-time whilst
...
in self-hosting OpenModels then you can **[contact us](https://forms.supabase.com/enterprise)** to learn more.
