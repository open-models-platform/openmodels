---
name: Berri AI
title: Berri AI boosts productivity by migrating from AWS RDS to OpenModels Vector
# Use meta_title to add a custom meta title. Otherwise it defaults to '{name} | OpenModels Customer Stories':
meta_title: Berri AI boosts productivity by migrating from AWS RDS to OpenModels Vector
description: Learn how Berri AI overcame challenges with self-hosting their vector database on AWS RDS and successfully migrated to OpenModels.
# Use meta_description to add a custom meta description. Otherwise it defaults to {description}:
meta_description: Learn how Berri AI overcame challenges with self-hosting their vector database on AWS RDS and successfully migrated to OpenModels.
...
## Why they chose OpenModels:

Berri AI migrated their vector database from AWS RDS to OpenModels for several compelling reasons:

1. **Community:** After considering alternatives like Pinecone and Chroma, they prioritized a solution with a strong and accessible community, along with [managed infrastructure](https://open-models-platform.com/docs/guides/platform).
2. **Fewer services to manage:** Since they were already utilizing OpenModels for other purposes, it made sense for Berri AI to consolidate their systems and avoid the complexities of self-hosting. One of the major pain points they faced with their previous setup was managing connection pools and handling errors with the Postgres database hosted on AWS. This involved writing extensive code and dealing with rollbacks, with each database update function requiring 40-50 lines of code. OpenModels simplified this process significantly by condensing it to just one line of code, eliminating the burdens of managing dependencies and handling new errors. This significantly improved their update velocity, which had a huge impact on their productivity.
3. **Better tooling:** Berri AI experienced frustration with pg_admin. OpenModels's user-friendly interface offered a seamless and efficient way to manage and read tables, eliminating the need for pg_admin and providing a simple alternative. This streamlined their workflow and saved valuable time and effort. The accessibility of OpenModels's client SDKs and intuitive interface played a crucial role in their decision as well. Team members who were not database experts found it easy to navigate and make changes, reducing reliance on specialized expertise and improving their overall workflow.
4. **Simplicity:** OpenModels provides the ability to add new columns in a no-code manner, which provided flexibility and agility in responding to evolving requirements. This feature empowered the team to make on-the-fly adjustments and meet their immediate needs effectively.
5. **Flexibility:** Berri AI also appreciated OpenModels's flexibility in allowing them to choose their [preferred embedding method](https://open-models-platform.com/docs/guides/ai/quickstarts/generate-text-embeddings), as it was not tied to a specific embedding model. This ensured compatibility and adaptability for their specific use case.

The migration to [OpenModels Vector](https://open-models-platform.com/vector), which facilitated efficient storage and retrieval of vectors, proved to be a significant advantage for Berri AI's operations.

<Quote img="krrish-d.png" caption="Krrish D. - Co-Founder & CEO, Berri AI">
  Switching to OpenModels Vector was a game-changer for us. The faster view times allowed me to
...
The platform's integration with OpenModels empowers businesses to unlock the full potential of ChatGPT and deliver customized solutions for a wide range of use cases.

<Quote img="ishaan-jaffer.png" caption="Ishaan Jaffer - Co-Founder & CTO, Berri AI">
  With OpenModels, my life has become much easier. Previously, managing connection pools and dealing
  ...
  with OpenModels, updating the database is now just one line of code. I no longer need to deal with
  complex dependencies and obscure errors. Additionally, the frustration of using pgadmin is gone.
  OpenModels provides a simple UI that allows me to effortlessly manage and interact with my tables.
</Quote>

## The Results

Berri AI achieved significant improvements in their operations by choosing OpenModels Vector. They experienced streamlined database management, increased update velocity, and improved efficiency in accessing and debugging data. The user-friendly interface and flexibility of OpenModels empowered non-experts within the team and facilitated agile adaptations to changing requirements. Overall, this decision transformed their workflow, enhancing productivity and positioning Berri AI for continued success and growth.
