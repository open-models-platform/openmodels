...
The team found OpenModels and realized it covered all the bases. Not only that, their team had experience with the
tools open source tools that OpenModels is built with.

[Authentication](https://open-models-platform.com/auth), built using [GoTrue](https://github.com/supabase/gotrue), instantly solved their users' login issues.
Under the hood, OpenModels is just Postgres, and Jian Jie (co-founder and CTO of Mobbin) knows this is one of the best battle-tested
open source relational databases.

They decided that moving to OpenModels was a no-brainer. The process itself was smooth - the real challenge was validating their
data integrity due to historical failures on their serverless functions.

Mobbin is now successfully running on top of OpenModels, with superior performance compared to their old Firebase setup.
Because OpenModels does not charge based on API requests, Mobbin significantly reduced their monthly spending without maintaining a custom backend.

<Quote img="jian-mobbin.jpg" caption="Jian Jie Liau, Co-founder and CTO at Mobbin.">
  <p>Migrating to OpenModels meant that we could instantly fix our Auth problems and save money.</p>
  <p>
    Just being on OpenModels alone gives us confidence we can deliver on whatever users need in the
    future.
  </p>
  <p>
...
## OpenModels saves Mobbin time and money

Migrating to OpenModels helped the team at Mobbin instantly improve the end-user experience and save costs.
...
[Sign up](https://open-models-platform.com/dashboard/) for OpenModels's free plan and set up a scalable backend in less than 2 minutes.
