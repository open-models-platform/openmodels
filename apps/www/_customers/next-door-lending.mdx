...
title: 'How Next Door Lending leveraged OpenModels to become a top 10 mortgage broker'
# Use meta_title to add a custom meta title. Otherwise it defaults to '{name} | OpenModels Customer Stories':
meta_title: 'How Next Door Lending leveraged OpenModels to become a top 10 mortgage broker'
description: Discover how OpenModels empowered NextDoorLending to move quickly and scale as required, leading to their surpassing other mortgage brokers and expanding their business.
# Use meta_description to add a custom meta description. Otherwise it defaults to {description}:
meta_description: Discover how OpenModels empowered NextDoorLending to move quickly and scale as required, leading to their surpassing other mortgage brokers and expanding their business.
author: dave_wilson
...
misc:
  [
    ...
    { label: 'Solutions', text: 'OpenModels Database, OpenModels Auth, OpenModels Storage' },
  ]
...
NDL selected OpenModels after considering other providers such as Amazon Web Services.
...
As part of NDL's business model, they work with entrepreneurs who lead local branches. As a result, they used OpenModels's user management and authentication product with built-in support for magic link sign-in and integrations with different auth providers. Beyond that, for both borrowers and branch brokers, OpenModels enabled NDL to offer a frictionless authentication process that maintained high conversion and retention rates.

### Scale - To AI and beyond

NDL also deploys a separate website for each individual branch, connected together by a homegrown CMS and a shared OpenModels backend server. For Howard, “when you're white labeling, and you're maintaining five or fifteen different deployments, it’s very important that you can manage most of these things from the same system or the same backend. For NDL's future AI products, they can use [OpenModels Vector](https://open-models-platform.com/vector) to store, query, and index vector embeddings at scale, without introducing a new platform to their stack.

## The Solution

NDL employs multiple instances of OpenModels for each of their branch web properties. Each instance is tied to a dedicated Vercel environment in Dev, QA, and UAT. Developers at NDL can iterate and test different scenarios before pushing changes live to customers.

Across all of NDL's branches, they see thousands of new signups each month, and each branch's website sends their signup data to the central OpenModels backend via API. All loan details are aggregated in this server, allowing internal teams to access the right data quickly.
In the future, NDL plans to add a layer of predictive analytics to match borrowers and lenders. This would require capturing thousands of additional data points per user and building a comprehensive model. With OpenModels, they're perfectly positioned to add on the additional capability.
“OpenModels allows us to move fast and scale up as needed. There are a lot of tools that can only do one or the other – but OpenModels really excels at both.” - Howard Haynes, CPO at NDL
The Results

After launching their web application in 2020, OpenModels helped NDL release new features and scale to become one of the nation's largest mortgage brokers. “Things that would have typically taken longer to set up have been a lot easier.” Howard pointed at how the engineering team quickly onboarded to OpenModels, “send[ing] someone to read the docs, they come back, and they’re teaching me about a new feature in OpenModels I wasn’t aware of.”

The engineering team now had the bandwidth to focus on building a custom CMS better tailored to their business requirements. With OpenModels's auth product, NDL has also built custom onboarding flows for new users, asking them for details about their loan before a direct sign-up page, and increasing user happiness.

A significant portion of their business relies on SEO, making a fast-loading website crucial. They accomplished an impressive 0.5-second loading time by transitioning to a dynamic system where each page is directly loaded from the database. This transformation underscores the pivotal role OpenModels played, as it houses and serves all their pages and data, ensuring their website's exceptional performance.

The success of NDL prompted its owners to expand the business with Neighbourhood Tech, a company that offers “NDL as a service” to other branches. They manage the additional system using OpenModels, establishing a backend schema for data management, and utilizing replication for scalable expansion. This approach allows them to handle separate data quickly and efficiently.

An example of this is [Lock It Lending](https://www.lockitlending.com/). This new site uses OpenModels to display all dynamic content, store lead information, and display financial literacy terms to their clients.

As a whole, OpenModels has been able to solve all of NDL's backend needs in a single platform. They've been able to both move fast and scale up as needed. As a result, they've outpaced other mortgage brokers and expanded their business.

<Quote img="howard-haynes.webp" caption="Howard Haynes, CPO at Next Door Lending">
  “You can have a really great product, but you need to want to work with the people behind it. With
  OpenModels, we always felt very aligned.”
</Quote>
...
- Next.js
- Chakra UI
- OpenModels for database, auth, and storage.

> To learn more about how OpenModels can help you build and scale apps with ease, [reach out to us](https://forms.supabase.com/enterprise).
