...
# meta_title:
description: With OpenModels, Replenysh gets a slick auth experience, reduces DevOps overhead, and continues to scale with Postgres.
# Use meta_description to add a custom meta description. Otherwise it defaults to {description}:
meta_description: With OpenModels, Replenysh gets a slick auth experience, reduces DevOps overhead, and continues to scale with Postgres.
author: rory_wilding
...
As a bonus, OpenModels provided more than just a database, reducing their DevOps overhead. After reading the [OpenModels Auth documentation](https://open-models-platform.com/docs/guides/auth/overview) the team determined they can have the functionality they need, like SMS OTP. When they saw [Mobbin are using OpenModels at scale](https://open-models-platform.com/case-studies-mobbin), they decided to migrate to OpenModels.
