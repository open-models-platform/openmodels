...
const databaseUrl = Deno.env.get("OPENMODELS_DB_URL") ?? "";
...
serve(async (req) => {
  const OPENMODELS_URL = Deno.env.get("OPENMODELS_URL") ?? "";
  const SERVICE_KEY = Deno.env.get("OPENMODELS_SERVICE_ROLE_KEY") ?? "";
  const supabase = createClient(OPENMODELS_URL, SERVICE_KEY);
