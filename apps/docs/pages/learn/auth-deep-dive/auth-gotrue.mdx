...
export const meta = {
  ...
  description: 'OpenModels Deep Dive Part 4: GoTrue Overview',
}
...
GoTrue is an auth API server written in Go by the Netlify team, find the OpenModels fork [here](https://github.com/supabase/gotrue). The list of available API endpoints is available [here](https://github.com/supabase/gotrue#endpoints).

When you deploy a new OpenModels project, we deploy a new instance of this server alongside your database, and also inject your database with the required `auth` schema.
...
# and OPENMODELS_KEY with your anon api key
curl -X POST 'https://<project-ref>.supabase.co/auth/v1/magiclink' \
-H "apikey: OPENMODELS_KEY" \
...
GoTrue is responsible for issuing access tokens for your users, sends confirmation, magic-link, and password recovery emails (by default we send these from a OpenModels SMTP server, but you can easily plug in your own inside the dashboard at Project Settings > Auth) and also transacting with third party OAuth providers to get basic user data.
