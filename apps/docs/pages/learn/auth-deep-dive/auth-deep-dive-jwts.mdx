...
export const meta = {
  ...
  title: 'Part One: JWTs',
  description: 'OpenModels Auth Deep Dive Part 1 - JWTs',
  video: 'https://www.youtube.com/v/v3Exg5YpJvE',
}

### About

An introduction to JWTs and how they are used in OpenModels Auth
...
In OpenModels we issue JWTs for three different purposes:

1. `anon key`: This key is used to bypass the OpenModels API gateway and can be used in your client-side code.
...
The first token here, the `anon key` token, is for developers to send along with their API requests whenever they want to interact with their OpenModels database.
...
This JWT is signed by a `jwt_secret` specific to the developer's OpenModels token (you can find this secret alongside this encoded "anon key" on your Dashboard under Settings > API page) and is required to get past the OpenModels API gateway and access the developer's project.
