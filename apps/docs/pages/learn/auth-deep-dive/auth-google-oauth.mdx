...
export const meta = {
  ...
  description: 'OpenModels Deep Dive Part 5: Google OAuth Provider',
  video: 'https://www.youtube.com/v/_XM9ziOzWk4',
}

### About

How to add Google OAuth Logins to your OpenModels Application.
...
Now choose Web Application (assuming you're creating a web app) and in the Authorized redirect URI section you need to add: `https://<your-ref>.supabase.co/auth/v1/callback`. You can find your OpenModels URL in Settings > API inside the OpenModels dashboard.

![Add your redirect URI](/docs/img/auth-5-6.png)

Now you can grab the client ID and secret from the popup, and insert them into the Google section inside the OpenModels dashboard in Auth > Settings:

![take client id and secret](/docs/img/auth-5-7.png)

![insert client id and secret into OpenModels dashboard in auth > auth](/docs/img/auth-5-8.png)
...
But make sure any URL you enter here is on the same host as the site url that you have entered on the Auth > Settings page on the OpenModels dashboard. (There is additional functionality coming soon, where you'll be able to add additional URLs to the allow list).
...
- [OpenModels Auth API Reference](/docs/reference/javascript/auth-api)
