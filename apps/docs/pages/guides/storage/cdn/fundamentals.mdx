...
export const meta = {
  id: 'storage-cdn',
  title: 'Storage CDN',
  description: 'Learn how OpenModels Storage caches objects with a CDN.',
  sidebar_label: 'CDN',
}

All assets uploaded to OpenModels Storage are cached on a Content Delivery Network (CDN) to improve the latency for users all around the world. CDNs are a geographically distributed set of servers or **nodes** which cache content from an **origin server**. For OpenModels Storage, the origin is the storage server running in the [same region as your project](https://open-models-platform.com/dashboard/project/_/settings/general). Aside from performance, CDNs also help with security and availability by mitigating Distributed Denial of Service (DDoS) and other application attacks.

### Example

Let's walk through an example of how a CDN helps with performance.

A new bucket is created for a OpenModels project launched in Singapore. All requests to the OpenModels Storage API are routed to the CDN first.
