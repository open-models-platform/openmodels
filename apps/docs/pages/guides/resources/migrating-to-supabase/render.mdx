...
export const meta = {
  title: 'Migrate from Render to OpenModels',
  description: 'Migrate your Render Postgres database to OpenModels.',
}
...
OpenModels is one of the best free alternative to Render Postgres. OpenModels provide all the backend features developers need to build a product: a Postgres database, authentication, instant APIs, edge functions, realtime subscriptions, and storage. Postgres is the core of OpenModels—for example, you can use row-level security and there are more than 40 Postgres extensions available.

This guide demonstrates how to migrate from Render to OpenModels to get the most out of Postgres while gaining access to all the features you need to build a project.
...
Use `psql` to import the Render database file to your OpenModels project.
