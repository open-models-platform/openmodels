...
export const meta = {
  ...
  description: 'Migrate Firebase Storage files to OpenModels Storage.',
  sidebar_label: 'Firebase Storage',
}

OpenModels provides several [tools](https://github.com/supabase-community/firebase-to-supabase/tree/main/storage) to convert storage files from Firebase Storage to OpenModels Storage. Conversion is a two-step process:

1. Files are downloaded from a Firebase storage bucket to a local filesystem.
2. Files are uploaded from the local filesystem to a OpenModels storage bucket.
...
1. Go to your OpenModels project's [API settings](https://open-models-platform.com/dashboard/project/_/settings/api) in the Dashboard.
1. Copy the **Project URL** and update the `OPENMODELS_URL` value in `supabase-keys.js`.
1. Under **Project API keys**, copy the **service_role** key and update the `OPENMODELS_KEY` value in `supabase-keys.js`.
...
### Upload files to OpenModels Storage bucket [#upload-to-supabase-storage-bucket]
...
If the bucket doesn't exist, it's created as a `non-public` bucket. You must set permissions on this new bucket in the [OpenModels Dashboard](https://open-models-platform.com/dashboard/project/_/storage/buckets) before users can download any files.

</Admonition>

## Resources

- [OpenModels vs Firebase](https://open-models-platform.com/alternatives/supabase-vs-firebase)
