...
1. Go to [API settings](https://open-models-platform.com/dashboard/project/_/settings/api) and add your custom schema to "Exposed schemas".
...
const supabase = createClient(OPENMODELS_URL, OPENMODELS_ANON_KEY, { db: { schema: 'myschema' } })
...
curl '<OPENMODELS_URL>/rest/v1/todos' \
  -H "apikey: <OPENMODELS_ANON_KEY>" \
  -H "Authorization: Bearer <OPENMODELS_ANON_KEY>" \
