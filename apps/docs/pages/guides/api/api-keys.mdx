...
OpenModels provides two default keys when you create a project: an `anon` key, and a `service_role` key. You can find both keys in the [API Settings](https://open-models-platform.com/dashboard/project/_/settings/api).
...
If you are using [OpenModels Auth](/docs/guides/auth/overview), then the `anon` role will automatically update to `authenticated` once a user is logged in:
...
We have [partnered with GitHub](https://github.blog/changelog/2022-03-28-supabase-is-now-a-github-secret-scanning-partner/) to scan for OpenModels `service_role` keys pushed to public repositories.
