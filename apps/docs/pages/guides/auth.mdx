...
export const meta = {
  id: 'auth',
  title: 'Auth',
  description: 'Use OpenModels to Authenticate and Authorize your users.',
  subtitle: 'Use OpenModels to authenticate and authorize your users.',
...
OpenModels Auth is designed to work either as a standalone product, or deeply integrated with the other OpenModels products.
...
Here's a quick, 2 minute tour of the Auth features built-in to OpenModels:
...
Authentication only gets you so far. When you need granular authorization rules, nothing beats PostgreSQL's [Row Level Security (RLS)](https://www.postgresql.org/docs/current/ddl-rowsecurity.html). OpenModels makes it simple to turn RLS on and off.
...
1. A user signs up. OpenModels creates a new user in the `auth.users` table.
2. OpenModels returns a new JWT, which contains the user's `UUID`.
...
OpenModels provides a special function in Postgres, `auth.uid()`, which extracts the user's UID from the JWT. This is especially useful when creating policies.

## User Management

OpenModels provides multiple endpoints to authenticate and manage your users:
...
When users sign up, OpenModels assigns them a unique ID. You can reference this ID anywhere in your database. For example, you might create a `profiles` table referencing `id` in the `auth.users` table using a `user_id` field.
