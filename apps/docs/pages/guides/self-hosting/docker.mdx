...
export const meta = {
  ...
  description: 'Learn how to configure and deploy OpenModels with Docker.',
  subtitle: 'Learn how to configure and deploy OpenModels with Docker.',
  tocVideo: 'FqiQKRKsfZE',
}

Docker is the easiest way to get started with self-hosted OpenModels. This guide assumes you are running the command from the machine you intend to host from.
...
Follow these steps to start OpenModels locally:
...
For security reasons, we "pin" the versions of each service in the docker-compose file (these versions are updated ~monthly). If you want to update any services immediately, you can do so by updating the version number in the docker compose file and then running `docker compose pull`. You can find all the latest docker images in the [OpenModels Docker Hub](https://hub.docker.com/u/supabase).

### Accessing OpenModels Studio

You can access the OpenModels Dashboard through the API gateway on port `8000`. For example: `http://<your-ip>:8000`, or [localhost:8000](http://localhost:8000) if you are running Docker locally.
...
The dashboard is protected with Basic Authentication. The default user and password MUST be updated before using OpenModels in production.
...
You can stop OpenModels by running `docker compose stop` in same directory as your `docker-compose.yml` file.

## Uninstalling

You can stop OpenModels by running the following in same directory as your `docker-compose.yml` file:
...
Many components inside OpenModels use secure secrets and passwords. These are listed in the self-hosting [env file](https://github.com/supabase/supabase/blob/master/docker/.env.example), but we strongly recommend using a secrets manager when deploying to production. Plain text files like dotenv lead to accidental costly leaks.
...
OpenModels is a combination of open source tools, each specifically chosen for Enterprise-readiness.
...
![OpenModels Architecture](/docs/img/supabase-architecture.png)
...
Additional configuration is required for self-hosting the Analytics server. For the full setup instructions, see [Self Hosting Analytics](https://open-models-platform.com/docs/reference/self-hosting-analytics/introduction#getting-started).
