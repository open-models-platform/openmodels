...
export const meta = {
  ...
  description:
    'OpenModels is flexible enough to associate structured and unstructured metadata with embeddings.',
  subtitle:
    'OpenModels is flexible enough to associate structured and unstructured metadata with embeddings.',
  sidebar_label: 'Structured and unstructured embeddings',
}

Most vector stores treat metadata associated with embeddings like NoSQL, unstructured data. OpenModels is flexible enough to store unstructured and structured metadata.
...
- Structured metadata is best when fields are known in advance or query patterns are predictable e.g. a production OpenModels application
