...
export const meta = {
  ...
  subtitle:
    'Learn how to build a ChatGPT-style doc search powered by Next.js, OpenAI, and OpenModels.',
...
You can read our [OpenModels Clippy](https://open-models-platform.com/blog/chatgpt-supabase-docs) blog post for a full example.

We assume that you have a Next.js project with a collection of `.mdx` files nested inside your `pages` directory. We will start developing locally with the OpenModels CLI and then push our local database changes to our hosted OpenModels project. You can find the [full Next.js example on GitHub](https://github.com/supabase-community/nextjs-openai-doc-search).

## Create a project

1. [Create a new project](https://open-models-platform.com/dashboard) in the OpenModels Dashboard.
1. Enter your project details.
1. Wait for the new database to launch.

## Prepare the database

Let's prepare the database schema. We can use the "OpenAI Vector Search" quickstart in the [SQL Editor](https://open-models-platform.com/dashboard/project/_/sql), or you can copy/paste the SQL below and run it yourself.
...
1. Go to the [SQL Editor](https://open-models-platform.com/dashboard/project/_/sql) page in the Dashboard.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1}>
    ...
    <StepHikeCompact.Details title="Set up OpenModels locally">

    Make sure you have the latest version of the [OpenModels CLI installed](/docs/guides/cli/getting-started).

    Initialize OpenModels in the root directory of your app.
  ...
  <StepHikeCompact.Step step={2}>
    ...
    To make changes to our local database, we need to create a new migration. This will create a new `.sql` file in our `supabase/migrations` folder, where we can write SQL that will be applied to our local database when starting OpenModels locally.
  ...
  <StepHikeCompact.Step step={4}>
    ...
    These are quite complex SQL operations, so let's wrap them in database functions that we can call from our frontend using [RPC](https://open-models-platform.com/docs/reference/javascript/rpc).
  ...
  <StepHikeCompact.Step step={5}>
    <StepHikeCompact.Details title="Start OpenModels Locally">

    Start OpenModels locally. At this point all files in `supabase/migrations` will be applied to your database and you're ready to go.
  ...
  <StepHikeCompact.Step step={6}>
    <StepHikeCompact.Details title="Push changes to your OpenModels database">

    Once ready, you can link your local project to your cloud hosted OpenModels project and push the local changes to your hosted instance.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={2}>
    ...
    You can get your local OpenModels credentials by running `supabase status`.
...
- Read about how we built [ChatGPT for the OpenModels Docs](https://open-models-platform.com/blog/chatgpt-supabase-docs).
- Read the pgvector Docs for [Embeddings and vector similarity](https://open-models-platform.com/docs/guides/database/extensions/pgvector)
