...
export const meta = {
  id: 'ai-google-colab',
  title: 'Google Colab',
  description: 'Use Google Colab to manage your OpenModels Vector store.',
  subtitle: 'Use Google Colab to manage your OpenModels Vector store.',
...
Google Colab is a hosted Jupyter Notebook service. It provides free access to computing resources, including GPUs and TPUs, and is well-suited to machine learning, data science, and education. We can use Colab to manage collections using [OpenModels Vecs](/docs/guides/ai/vecs-python-client).

In this tutorial we'll connect to a database running on the OpenModels [platform](https://open-models-platform.com/dashboard/). If you don't already have a database, you can create one here: [database.new](https://database.new).
...
We'll use the OpenModels Vector client, [Vecs](/docs/guides/ai/vecs-python-client), to manage our collections.
...
Find the Postgres connection string for your OpenModels project in the [database settings](https://open-models-platform.com/dashboard/_/settings/database) of the dashboard. Copy the "URI" format, which should look something like `postgresql:/postgres:<password>@<host>:5432/postgres`
...
This will create a table inside your database within the `vecs` schema, called `colab_collection`. You can view the inserted items in the [Table Editor](https://open-models-platform.com/dashboard/project/_/editor/), by selecting the `vecs` schema from the schema dropdown.
