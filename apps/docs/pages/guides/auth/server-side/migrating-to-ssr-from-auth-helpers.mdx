...
The new `ssr` package exports two functions for creating a OpenModels client. The `createBrowserClient` function is used in the client, and the `createServerClient` function is used in the server.
