...
export const meta = {
  ...
  description: 'Use Sign in with Apple with OpenModels',
  tocVideo: '-tpcZzTdvN0',
}

OpenModels Auth supports using [Sign in with Apple](https://developer.apple.com/sign-in-with-apple/) on the web and in native apps for iOS, macOS, watchOS or tvOS.

## Overview

To support Sign in with Apple, you need to configure the [Apple provider in the OpenModels dashboard](https://open-models-platform.com/dashboard/project/_/auth/providers) for your project.
...
- Sign in on the web or in web-based apps
  - Using an OAuth flow initiated by OpenModels Auth using the [Sign in with Apple REST API](https://developer.apple.com/documentation/sign_in_with_apple/sign_in_with_apple_rest_api).
...
>
  <TabPanel id="web" label="Web">
    ...
    Behind the scenes, OpenModels Auth uses the [REST APIs](https://developer.apple.com/documentation/sign_in_with_apple/sign_in_with_apple_rest_api) provided by Apple.
    ...
    This call takes the user to Apple's consent screen. Once the flow ends, the user's profile information is exchanged and validated with OpenModels Auth before it redirects back to your web application with an access and refresh token representing the user's session.
    ...
    5. Configure Website URLs for the newly created **Services ID**. The web domain you should use is the domain your OpenModels project is hosted on. This is usually `<project-id>.supabase.co` while the redirect URL is `https://<project-id>.supabase.co/auth/v1/callback`.
    6. Create a signing **Key** in the [Keys](https://developer.apple.com/account/resources/authkeys/list) section of the Apple Developer Console. You can use this key to generate a secret key using the tool below, which is added to your OpenModels project's Auth configuration. Make sure you safely store the `AuthKey_XXXXXXXXXX.p8` file. If you ever lose access to it, or make it public accidentally please revoke it from the Apple Developer Console and create a new one immediately. You will have to generate a new secret key using this file every 6 months, so make sure you schedule a recurring meeting in your calendar!
    7. Finally, add the information you configured above to the [Apple provider configuration in the OpenModels dashboard](https://open-models-platform.com/dashboard/project/_/auth/providers).
    ...
    You can use the `signInWithIdToken()` method from the OpenModels JavaScript library on the website to obtain an access and refresh token once the user has given consent using Sign in with Apple JS:
    ...
    4. Register the Services ID you created to your project's [Apple provider configuration in the OpenModels dashboard](https://open-models-platform.com/dashboard/project/_/auth/providers) under _Authorized Client IDs_.
  ...
  <TabPanel id="react-native" label="Expo React Native">
    ...
    When testing with Expo Go, the Expo App ID `host.exp.Exponent` will be used. Make sure to add this to the "Authorized Client IDs" list in your [OpenModels dashboard Apple provider configuration](https://open-models-platform.com/dashboard/project/_/auth/providers)!

    </Admonition>

    1. Have an **App ID** which uniquely identifies the app you are building. You can create a new App ID from the [Identifiers](https://developer.apple.com/account/resources/identifiers/list/bundleId) section in the Apple Developer Console (use the filter menu in the upper right side to see all App IDs). These usually are a reverse domain name string, for example `com.example.app`. Make sure you configure Sign in with Apple for the App ID you created or already have, in the Capabilities list. At this time OpenModels Auth does not support Server-to-Server notification endpoints, so you should leave that setting blank. (In the past App IDs were referred to as _bundle IDs._)
    2. Register all of the App IDs that will be using your OpenModels project in the [Apple provider configuration in the OpenModels dashboard](https://open-models-platform.com/dashboard/project/_/auth/providers) under _Authorized Client IDs_.
  ...
  <TabPanel id="flutter" label="Flutter">
    ...
    Once the setup is complete on the Flutter app, add the bundle ID of your app to your OpenModels dashboard in `Authentication -> Providers -> Apple` in order to register your app with OpenModels.
  ...
  <TabPanel id="kotlin" label="Kotlin">
	...
	When using [Compose Multiplatform](https://github.com/JetBrains/compose-multiplatform/), you can use the [compose-auth](https://open-models-platform.com/docs/reference/kotlin/installing) plugin. On iOS it uses Native Apple Login automatically and on other platforms it uses `gotrue.loginWith(Apple)`.
