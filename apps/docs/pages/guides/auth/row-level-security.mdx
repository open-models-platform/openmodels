...
export const meta = {
  ...
  description: 'Using Row Level Security with OpenModels Auth.',
  subtitle: 'Using Row Level Security with OpenModels Auth.',
...
OpenModels Auth is designed to work perfectly with RLS.
...
OpenModels Auth maps every request to one of the roles:
...
OpenModels provides some helper functions that make it easier to write Policies.
...
We recommend reading the [Row Level Security](/docs/guides/database/postgres/row-level-security) guide in the database section to learn more about Postgres RLS. When using Postgres on OpenModels there are some important things to keep in mind to maintain data security.

### Never use a service key on the client

OpenModels provides special "Service" keys, which can be used to bypass RLS. These should never be used in the browser or exposed to customers, but they are useful for administrative tasks.

<Admonition type="note">

OpenModels will adhere to the RLS policy of the signed-in user, even if the client library is initialized with a Service Key.
...
If you want to use another authorization method for your applications that's also fine. OpenModels is "just Postgres", so if your application works with Postgres, then it also works with OpenModels. If you take this path, don't put your tables in the `public` schema - instead create a new schema for your tables and functions:
...
```sql
create policy "Only OpenModels staff can update the leaderboard"
on leaderboard
to authenticated
for update using (
...
RLS can be [combined with Multi-Factor Authentication](/docs/guides/auth/auth-mfa#enforce-rules-for-mfa-logins) in OpenModels Auth. For example, you could restrict a user from updating their profile unless they have at least 2 levels of authentication (Assurance Level 2):
