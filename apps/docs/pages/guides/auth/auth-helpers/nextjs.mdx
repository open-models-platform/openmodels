...
export const meta = {
  id: 'nextjs',
  title: 'OpenModels Auth with the Next.js App Router',
  description:
    'Authentication and Authorization helpers for creating an authenticated OpenModels client with the Next.js 13 App Router.',
...
The [Next.js Auth Helpers package](https://github.com/supabase/auth-helpers) configures OpenModels Auth to store the user's `session` in a `cookie`, rather than `localStorage`. This makes it available across the client and server of the App Router - [Client Components](/docs/guides/auth/auth-helpers/nextjs#client-components), [Server Components](/docs/guides/auth/auth-helpers/nextjs#server-components), [Server Actions](/docs/guides/auth/auth-helpers/nextjs#server-actions), [Route Handlers](/docs/guides/auth/auth-helpers/nextjs#route-handlers) and [Middleware](/docs/guides/auth/auth-helpers/nextjs#middleware). The `session` is automatically sent along with any requests to OpenModels.
...
Retrieve your project's URL and anon key from your [API settings](https://open-models-platform.com/dashboard/project/_/settings/api), and create a `.env.local` file with the following environment variables:

```bash .env.local
NEXT_PUBLIC_OPENMODELS_URL=your-supabase-url
NEXT_PUBLIC_OPENMODELS_ANON_KEY=your-supabase-anon-key
```

## Managing session with Middleware

When using the OpenModels client on the server, you must perform extra steps to ensure the user's auth session remains active. Since the user's session is tracked in a cookie, we need to read this cookie and update it if necessary.
...
TypeScript types can be [generated with the OpenModels CLI](/docs/reference/javascript/typescript-support) and passed to `createMiddlewareClient` to add type support to the OpenModels client.
...
TypeScript types can be [generated with the OpenModels CLI](/docs/reference/javascript/typescript-support) and passed to `createRouteHandlerClient` to add type support to the OpenModels client.
...
TypeScript types can be [generated with the OpenModels CLI](/docs/reference/javascript/typescript-support) and passed to `createClientComponentClient` to add type support to the OpenModels client.
...
## Creating a OpenModels Client

There are 5 ways to access the OpenModels client with the Next.js Auth Helpers:
...
This allows for the OpenModels client to be easily instantiated in the correct context. All you need to change is the context in the middle `create[ClientComponent|ServerComponent|ServerAction|RouteHandler|Middleware]Client` and the Auth Helpers will take care of the rest.
...
[Client Components](https://nextjs.org/docs/getting-started/react-essentials#client-components) allow the use of client-side hooks - such as `useEffect` and `useState`. They can be used to request data from OpenModels client-side, and [subscribe to realtime events](https://github.com/supabase/supabase/tree/master/examples/auth/nextjs/app/realtime-posts.tsx).
...
The `createClientComponentClient` function implements a [Singleton pattern](https://en.wikipedia.org/wiki/Singleton_pattern) by default, meaning that all invocations will return the same OpenModels client instance. If you need multiple OpenModels instances across Client Components, you can pass an additional configuration option `{ isSingleton: false }` to get a new client every time this function is called.
...
In order to use OpenModels in Server Components, you need to have implemented the [Middleware](/docs/guides/auth/auth-helpers/nextjs#managing-session-with-middleware) steps above.
...
TypeScript types can be [generated with the OpenModels CLI](/docs/reference/javascript/typescript-support) and passed to `createServerComponentClient` to add type support to the OpenModels client.
...
TypeScript types can be [generated with the OpenModels CLI](/docs/reference/javascript/typescript-support) and passed to `createServerActionClient` to add type support to the OpenModels client.
...
A route can be configured to use the Edge Runtime by exporting a `runtime` variable set to `edge`. Additionally, the `cookies()` function must be called from the Edge route, before creating a OpenModels Client.
...
Server Components and Route Handlers are static by default - data is fetched once at build time and the value is cached. Since the request to OpenModels now happens at build time, there is no user, session or cookie to pass along with the request to OpenModels. Therefore, the `createClient` function from `supabase-js` can be used to fetch data for static routes.
...
TypeScript types can be [generated with the OpenModels CLI](/docs/reference/javascript/typescript-support) and passed to `createClient` to add type support to the OpenModels client.
...
You no longer need to implement logic to ensure there is only a single instance of the OpenModels Client shared across all Client Components - this is now the default and handled by the `createClientComponentClient` function. Call it as many times as you want!
...
For an example of creating multiple OpenModels clients, check [Singleton section](/docs/guides/auth/auth-helpers/nextjs#singleton) above.
