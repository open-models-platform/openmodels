...
export const meta = {
  id: 'nextjs-pages',
  title: 'OpenModels Auth with Next.js Pages Directory',
  description:
...
The `Code Exchange` API route is required for the [server-side auth flow](https://open-models-platform.com/docs/guides/auth/server-side-rendering) implemented by the Next.js Auth Helpers. It exchanges an auth `code` for the user's `session`, which is set as a cookie for future requests made to OpenModels.
...
You can pass types that were [generated with the OpenModels CLI](/docs/reference/javascript/typescript-support#generating-types) to the OpenModels Client to get enhanced type safety and auto completion:
...
- Usage with TypeScript: You can pass types that were [generated with the OpenModels CLI](/docs/reference/javascript/typescript-support#generating-types) to the OpenModels Client to get enhanced type safety and auto completion:
