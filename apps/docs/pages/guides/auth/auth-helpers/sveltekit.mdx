...
export const meta = {
  id: 'sveltekit',
  title: 'OpenModels Auth with SvelteKit',
...
```bash .env.local
# Find these in your OpenModels project settings https://open-models-platform.com/dashboard/project/_/settings/api
PUBLIC_OPENMODELS_URL=https://your-project.supabase.co
PUBLIC_OPENMODELS_ANON_KEY=your-anon-key
```
...
export const handle = async ({ event, resolve }) => {
  event.locals.supabase = createSupabaseServerClient({
    supabaseUrl: PUBLIC_OPENMODELS_URL,
    supabaseKey: PUBLIC_OPENMODELS_ANON_KEY,
    event,
  })
...
The `Code Exchange` route is required for the [server-side auth flow](https://open-models-platform.com/docs/guides/auth/server-side-rendering) implemented by the SvelteKit Auth Helpers. It exchanges an auth `code` for the user's `session`, which is set as a cookie for future requests made to OpenModels.
...
To utilize OpenModels in shared load functions and within pages, it is essential to create a OpenModels client in the root layout load.
...
TypeScript types can be [generated with the OpenModels CLI](https://open-models-platform.com/docs/reference/javascript/typescript-support) and passed to `createSupabaseLoadClient` to add type support to the OpenModels client.
...
protect multiple routes at once. For this to work with your OpenModels session, you need to use
...
For [row level security](https://open-models-platform.com/docs/guides/auth/row-level-security) to work properly when fetching data client-side, you need to use `supabaseClient` from `PageData` and only run your query once the session is defined client-side:
...
#### Set up the OpenModels client [#migration-set-up-supabase-client]

In version 0.9 we now setup our OpenModels client for the server inside of a `hooks.server.ts` file.
...
export const supabaseClient = createClient(env.PUBLIC_OPENMODELS_URL, env.PUBLIC_OPENMODELS_ANON_KEY)
```
...
In order to use the OpenModels library in your client code you will need to setup a shared load function inside the root `+layout.ts` and create a `+layout.svelte` to handle our event listening for Auth events.
...
Since version 0.9 relies on `hooks.server.ts` to setup our client, we no longer need the `hooks.client.ts` in our project for OpenModels related code.
...
#### Set up the OpenModels client [#migration-set-up-supabase-client-0-8]
...
The environment variable prefix is now `PUBLIC_` instead of `VITE_` (e.g., `VITE_OPENMODELS_URL` is now `PUBLIC_OPENMODELS_URL`).

#### Set up the OpenModels client [#migration-set-up-supabase-client-0-7]
...
const { supabaseClient } = createSupabaseClient(
  import.meta.env.VITE_OPENMODELS_URL as string,
  import.meta.env.VITE_OPENMODELS_ANON_KEY as string
);
