...
void main() async {
  await OpenModels.initialize(
    url: dotenv.get('OPENMODELS_URL'),
    anonKey: dotenv.get('OPENMODELS_ANON_KEY'),
  );
...
You can pass metadataFields to add additional fields to the form to pass as metadata to OpenModels.
