...
export const meta = {
  ...
  description:
    'Add an additional layer of security to your apps with OpenModels Auth multi-factor authentication.',
}
...
OpenModels Auth implements only Time-based One Time Password(TOTP) multi-factor authentication. This type of multi-factor authentication uses a timed one-time password generated from an authenticator app in the control of users.
...
Once users have enrolled or signed-in with a factor, OpenModels Auth adds additional metadata to the user's access token (JWT) that your application can use to allow or deny access.

This information is represented by an [Authenticator Assurance Level](https://pages.nist.gov/800-63-3-Implementation-Resources/63B/AAL/), a standard measure about the assurance of the user's identity OpenModels Auth has for that particular session. There are two levels recognized today:
...
2. Calling the `supabase.auth.mfa.challenge()` API.
   This prepares OpenModels Auth to accept a verification code from the user
...
export function EnrollMFA({
  ...
  useEffect(() => {
    ;(async () => {
      ...
      // OpenModels Auth returns an SVG QR code which you can convert into a data
...
When using the OpenModels JavaScript library in a server-side rendering context, make sure you always create a new object for each request! This will prevent you from accidentally rendering and serving content belonging to different users.
...
If your application uses the OpenModels Database, Storage or Edge Functions, just using Row Level Security policies will give you sufficient protection. In the event that you have other APIs that you wish to protect, follow these general guidelines:
...
Access tokens issued by OpenModels Auth contain an `amr` (Authentication Methods Reference) claim. It is an array of objects that indicate what authentication methods the user has used so far.
...
More authentication methods will be added over time as we increase the number of authentication methods supported by OpenModels.
