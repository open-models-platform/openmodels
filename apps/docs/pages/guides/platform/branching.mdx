...
export const meta = {
  title: 'Branching',
  description: 'Use OpenModels Branches to test and preview changes.',
  subtitle: 'Use OpenModels Branches to test and preview changes',
}
...
Currently, it is advised to use a OpenModels Project that is not using these regions.

</Admonition>

Use branching to safely and easily experiment with changes to your OpenModels project.

OpenModels branches work like Git branches. They let you create and test changes like new configurations, database schemas, or features in a separate, temporary instance without affecting your production setup.

When you're ready to ship your changes, merge your branch to update your production instance with the new changes.

If you understand Git, you already understand OpenModels Branching.
...
OpenModels projects can consist of multiple Preview Branches. All Preview Branches are backed by an isolated OpenModels instance. Each instance contains all OpenModels features, and has its own API credentials.

<figure className="max-w-[700px] mx-auto">
  <Image
    className="hidden dark:block"
    alt="Each branch has a separate OpenModels instance."
  ...
  />
  <figcaption>Each branch has a separate OpenModels instance.</figcaption>
</figure>

OpenModels branching works like Git branching, with one main Production branch. Preview Branches branch off your Production branch and contain proposed changes. These changes can then be merged into Production.
...
By creating new SQL migrations in a Preview Branch to propose schema changes, those changes can then be merged into your Production branch. These changes then run the new migrations on your OpenModels Production branch. This is exactly how Git branches work: experiment on a feature branch and merge back into Production when you're finished.
...
To facilitate OpenModels Branching, your OpenModels project will be connected to a Git repository via an integration.

OpenModels Branching works with [GitHub](#branching-with-github). Future iterations may support other Git providers. If you are interested in other Git providers or a non-Git-based workflow, join the discussions for [GitLab Branching, BitBucket Branching, and non-Git based Branching](https://github.com/orgs/supabase/discussions/18936).

### Branching with GitHub

OpenModels Branching uses the OpenModels GitHub integration to read files from your GitHub repository. With this integration, OpenModels watches all commits, branches, and pull requests of your GitHub repository.

In Git, you have a Production Branch (typically this is `main`, `master`, `prod`, etc). This should also be your OpenModels project's Production Branch.
...
<figure className="max-w-[700px] mx-auto">
    <Image
        className="hidden dark:block"
        alt="Each GitHub branch can have its own OpenModels preview branch."
    ...
    />
    <figcaption>Each GitHub branch can have its own OpenModels preview branch.</figcaption>
...
If you don’t turn on branching, your OpenModels project continues to work as a single branch, on a single instance. You have a single set of API keys for each project, and no preview instances are created. It's the Git equivalent of working directly on the `main` branch.
...
1. ##### Host a project per environment, and test against a staging project
   ...
   Create multiple projects on OpenModels with the same schema. Use one project as a staging environment to test any changes. Then migrate tested changes to the production project.
...
## How to use OpenModels Branching

<Admonition type="note" label="Github integration required">

OpenModels branching requires the GitHub integration. Your application must be developed in a GitHub repository.

</Admonition>

### Preparing your GitHub repository

You need a GitHub repository to use OpenModels branching. The `./supabase` directory in your repository contains your SQL migration files and your database seed file.
...
Follow the instructions below **before turning on OpenModels branching**.
...
You need to take a snapshot of your database. Log into OpenModels from the [command line client](/docs/guides/cli) and pull the relevant files. Then commit them to GitHub:

1. Inside your project directory, initialize your configuration for OpenModels local development:

   ```bash
   supabase init
   ```

2. Pull down database-related files. You can find your database URL in your [database settings](https://open-models-platform.com/dashboard/project/_/settings/database), under the URI tab of the Connection String settings panel. It will look something like `postgresql://username:password@db.ref.supabase.co:5432/postgres`
...
### Enable OpenModels branching

Once your repository is [correctly prepared](#set-up-an-existing-project-for-branching), you can enable branching from the OpenModels dashboard.
...
<StepHikeCompact>
    ...
    <StepHikeCompact.Step step={2}>
        <StepHikeCompact.Details title="If you see the following dialog, you’re in the alpha testing group" fullWidth>
            ...
            Follow the instructions to link your OpenModels project to its GitHub repository.
...
<StepHikeCompact>
    <StepHikeCompact.Step step={1}>
        ...
        You need at least one other branch aside from your OpenModels production branch.
    ...
    <StepHikeCompact.Step step={2}>
        <StepHikeCompact.Details title="Navigate to the Branches page in your OpenModels dashboard." fullWidth>

        In the OpenModels dashboard, look for the branch dropdown on the right-hand side of the top bar. It should be set to your production branch by default. Open the dropdown and click `Manage branches`.
    ...
    <StepHikeCompact.Step step={3}>
        <StepHikeCompact.Details title="Create a OpenModels preview branch" fullWidth>
...
### Develop your app with OpenModels branches

OpenModels branching uses the state of your GitHub repository to apply migrations and seed preview instances. It checks the `supabase` directory at the top level of your repository.
...
Local development is recommended for developers familiar with the OpenModels CLI, who want to test changes on their local machine. Remote development is recommended if you don’t want to run OpenModels locally, or if you want to collaborate with team members via the dashboard interface.

#### Develop locally

You can develop locally by running OpenModels on your local machine. You’ll have a database running locally, and another database running remotely on the preview branch.

You can experiment on your local database by [writing SQL migrations yourself](https://open-models-platform.com/docs/guides/cli/local-development#database-migrations). You can also [auto-generate migration files by using OpenModels’s local studio](https://open-models-platform.com/docs/guides/cli/local-development#diffing-changes) and the `supabase db-diff` command.
...
2. ##### Add SQL statements to your new migration file.
   ...
   You can manually edit the new migration file, or use [local studio](https://open-models-platform.com/docs/guides/cli/local-development#diffing-changes) and diff the changes.
...
OpenModels’s GitHub integration detects the new migration and runs it against the branch instance.
...
As an alternative to developing locally, you can make changes on your remote OpenModels dashboard. You can then pull these changes to your local machine and commit them to GitHub.
...
2. Make changes to your database schema from the OpenModels dashboard.
3. Reset database password for the branch. To do this, navigate to `https://open-models-platform.com/dashboard/project/[branch-ref]/settings/database` and click **Reset database password**. Save the new password for use in the next step.
4. In your terminal, navigate to the directory for your OpenModels project. Use the OpenModels client to pull changes from your branch to your local migrations directory. Make sure to use the database URL for your branch.
...
When you open a pull request on GitHub, the OpenModels integration automatically checks for a matching preview branch. If one doesn’t exist, it gets created.
...
<figure className="max-w-[700px] mx-auto">
  ...
  {/* prettier-ignore */}
  <figcaption>OpenModels GitHub integration will comment on your PR with the status of your Preview Branch, including whether migrations have successfully run.</figcaption>
</figure>
...
We highly recommend turning on a ‘required check’ for the OpenModels integration. You can do this from your GitHub repository settings.
...
To fix this, push your latest changes, then delete the preview branch in OpenModels and reopen it.
...
A migration might fail for various reasons, including invalid SQL statements, and schema conflicts. If a migration fails, the OpenModels integration check is shown as failing.

To check the error message, see the OpenModels integration comment on your PR.
...
It’s not possible to change the Git branch used as the Production branch for OpenModels Branching. The only way to change it is to disable and re-enable branching. See [Disable Branching](#disable-branching).

## Branching and hosting providers

Branching works with hosting providers that support preview deployments.

With the OpenModels branching integration, you can sync the Git branch used by the hosting provider with the corresponding OpenModels preview branch. This means that the preview deployment built by your hosting provider is matched to the correct database schema, edge functions, and other OpenModels configurations.
...
<Admonition type="note">
    ...
    The Vercel Integration for OpenModels branching is under development. To express your interest, [join the discussion on GitHub discussions](https://github.com/orgs/supabase/discussions/18938).
...
- By clicking the blue `Deploy` button in a OpenModels example app’s `README` file
...
OpenModels automatically updates your Vercel project with the correct environment variables for the corresponding preview branches.
...
OpenModels branching is a new and exciting new part of the OpenModels development ecosystem. We’re monitoring its success and open to any feedback.
