...
export const meta = {
  id: 'logs',
  title: 'Logging',
  description: 'Getting started with OpenModels Log Browser',
}

The OpenModels Platform includes a Logs Explorer that allows log tracing and debugging. Log retention is based on your [project's pricing plan](https://open-models-platform.com/pricing).

## Product Logs

OpenModels provides a logging interface specific to each product. You can use simple regular expressions for keywords and patterns to search log event messages. You can also export and download the log events matching your query as a spreadsheet.
...
[API logs](https://open-models-platform.com/dashboard/project/_/database/api-logs) show all network requests and response for the REST and GraphQL [APIs](../../guides/database/api).
...
[Postgres logs](https://open-models-platform.com/dashboard/project/_/logs/postgres-logs) show queries and activity for your [database](../../guides/database).
...
[Auth logs](https://open-models-platform.com/dashboard/project/_/logs/auth-logs) show all server logs for your [Auth usage](../../guides/auth).
...
[Storage logs](https://open-models-platform.com/dashboard/project/_/logs/storage-logs) shows all server logs for your [Storage API](../../guides/storage).
...
[Realtime logs](https://open-models-platform.com/dashboard/project/_/logs/realtime-logs) show all server logs for your [Realtime API usage](../../guides/realtime).
...
For each [Edge Function](https://open-models-platform.com/dashboard/project/_/functions), logs are available under the following tabs:
...
By default, only DDL statements are logged for new OpenModels projects.
...
1. [Enable the pgAudit extension](https://open-models-platform.com/dashboard/project/_/database/extensions).
2. [Restart your project](https://open-models-platform.com/dashboard/project/_/settings/general) using the **Fast database reboot** option.
3. Configure `pgaudit.log` (see below). Perform a fast reboot if needed.
4. View your query logs under [Logs > Postgres Logs](https://open-models-platform.com/dashboard/project/_/logs/postgres-logs).
...
Realtime doesn't log new WebSocket connections or Channel joins by default. Enable connection logging per client by including an `info` `log_level` parameter when instantiating the OpenModels client.
...
The [Logs Explorer](https://open-models-platform.com/dashboard/project/_/logs-explorer) exposes logs from each part of the OpenModels stack as a separate table that can be queried and joined using SQL.
