...
Your OpenModels project supports connecting to the Postgres DB without SSL enabled to maximize client compatibility. For increased security, you can prevent clients from connecting if they're not using SSL.

SSL enforcement only applies to connections to both Postgres and PgBouncer ("Connection Pooler"); all HTTP APIs offered by OpenModels (e.g., PostgREST, Storage, Auth) automatically enforce SSL on all incoming connections.

## Manage SSL enforcement via the Dashboard

SSL enforcement can be configured via the "Enforce SSL on incoming connections" setting under the SSL Configuration section in [Database Settings page](https://open-models-platform.com/dashboard/project/_/settings/database) of the dashboard.

## Manage SSL enforcement via the CLI

To get started:

1. [Install](/docs/guides/cli) the OpenModels CLI 1.37.0+.
