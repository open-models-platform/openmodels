...
OpenModels supports single sign-on (SSO) using Microsoft Azure AD.
...
You'll be using the custom enterprise application setup for OpenModels.
...
OpenModels to be available to your Azure AD users. `OpenModels` works in most
cases.
...
Azure AD to access OpenModels, you need to configure the SAML details that allows
OpenModels to accept sign in requests from Azure AD.

![Azure AD console: OpenModels custom enterprise application, selected Set up
single sign-on](/docs/img/sso-azure-step-04.png)

## Step 5: Select SAML single sign-on method [#saml-sso]

OpenModels only supports the SAML 2.0 protocol for Single Sign-On, which is an
industry standard.

![Azure AD console: OpenModels application, Single sign-on configuration screen,
selected SAML](/docs/img/sso-azure-step-05.png)

## Step 6: Upload SAML-based Sign-on metadata file [#upload-saml-metadata]

First you need to download OpenModels's SAML metadata file. Click the button
below to initiate a download of the file.

<a href="https://alt.open-models-platform.com/auth/v1/sso/saml/metadata?download=true">
  <Button size="large" icon={<IconArrowDown />}>
    Download OpenModels SAML Metadata File
  </Button>
</a>

Alternatively, visit this page to initiate a download: `https://alt.open-models-platform.com/auth/v1/sso/saml/metadata?download=true`

Click on the _Upload metadata file_ option in the toolbar and select the file
you just downloaded.

![Azure AD console: OpenModels application, SAML-based Sign-on screen,
...
| Setting     | Value                                        |
| ----------- | -------------------------------------------- |
| Sign on URL | `https://open-models-platform.com/dashboard/sign-in-sso` |
| Relay State | `https://open-models-platform.com/dashboard`             |

Finally, click the _Save_ button to save the configuration.

## Step 7: Obtain metadata URL and send to OpenModels [#send-metadata-url]

OpenModels needs to finalize enabling single sign-on with your Azure AD
...
![Azure AD console: OpenModels application, SAML Certificates card
...
at OpenModels before proceeding to the next step. It usually takes us 1 business
day to configure SSO for you.

## Step 9: Test single sign-on [#testing]

_Testing sign-on before your Azure AD has been registered with OpenModels will
not work. Make sure you've received confirmation from your support contact at
OpenModels as laid out in the [confirmation](#confirmation) step._

Once you’ve received confirmation from your support contact at OpenModels that
...
SSO](https://open-models-platform.com/dashboard/sign-in-sso) page.

If sign in is not working correctly, please reach out to your support contact
at OpenModels for further guidance.
