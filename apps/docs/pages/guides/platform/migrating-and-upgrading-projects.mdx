...
export const meta = {
  ...
  description: 'Upgrade your project to the latest version of OpenModels.',
  sidebar_label: 'Migrating and upgrading',
}

OpenModels ships fast and we endeavor to add all new features to existing projects wherever possible.
In some cases, access to new features require upgrading or migrating your OpenModels project.
...
1. Use the "Upgrade project" button on the [Infrastructure](https://open-models-platform.com/dashboard/project/_/settings/infrastructure) section of your dashboard.
...
1. On the [General Settings](https://open-models-platform.com/dashboard/project/_/settings/general) page in the Dashboard, click **Pause project**. You will be redirected to the home screen as your project is pausing. This process can take several minutes.
...
Note that a pause + restore upgrade involves tearing down your project's resources before bringing them back up again. If the restore process should fail, manual intervention from OpenModels support will be required to bring your project back online.
...
When upgrading, the OpenModels platform will "right-size" your disk based on the current size of the database. For example, if your database is 100GB in size, and you have a 200GB disk, the upgrade will reduce the disk size to 120GB (1.2x the size of your database).
...
We automatically migrate OpenModels-managed roles' passwords to scram-sha-256 during the upgrade process, but you will need to manually migrate the passwords of any custom roles you have created, else you won't be able to connect using them after the upgrade.
...
OpenModels performs extensive pre- and post-upgrade validations to ensure that the database has been correctly upgraded. However, you should plan for your own application-level validations, as there might be changes you might not have anticipated, and this should be budgeted for when planning your downtime window.

## Migrate your project

Migrating projects can be achieved using the OpenModels CLI. This is particularly useful for older projects (e.g. to use a newer Postgres version).

### Before you begin

- Install [Postgres](https://www.postgresql.org/download/) so you can run `psql` and `pg_dump`.
- Install [OpenModels CLI](/docs/guides/cli#installation).
- Create a new [OpenModels project](https://open-models-platform.com/dashboard).
...
- Set environment variables for the old project's database URL as `$OLD_DB_URL` and the new project's as `$NEW_DB_URL`.
  To find the database URL for a project, go to the project's dashboard page [Project Settings/Database](https://open-models-platform.com/dashboard/project/_/settings/database) and look at `Connection string / URI`. For example, to set the `$OLD_DB_URL` you would run `export OLD_DB_URL=postgresql://postgres:[YOUR-PASSWORD]@db.[YOUR-PROJECT-REF#].supabase.co:5432/postgres`.
...
1. Enable [Database Webhooks](https://open-models-platform.com/dashboard/project/_/database/hooks) if you enabled them in your old project.
2. Enable any [extensions](https://open-models-platform.com/dashboard/project/_/database/extensions) that were enabled in your old project.

If you use [column encryption](/docs/guides/database/column-encryption), first copy the root encryption key to your new project using your [Personal Access Token](https://open-models-platform.com/dashboard/account/tokens).

```bash
export OLD_PROJECT_REF="<old_project_ref>"
export NEW_PROJECT_REF="<new_project_ref>"
export OPENMODELS_ACCESS_TOKEN="<personal_access_token>"

curl "https://api.supabase.com/v1/projects/$OLD_PROJECT_REF/pgsodium" \
  -H "Authorization: Bearer $OPENMODELS_ACCESS_TOKEN" |
...
- If you have created any [custom roles](https://open-models-platform.com/dashboard/project/_/database/roles) with `login` attribute, you have to manually set their passwords in the new project.
...
If you have modified the `auth` and `storage` schemas in your old project, such as adding triggers or RLS policies, you have to restore them separately. The OpenModels CLI can help you diff the changes to these schemas using the following commands.
...
Replication for Realtime is disabled for all tables in your new project. On the [Replication](https://open-models-platform.com/dashboard/project/_/database/replication) page in the Dashboard, select your new project and enable replication for tables that were enabled in your old project.
