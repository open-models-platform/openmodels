...
export const meta = {
  ...
  title: 'Build a OpenModels Integration (Beta)',
  subtitle:
    "This guide steps through building a OpenModels Integration using OAuth2 and the management API, allowing you to manage users' organizations and projects on their behalf.",
  description: 'Build a OpenModels Integration using OAuth2 and the Management API.',
}

Using OAuth2.0 you can retrieve an access and refresh token that grant your application full access to the [Management API](https://open-models-platform.com/docs/reference/api/introduction) on behalf of the user.
...
## Show a "Connect OpenModels" button

In your user interface, add a "Connect OpenModels" button to kick off the OAuth flow. Follow the design guidelines outlined in our [brand assets](/brand-assets).

## Implementing the OAuth 2.0 flow

Once you've published your OAuth App on OpenModels, you can use the OAuth 2.0 protocol get authorization from OpenModels users to manage their organizations and projects.

You can use your preferred OAuth2 client or follow the steps below. You can see an example implementation in TypeScript using OpenModels Edge Functions [on our GitHub](https://github.com/supabase/supabase/tree/master/examples/edge-functions/supabase/functions/connect-supabase).
...
- `redirect_uri`: The URL where OpenModels will redirect the user to after providing consent.
...
Once the user consents to providing API access to your OAuth App, OpenModels will redirect the user to the `redirect_uri` provided in the previous step. The URL will contain these query parameters:

- `code`: An authorization code you should exchange with OpenModels to get the access and refresh token.
...
- `client_secret`: The secret that authenticates your OAuth App to OpenModels.
...
If the user has revoked access to your application, you will not be able to refresh a token. Furthermore, access tokens will stop working. Make sure you handle HTTP Unauthorized errors when calling any OpenModels API.
...
You can configure the user's [custom SMTP settings](https://open-models-platform.com/docs/guides/auth/auth-smtp) using the [`/config/auth` endpoint](https://api.supabase.com/api/v1#/projects%20config/updateV1AuthConfig).
