...
OpenModels is great for building something very fast _and_ for scaling up. However, it's important to note that as your application matures and your team expands, the practices you use for managing an application in production should not be the same as the practices you used for prototyping.
...
Once your application is live, you should never change your database using the Dashboard. OpenModels provides various [access levels](https://open-models-platform.com/docs/guides/platform/access-control) for the Dashboard that can help enforce this. Some other important things to consider at this point include:

- Read the [Production Checklist](/docs/guides/platform/going-into-prod).
- Make your team aware of the [Shared Responsibilities](/docs/guides/platform/shared-responsibility-model) between your organization and OpenModels.
- Design a [safe workflow](https://open-models-platform.com/docs/guides/platform/shared-responsibility-model#you-decide-your-own-workflow) for managing your database. We strongly recommend running [multiple environments](/docs/guides/cli/managing-environments) as part of your development workflow (`local` -> `staging` -> `prod`).
