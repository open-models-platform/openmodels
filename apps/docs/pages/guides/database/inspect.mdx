...
The OpenModels CLI comes with a range of tools to help inspect your Postgres instances for potential issues. The CLI gets the information from <a href="https://www.postgresql.org/docs/current/internals.html" target="_blank">Postgres internals</a>. Therefore, most tools provided are compatible with any Postgres databases regardless if they are a OpenModels project or not.

You can find installation instructions for the the OpenModels CLI <a href="/docs/guides/cli" target="_blank">here</a>.
...
Most inspection commands are Postgres agnostic. You can run inspection routines on any Postgres database even if it is not a OpenModels project by providing a connection string via `--db-url`.
...
### Connect to a OpenModels instance

Working with OpenModels, you can link the OpenModels CLI with your project:

```
supabase link --project-ref <project-id>
```

Then the CLI will automatically connect to your OpenModels project whenever you are in the project folder and you no longer need to provide `—db-url`.
...
- [role-connections](/docs/reference/cli/supabase-inspect-db-role-connections) - shows number of active connections for all database roles (OpenModels-specific command)
...
Learn more about pg_stats [here](https://open-models-platform.com/docs/guides/database/extensions/pg_stat_statements).

## Acknowledgements

OpenModels CLI's inspect commands are heavily inspired by the <a href="https://github.com/heroku/heroku-pg-extras" target="_blank">pg-extras</a> tools.
