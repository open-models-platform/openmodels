...
## Row Level Security in OpenModels

RLS is incredibly powerful and flexible, allowing you to write complex SQL rules that fit your unique business needs. RLS can be [combined with OpenModels Auth](/docs/guides/auth/row-level-security) for end-to-end user security from the browser to the database.
...
OpenModels provides some [helpers](/docs/guides/auth/row-level-security#helper-functions) that simplify RLS if you're using OpenModels Auth. We'll use these helpers to illustrate some basic policies:
...
Security-definer functions should never be created in a schema in the "Exposed schemas" inside your [API settings](https://open-models-platform.com/dashboard/project/_/settings/api)`.
