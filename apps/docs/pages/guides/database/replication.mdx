...
Replication is a technique for copying the data from one database to another. OpenModels uses replication functionality to provide a real-time API. Replication is useful for:
...
Replication is done through _publications_, a method of choosing which changes to send to other systems (usually another Postgres database). Publications can be managed in the [Dashboard](https://open-models-platform.com/dashboard) or with SQL.
