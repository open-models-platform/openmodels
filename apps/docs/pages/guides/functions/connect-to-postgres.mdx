...
The `supabase-js` client is a great option for connecting to your OpenModels database since it handles authorization with Row Level Security, and it automatically formats your response as JSON.
...
// Get the connection string from the environment variable "OPENMODELS_DB_URL"
...
Deployed edge functions are pre-configured to use SSL for connections to the OpenModels database. You don't need to add any extra configurations.

If you want to use SSL connections during local development, follow these steps:

- Download the SSL certificate from [Database settings](https://open-models-platform.com/dashboard/project/_/settings/database)

- In your [local .env file](https://open-models-platform.com/docs/guides/functions/secrets), add these two variables:
