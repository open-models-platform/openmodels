...
- `OPENMODELS_URL`: The API gateway for your OpenModels project.
- `OPENMODELS_ANON_KEY`: The `anon` key for your OpenModels API. This is safe to use in a browser when you have [Row Level Security](/docs/guides/auth/row-level-security) enabled.
- `OPENMODELS_SERVICE_ROLE_KEY`: The `service_role` key for your OpenModels API. This is safe to use in Edge Functions, but it should NEVER be used in a browser. This key will bypass [Row Level Security](/docs/guides/auth/row-level-security).
- `OPENMODELS_DB_URL`: The URL for your [PostgreSQL database](/docs/guides/database). You can use this to connect directly to your database.
