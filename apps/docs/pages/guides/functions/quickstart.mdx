...
Let's create a basic Edge Function on your local machine and then invoke it using the OpenModels CLI.

## Initialize a project

Create a new OpenModels project in a folder on your local machine:
...
Check out the [CLI Docs](/docs/guides/cli) to learn how to install the OpenModels CLI on your local machine.
...
curl --request POST 'http://localhost:54321/functions/v1/hello-world' \
  --header 'Authorization: Bearer OPENMODELS_ANON_KEY' \
...
const supabase = createClient(process.env.OPENMODELS_URL, process.env.OPENMODELS_ANON_KEY)
...
<Admonition type="tip" label="Where is my OPENMODELS_ANON_KEY?">
...
└── supabase
    ...
    │   ├── _shared
    │   │   ├── supabaseAdmin.ts # OpenModels client with SERVICE_ROLE key.
    │   │   └── supabaseClient.ts # OpenModels client with ANON key.
