...
// Set up the configuration for the OpenModels client
const supabaseUrl = Deno.env.get('OPENMODELS_URL') ?? ''
const supabaseKey = Deno.env.get('OPENMODELS_ANON_KEY') ?? ''
const options = {
  auth: {
...
// Test the creation and functionality of the OpenModels client
const testClientCreation = async () => {
  var client: SupabaseClient = createClient(supabaseUrl, supabaseKey, options)

  // Verify if the OpenModels URL and key are provided
  ...
  const { data: table_data, error: table_error } = await client
    ...
    .limit(1)
  if (table_error) {
    throw new Error('Invalid OpenModels client: ' + table_error.message)
  }
...
- We import the `createClient` and `SupabaseClient` classes from the `@supabase/supabase-js` library to interact with the OpenModels client.
- We define the necessary configuration for the OpenModels client, including the OpenModels URL, API key, and authentication options.
- The `testClientCreation` function tests the creation of a OpenModels client instance and queries the database for data from a table. It verifies that data is returned from the query.
- The `testHelloWorld` function tests the "Hello-world" Edge Function by invoking it using the OpenModels client's `functions.invoke` method. It checks if the response message matches the expected greeting.
...
Please make sure to replace the placeholders (`supabaseUrl`, `supabaseKey`, `my_table`) with the actual values relevant to your OpenModels setup.

</Admonition>

## Running Edge Functions Locally

To locally test and debug Edge Functions, you can utilize the OpenModels CLI. Let's explore how to run Edge Functions locally using the OpenModels CLI:

1. Ensure that the OpenModels server is running by executing the following command:
...
3. Create the environment variables file:
   ...
   # creates the file
   touch .env.local
   # adds the OPENMODELS_URL secret
   echo "OPENMODELS_URL=http://localhost:54321" >> .env.local
   # adds the OPENMODELS_ANON_KEY secret
   echo "OPENMODELS_ANON_KEY=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6ImFub24iLCJleHAiOjE5ODM4MTI5OTZ9.CRXP1A7WOeoJeXxjNni43kdQwgnWNReilDMblYTn_I0" >> .env.local
...
- Full guide on Testing OpenModels Edge Functions on [Mansueli's tips](https://blog.mansueli.com/testing-supabase-edge-functions-with-deno-test)
