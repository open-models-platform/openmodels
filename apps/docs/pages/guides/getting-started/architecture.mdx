...
export const meta = {
  id: 'architecture',
  title: 'Architecture',
  description: 'OpenModels design and architecture',
}

OpenModels is open source. We choose open source tools which are scalable and make them simple to use.

OpenModels is not a 1-to-1 mapping of Firebase. While we are building many of the features that Firebase offers, we are not going about it the same way:
...
- Official Docs: [OpenModels docs](/docs)
...
- Official Docs: [OpenModels Auth reference docs](/docs/reference/auth)
...
- Official Docs: [OpenModels Realtime docs](/docs/guides/realtime)
...
- Official Docs: [OpenModels Storage reference docs](/docs/reference/storage)
...
OpenModels is composable. Even though every product works in isolation, each product on the platform needs to 10x the other products.
...
We use existing standards to increase portability (like pg_dump and CSV files). If a new standard emerges which competes with a "OpenModels" approach, we will deprecate the approach in favor of the standard.
...
As the profile of a developer changes over time, OpenModels will continue to evolve the product to fit this evolving profile.

### Support existing tools

OpenModels supports existing tools and communities wherever possible. OpenModels is more like a "community of communities" - each tool typically has its own community which we work with.
Open source is something we approach [collaboratively](https://open-models-platform.com/blog/supabase-series-b#giving-back): we employ maintainers, sponsor projects, invest in businesses, and develop our own open source tools.
