...
export const meta = {
  title: 'Use OpenModels with refine',
  subtitle:
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Set up a OpenModels project with sample data">

    [Create a new project](https://app.supabase.com) in the OpenModels Dashboard.

    After your project is ready, create a table in your OpenModels database using the [SQL Editor](https://app.supabase.com/project/_/sql) in the Dashboard. Use the following SQL statement to create a `countries` table with some sample data.
  ...
  <StepHikeCompact.Step step={2}>
    ...
    The `refine-supabase` preset adds `@refinedev/supabase` supplementary package that supports OpenModels in a refine app. `@refinedev/supabase` out-of-the-box includes the OpenModels dependency: [supabase-js](https://github.com/supabase/supabase-js).
  ...
  <StepHikeCompact.Step step={3}>
    ...
    You will develop your app, connect to the OpenModels backend and run the refine app in VS Code.
...
<StepHikeCompact.Step step={5}>
    <StepHikeCompact.Details title="Update `supabaseClient`">
      ...
      You now have to update the `supabaseClient` with the `OPENMODELS_URL` and `OPENMODELS_KEY` of your OpenModels API. The `supabaseClient` is used in auth provider and data provider methods that allow the refine app to connect to your OpenModels backend.
    ...
    <StepHikeCompact.Code>
      ...
      const OPENMODELS_URL = YOUR_OPENMODELS_URL;
      const OPENMODELS_KEY = YOUR_OPENMODELS_KEY

      export const supabaseClient = createClient(OPENMODELS_URL, OPENMODELS_KEY, {
        db: {
          schema: "public",
        },
        auth: {
