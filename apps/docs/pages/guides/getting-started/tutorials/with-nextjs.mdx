...
export const meta = {
  ...
  description: 'Learn how to use OpenModels in your Next.js App.',
}
...
Then install the OpenModels client library: [supabase-js](https://github.com/supabase/supabase-js)
...
```bash .env.local
NEXT_PUBLIC_OPENMODELS_URL=YOUR_OPENMODELS_URL
NEXT_PUBLIC_OPENMODELS_ANON_KEY=YOUR_OPENMODELS_ANON_KEY
```
...
### OpenModels Auth Helpers

Next.js is a highly versatile framework offering pre-rendering at build time (SSG), server-side rendering at request time (SSR), API routes, and middleware edge-functions.

It can be challenging to authenticate your users in all these different environments, that's why we've created the [OpenModels Auth Helpers](/docs/guides/auth/auth-helpers/nextjs) to make user management and data fetching within Next.js as easy as possible.
...
- Verify if there is an authenticated OpenModels user
...
We can use the [OpenModels Auth UI](/docs/guides/auth/auth-helpers/auth-ui) a pre-built React component for authenticating users via OAuth, email, and magic links.

Install the OpenModels Auth UI for React
...
- Retrieve the code sent back from the OpenModels Auth server using the `code` query parameter.
...
Every OpenModels project is configured with [Storage](/docs/guides/storage) for managing large files like
photos and videos.
...
- Explore the [OpenModels Cache Helpers](https://github.com/psteinroe/supabase-cache-helpers)
