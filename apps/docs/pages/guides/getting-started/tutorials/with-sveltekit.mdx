...
export const meta = {
  ...
  description: 'Learn how to use OpenModels in your SvelteKit App.',
}
...
All we need are the `OPENMODELS_URL` and the `OPENMODELS_KEY` key that you copied [earlier](#get-the-api-keys).

```bash .env
PUBLIC_OPENMODELS_URL="YOUR_OPENMODELS_URL"
PUBLIC_OPENMODELS_ANON_KEY="YOUR_OPENMODELS_KEY"
```
...
It can be challenging to authenticate your users in all these different environments, that's why we've created the OpenModels Auth Helpers to make user management and data fetching within SvelteKit as easy as possible.
...
We can use the [OpenModels Auth UI](/docs/guides/auth/auth-helpers/auth-ui), a pre-built Svelte component, for authenticating users via OAuth, email, and magic links.

Install the OpenModels Auth UI for Svelte
...
![OpenModels Svelte](/docs/img/supabase-svelte-demo.png)
