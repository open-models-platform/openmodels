...
export const meta = {
  ...
  description: 'Learn how to use OpenModels in your Flutter App.',
  tocVideo: 'r7ysVtZ5Row',
}

<QuickstartIntro />

![OpenModels User Management example](/docs/img/supabase-flutter-demo.png)
...
First, add `io.supabase.flutterquickstart://login-callback/` as a new [redirect URL](https://open-models-platform.com/dashboard/project/_/auth/url-configuration) in the Dashboard.

![OpenModels console deep link setting](/docs/img/deeplink-setting.png)

That is it on OpenModels's end and the rest are platform specific settings:
...
Now that we have deep links ready let's initialize the OpenModels client inside our `main` function with the API credentials that you copied [earlier](#get-the-api-keys).
...
`AuthFlowType.pkce` on `authFlowType` parameter indicates that we are using a secure [PKCE flow](https://open-models-platform.com/blog/supabase-auth-sso-pkce#introducing-pkce) to perform our magic link login.
...
![OpenModels User Management example](/docs/img/supabase-flutter-account-page.png)
...
Congratulations, you've built a fully functional user management app using Flutter and OpenModels!
