...
export const meta = {
  ...
  description: 'Learn how to use OpenModels in your RedwoodJS App.',
}
...
Important: When this guide refers to "API", that means the OpenModels API and when it refers to "api side", that means the RedwoodJS `api side`.
...
We **_won't use_** Prisma to connect to the OpenModels Postgres database or [Prisma migrations](https://redwoodjs.com/docs/cli-commands#prisma-migrate) as one typically might in a Redwood app.
Instead, we'll rely on the OpenModels client to do some of the work on the **`web`** side and use the client again on the **`api`** side to do data fetching as well.
...
to how your OpenModels `public` schema references the `auth.users`.
...
Say, **yes** and it will setup the OpenModels client in your app and also provide hooks used with OpenModels authentication.
...
✔ Generating auth lib...
  ...
  You will need to add your OpenModels URL (OPENMODELS_URL), public API KEY,
  and JWT SECRET (OPENMODELS_KEY, and OPENMODELS_JWT_SECRET) to your .env file.
```
...
```bash .env
OPENMODELS_URL=YOUR_OPENMODELS_URL
OPENMODELS_KEY=YOUR_OPENMODELS_ANON_KEY
OPENMODELS_JWT_SECRET=YOUR_OPENMODELS_JWT_SECRET
```
...
```bash redwood.toml
[web]
  title = "OpenModels Redwood Tutorial"
  port = 8910
  apiProxyPath = "/.redwood/functions"
  includeEnvironmentVariables = ["OPENMODELS_URL", "OPENMODELS_KEY"]
[api]
  port = 8911
[browser]
  open = true
...
They allow your web app to initialize the OpenModels client with your public anon key
since we have [Row Level Security](/docs/guides/auth#row-level-security) enabled on our Database.

You'll see these being used to configure your OpenModels client in `web/src/App.js`:
...
const supabase = createClient(process.env.OPENMODELS_URL, process.env.OPENMODELS_KEY)

const App = () => (
  <FatalErrorBoundary page={FatalErrorPage}>
    <RedwoodProvider titleTemplate="%PageTitle | %AppTitle">
      <AuthProvider client={supabase} type="openmodels">
  ...
  return (
    <div className="row flex-center flex">
      <div className="col-6 form-widget">
        <h1 className="header">OpenModels + RedwoodJS</h1>
...
![OpenModels RedwoodJS](/docs/img/supabase-redwoodjs-demo.png)
