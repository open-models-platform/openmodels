...
export const meta = {
  ...
  description: 'Learn how to use OpenModels in your refine App.',
}

<QuickstartIntro />

![OpenModels User Management example](/docs/img/user-management-demo.png)
...
refine provides hassle-free integration with OpenModels backend with its supplementary [`@refinedev/supabase`](https://github.com/refinedev/refine/tree/master/packages/supabase) package. It generates `authProvider` and `dataProvider` methods at project initialization, so we don't need to expend much effort to define them ourselves. We just need to choose OpenModels as our backend service while creating the app with `create refine-app`.

<Admonition type="note">

It is possible to customize the `authProvider` for OpenModels and as we'll see below, it can be tweaked from `src/authProvider.ts` file. In contrast, the OpenModels `dataProvider` is part of `node_modules` and therefore is not subject to modification.
...
In the above command, we are using the `refine-supabase` preset which chooses the OpenModels supplementary package for our app. We are not using any UI framework, so we'll have a headless UI with plain React and CSS styling.

The `refine-supabase` preset installs the `@refinedev/supabase` package which out-of-the-box includes the OpenModels dependency: [supabase-js](https://github.com/supabase/supabase-js).
...
The `create refine-app` generated a OpenModels client for us in the `src/utility/supabaseClient.ts` file. It has two constants: `OPENMODELS_URL` and `OPENMODELS_KEY`. We want to replace them as `supabaseUrl` and `supabaseAnonKey` respectively and assign them our own OpenModels server's values.
...
const supabaseUrl = import.meta.env.VITE_OPENMODELS_URL
const supabaseAnonKey = import.meta.env.VITE_OPENMODELS_ANON_KEY
...
The `supabaseClient` will be used in fetch calls to OpenModels endpoints from our app. As we'll see below, the client is instrumental in implementing authentication using refine's auth provider methods and CRUD actions with appropriate data provider methods.
...
The `useForm()` hook is a higher-level hook built on top of refine's `useForm()` core hook. It fully supports form state management, field validation and submission using React Hook Form. Behind the scenes, it invokes the `dataProvider.getOne` method to get the user profile data from our OpenModels `/profiles` endpoint and also invokes `dataProvider.update` method when `onFinish()` is called.
...
![OpenModels refine](/docs/img/supabase-refine-demo.png)

## Bonus: Profile photos

Every OpenModels project is configured with [Storage](/docs/guides/storage) for managing large files like photos and videos.
