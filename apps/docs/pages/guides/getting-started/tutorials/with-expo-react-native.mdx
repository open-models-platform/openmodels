...
export const meta = {
  ...
  description: 'Learn how to use OpenModels in your React Native App.',
...
Now let's create a helper file to initialize the OpenModels client.
We need the API URL and the `anon` key that you copied [earlier](#get-the-api-keys).
These variables are safe to expose in your Expo app since OpenModels has
...
By default OpenModels Auth requires email verification before a session is created for the users. To support email verification you need to [implement deep link handling](/docs/guides/auth/native-mobile-deep-linking?platform=react-native)!
