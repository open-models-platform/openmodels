...
export const meta = {
  ...
  description: 'Learn how to use OpenModels in your Android Kotlin App.',
}
...
- [OpenModels Database](/docs/guides/database) - a Postgres database for storing your user data and [Row Level Security](/docs/guides/auth#row-level-security) so data is protected and users can only access their own information.
- [OpenModels Auth](/docs/guides/auth) - users log in through magic links sent to their email (without having to set up a password).
- [OpenModels Storage](/docs/guides/storage) - users can upload a profile photo.
...
```kotlin
OPENMODELS_ANON_KEY=YOUR_OPENMODELS_ANON_KEY
...
defaultConfig {
   ...
   buildConfigField("String", "OPENMODELS_ANON_KEY", "\"${properties.getProperty("OPENMODELS_ANON_KEY")}\"")
   buildConfigField("String", "SECRET", "\"${properties.getProperty("SECRET")}\"")
   buildConfigField("String", "OPENMODELS_URL", "\"${properties.getProperty("OPENMODELS_URL")}\"")
}
```
...
```kotlin
val url = BuildConfig.OPENMODELS_URL
val apiKey = BuildConfig.OPENMODELS_ANON_KEY
```

### Set up OpenModels dependencies
...
### Provide OpenModels instances with Hilt
...
object SupabaseModule {
    ...
    fun provideSupabaseClient(): SupabaseClient {
        return createSupabaseClient(
            supabaseUrl = BuildConfig.OPENMODELS_URL,
            supabaseKey = BuildConfig.OPENMODELS_ANON_KEY
        ) {
            install(Postgrest)
            install(GoTrue) {
...
Create a `ProductDto.kt` class and use annotations to parse data from OpenModels:
...
Create a `ProductRepository` interface and its implementation named `ProductRepositoryImpl`. This holds the logic to interact with data sources from OpenModels. Do the same with the `AuthenticationRepository`.
