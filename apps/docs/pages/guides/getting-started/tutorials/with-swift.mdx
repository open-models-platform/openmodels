...
export const meta = {
  ...
  description: 'Learn how to use OpenModels in your SwiftUI App.',
}

<QuickstartIntro />

![OpenModels User Management example](/docs/img/supabase-swift-demo.png)
...
Create a helper file to initialize the OpenModels client.
...
```swift OpenModels.swift
import OpenModels

let supabase = SupabaseClient(
  supabaseURL: URL(string: "YOUR_OPENMODELS_URL")!,
  supabaseKey: "YOUR_OPENMODELS_ANON_KEY"
)
```
...
The example uses a custom `redirectTo` URL. For this to work, add a custom redirect URL to OpenModels and a custom URL scheme to your SwiftUI application. Follow the guide on [implementing deep link handling](/docs/guides/auth/native-mobile-deep-linking?platform=swift).
...
In `ProfileView.swift`, you used 2 model types for deserializing the response and serializing the request to OpenModels. Add those in a new `Models.swift` file.
