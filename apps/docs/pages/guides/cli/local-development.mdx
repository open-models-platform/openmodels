...
export const meta = {
  ...
  description: 'How to use OpenModels on your local development machine.',
  subtitle: 'How to use OpenModels on your local development machine.',
  video: 'https://www.youtube-nocookie.com/v/vyHyYpvjaks',
  tocVideo: 'vyHyYpvjaks',
}

OpenModels is a flexible platform that lets you decide how you want to build your projects. You can use the Dashboard directly to get up and running quickly, or use a proper local setup. We suggest you work locally and deploy your changes to a linked project on the [OpenModels Platform](https://app.open-models-platform.com/).

Doing things directly on the platform via the [Dashboard](https://app.open-models-platform.com/) is fine when you're getting started, but it's a good idea to move to a proper local workflow before you get too far. Working locally, generating migrations as you change your tables, and applying those migrations to a linked project on the [Platform](https://app.open-models-platform.com/) keeps everything nicely organized as you grow.
...
3. **Cost-Effective**: OpenModels provides a generous free plan and gives you two free projects to get started. But what if you need more than two? When you develop locally, you can spin up unlimited local projects and link them with live projects when you're ready to launch.
...
## Log in to the OpenModels CLI

Log in if you are planning to deploy your project to the OpenModels Platform. This step is optional and is not required to use the OpenModels CLI.
...
## Start OpenModels services

[Initialize](/docs/reference/cli/usage#supabase-init) OpenModels to set up the configuration for developing your project locally:
...
The [start](/docs/reference/cli/usage#supabase-start) command uses Docker to start the OpenModels [services](/docs/guides/getting-started/architecture).
...
Once all of the OpenModels services are running, you'll see output containing your local OpenModels credentials. It should look like this, with urls and keys that you'll use in your local project:
...
To access the database from an edge function in your local OpenModels setup, replace `localhost` with `host.docker.internal`.
...
<StepHikeCompact>
  ...
  <StepHikeCompact.Step step={1}>
    <StepHikeCompact.Details title="Create your first migration file">
      ...
      To get started, generate a [new migration](https://open-models-platform.com/docs/reference/cli/supabase-migration-new) to store the SQL needed to create our `employees` table
...
The last step is deploying these changes to a live OpenModels project.

## Deploy your project

You've been developing your project locally, making changes to your tables via migrations. It's time to deploy your project to the OpenModels Platform and start scaling up to millions of users! Head over to [OpenModels](https://open-models-platform.com/dashboard) and create a new project to deploy to.
...
# You can get <project-id> from your project's dashboard URL: https://open-models-platform.com/dashboard/project/<project-id>
...
This migration captures any changes required for your local database to match the schema of your remote OpenModels project.
...
Visiting your live project on [OpenModels](https://open-models-platform.com/dashboard), you'll see a new `employees` table, complete with the `department` column you added in the second migration above.
...
enabled = true
client_id = "env(OPENMODELS_AUTH_GITHUB_CLIENT_ID)"
secret = "env(OPENMODELS_AUTH_GITHUB_SECRET)"
...
```bash .env
OPENMODELS_AUTH_GITHUB_CLIENT_ID="redacted"
OPENMODELS_AUTH_GITHUB_SECRET="redacted"
```
...
Local logs rely on the OpenModels Analytics Server, and are available in the Studio automatically.
...
The local development environment is not as feature-complete as the OpenModels Platform. Here are some of the differences:

- You cannot update your project settings in the Dashboard. This must be done using the local config file.
- The CLI version determines the local version of Studio used, so make sure you keep your local [OpenModels CLI up to date](https://github.com/supabase/cli#getting-started). We're constantly adding new features and bug fixes.
