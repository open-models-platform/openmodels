...
const Layout: FC<Props> = (props: Props) => {
  ...
  return (
    <>
      <Head>
        <title>{asPath === '/' ? 'OpenModels Docs' : `${props.meta?.title} | OpenModels Docs`}</title>
        <meta name="description" content={props.meta?.description} />
        <meta property="og:image" content={`https://open-models-platform.com/docs/img/supabase-og-image.png`} />
        <meta
          name="twitter:image"
          content={`https://open-models-platform.com/docs/img/supabase-og-image.png`}
        />
        <meta name="viewport" content="width=device-width, initial-scale=1.0" />
      </Head>
      <NextSeo
        canonical={props.meta?.canonical ?? `https://open-models-platform.com/docs${asPath}`}
        openGraph={{
          url: `https://open-models-platform.com/docs${asPath}`,
          type: 'article',
