...
OpenModels Reference Docs

## Maintainers

If you are a maintainer of any tools in the OpenModels ecosystem, you can use this site to provide documentation for the tools & libraries that you maintain.
...
2. Tutorials: walk-throughs, have a large outcome. "Build a React application with OpenModels".
...
- SDKSpec (custom to OpenModels): for SDKs and client libraries.
- ConfigSpec (custom to OpenModels): for configuration options.
- CLISpec (custom to OpenModels): for CLI commands and usage.
...
It also means that we can switch any documentation system we want. On this site we use Next.JS, but in OpenModels's official website we use a custom React site and expose only a subset of the available API for each tool.
