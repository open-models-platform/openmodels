...
- Changes options to require `OpenModels.SupabaseOptions.SessionPersistor` from using `ISupabaseSessionHandler`
...
- Add support for functions-csharp@1.0.1, giving access to invoking OpenModels's edge functions.
...
- Separate Storage client from OpenModels repo and into `storage-csharp`, `supabase-csharp` now references new repo.
