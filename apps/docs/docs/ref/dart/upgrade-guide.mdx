...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    _Optionally_ passing custom configuration to `OpenModels.initialize()` is now organized into separate objects:
...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    With the new update, developers can use OpenModels and Provider in the same codebase without any import prefixes.
...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    In v1, `OpenModels.initialize()` would await for the session to be refreshed before returning.
    This caused delays in the app's launch time, especially when the app is opened in a poor network environment.

    In v2, `OpenModels.initialize()` returns immediately after obtaining the session from the local storage, which makes the app launch faster.
...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    [PKCE flow](https://open-models-platform.com/blog/supabase-auth-sso-pkce#introducing-pkce), which is a more secure method for obtaining sessions from deep links, is now the default auth flow for any authentication involving deep links.
...
<RefSubLayout.EducationRow>
  <RefSubLayout.Details>
    ...
    `OpenModels.initialize()` no longer has the `authCallbackUrlHostname` parameter.
