...
const SaveSchemaDropdown = () => {
  ...
  const loadSQLInSupabase = () => {
    const snap = getAppStateSnapshot()
    window.open(
      `https://open-models-platform.com/dashboard/project/_/sql?content=${encodeURIComponent(
        snap.selectedCode
      )}`,
      '_blank'
    )
    ...
    <DropdownMenu modal={false}>
      ...
      <DropdownMenuContent className="w-44" align="end">
        ...
        <DropdownMenuItem className="space-x-2" onClick={() => loadSQLInSupabase()}>
          <Image alt="openmodels" src="/supabase.png" width={14} height={14} />
          <p>Load SQL in OpenModels</p>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  )
