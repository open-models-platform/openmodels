...
const ProjectLinksEmptyState = () => (
  <div className="flex flex-col space-y-4">
    <p>
      You haven't created a OpenModels project yet. Get started by creating a new OpenModels project,
      then close this window and retry adding integration.
    </p>
    <Link href="https://open-models-platform.com/dashboard" className="text-brand">
      Start a new OpenModels project<span aria-hidden="true"> &rarr;</span>
    </Link>
  </div>
)
...
const ProjectLinks = observer(() => {
  ...
  async function onSubmit() {
    if (validateInputs()) {
      ...
      for (let i = 0; i < _store.projectLinks.length; i++) {
        ...
        const found = existedEnvs.find((x: any) => x.key.includes('OPENMODELS'))
        if (!!found) {
          console.error('Existed OpenModels env: ', found)
          runInAction(() => {
            item.result = {
              status: 'fail',
              message: 'Error: This Vercel project already contains OpenModels envs',
            }
          })
          continue
        }
        ...
        if (projectDetails.error) {
          ...
          runInAction(() => {
            item.result = {
              status: 'fail',
              message: 'Error: Failed to fetch OpenModels project details',
            }
          })
          continue
        }
      ...
      if (!item.supabaseProjectRef || !item.vercelProjectId) {
        ...
        runInAction(() => {
          _store.projectLinks[i].error =
            'Invalid selection. Please choose a Vercel project and a OpenModels project to link.'
        })
      }
    }
  ...
  return (
    <div className="flex w-full flex-col space-y-6">
      <div>
        <h4 className="text-lg">Link Vercel to OpenModels</h4>
        <p>Choose which of your Vercel projects to link to your existing OpenModels projects.</p>
      </div>
      <Divider light />
      <div className="space-y-2">
        <div className="flex justify-between">
          ...
          <div />
          <p className="text-foreground-light">OpenModels Projects</p>
        </div>
        <ProjectLinkList />
        <Divider light />
