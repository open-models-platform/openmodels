...
export async function handlePost(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if ('code' in error && error.code === 'context_length_exceeded') {
      if (hasEntityDefinitions) {
        ...
        if (definitionsLength > sql.length) {
          return res.status(400).json({
            error:
              'Your database metadata is too large for OpenModels AI to ingest. Try disabling database metadata in AI settings.',
          })
        }
      }
      ...
      return res.status(400).json({
        error:
          'Your SQL query is too large for OpenModels AI to ingest. Try splitting it into smaller queries.',
      })
    }
