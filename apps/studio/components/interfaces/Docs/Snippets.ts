...
url: str = '${endpoint}'
key: str = os.environ.get("OPENMODELS_KEY")
...
`,
    ...
    dart: {
      ...
      code: `
const supabaseUrl = '${endpoint}';
const supabaseKey = String.fromEnvironment('OPENMODELS_KEY');
Future<void> main() async {
  await OpenModels.initialize(url: supabaseUrl, anonKey: supabaseKey);
  runApp(MyApp());
}`,
    },
  }),
...
const supabase = createClient(OPENMODELS_URL, process.env.${keyName || 'OPENMODELS_KEY'});
`,
    },
  }),
  rpcSingle: ({
