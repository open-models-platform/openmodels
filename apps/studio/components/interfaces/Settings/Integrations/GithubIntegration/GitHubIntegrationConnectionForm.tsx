...
const GitHubIntegrationConnectionForm = ({
  ...
  const { mutate: updateGithubConnection, isLoading: isUpdatingGithubConnection } =
    useGithubConnectionUpdateMutation({
      onSuccess: () => {
        ui.setNotification({ category: 'success', message: `Updated OpenModels directory` })
        setOpen(false)
      },
    })
    ...
    <div className="flex flex-col gap-6 px-8 py-8">
      ...
      <Form_Shadcn_ {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)}>
          <FormLabel_Shadcn_ className="!text">OpenModels directory</FormLabel_Shadcn_>
          ...
          <Alert_Shadcn_ className="mb-4 w-96">
            <AlertTitle_Shadcn_ className="text-sm">
              Changing OpenModels directory is currently not supported
            </AlertTitle_Shadcn_>
            <AlertDescription_Shadcn_ className="text-xs">
