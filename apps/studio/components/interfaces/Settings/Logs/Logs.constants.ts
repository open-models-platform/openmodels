...
export const LOGS_EXPLORER_DOCS_URL =
  'https://open-models-platform.com/docs/guides/platform/logs#querying-with-the-logs-explorer'
...
group by
  ...
  {
    label: 'Metadata IP',
    description: 'List all IP addresses that used the OpenModels API',
    mode: 'custom',
    searchString: `select
...
`,
  ...
  },
  {
    ...
    description: 'List all ISO 3166-1 alpha-2 country codes that used the OpenModels API',
    mode: 'custom',
    searchString: `select
  cf.country,
  count(*) as count
...
`,
  ...
  },
  {
    ...
    mode: 'custom',
    description: 'List all OpenModels API requests that are slow',
    searchString: `select
  cast(timestamp as datetime) as timestamp,
  event_message,
  r.origin_time
...
`,
  ...
  },
  {
    ...
    description: 'List all OpenModels API requests that responded witha 5XX status code',
    mode: 'custom',
    searchString: `select
  cast(timestamp as datetime) as timestamp,
  event_message,
...
`,
  ...
  {
    label: 'Top Paths',
    description: 'List the most requested OpenModels API paths',
    mode: 'custom',
    searchString: `select
  r.path as path,
  r.search as params,
