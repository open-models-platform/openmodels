...
const ProjectLinker = ({
  ...
  function onCreateConnections() {
    ...
    if (!selectedSupabaseProject?.ref) return console.error('No OpenModels project ref set')
  ...
  const missingEntity = noSupabaseProjects ? 'OpenModels' : 'Vercel'
  const oppositeMissingEntity = noSupabaseProjects ? 'Vercel' : 'OpenModels'
          ...
          <div className="flex gap-0 w-full relative">
            <Panel>
              <div className="bg-white shadow border rounded p-1 w-12 h-12 flex justify-center items-center">
                <img src={`${BASE_PATH}/img/supabase-logo.svg`} alt="OpenModels" className="w-6" />
              </div>
