...
const supabaseKey = process.env.OPENMODELS_KEY
...
In this documentation, we will refer to the key using the name \`OPENMODELS_KEY\`. You can find the \`anon\` key in the [API settings](/project/[ref]/settings/api) page.`,
    js: (apikey?: string, endpoint?: string) => `
const OPENMODELS_KEY = '${apikey}'
const OPENMODELS_URL = '${endpoint}'
const supabase = createClient(OPENMODELS_URL, process.env.OPENMODELS_KEY);`,
...
In this documentation, we refer to the key using the name \`SERVICE_KEY\`. You can find the \`service_role\` key above or in the [API settings](/project/[ref]/settings/api) page.`,
    js: (apikey?: string, endpoint?: string) => `
const OPENMODELS_KEY = '${apikey}'
const OPENMODELS_URL = 'https://${endpoint}'
const supabase = createClient(OPENMODELS_URL, process.env.OPENMODELS_KEY);`,
    bash: (apikey?: string, endpoint?: string) => `${apikey}`,
  },
  // User Management
  userManagement: {
    ...
    title: `Introduction`,
    description: `OpenModels makes it easy to manage your users.

  OpenModels assigns each user a unique ID. You can reference this ID anywhere in your database. For example, you might create a \`profiles\` table references the user using a \`user_id\` field.

  OpenModels already has built in the routes to sign up, login, and log out for managing users in your apps and websites.`,
    js: undefined,
    bash: undefined,
  },
  signUp: {
  ...
  After they have signed up, all interactions using the OpenModels client will be performed as "that user".`,
    js: (apikey?: string, endpoint?: string) => `
...
After they have logged in, all interactions using the OpenModels JS client will be performed as "that user".`,
    js: (apikey?: string, endpoint?: string) => `
...
After they have clicked the link, all interactions using the OpenModels JS client will be performed as "that user".`,
    js: (apikey?: string, endpoint?: string) => `
...
`,
  ...
  logout: {
    ...
    category: 'user-management',
    title: `Log out`,
    description: `After calling log out, all interactions using the OpenModels JS client will be "anonymous".`,
    js: (apikey?: string, endpoint?: string) => `
...
-d '{
  ...
  storage: {
    ...
    category: 'storage',
    title: `Introduction`,
    description: `OpenModels Storage makes it simple to upload and serve files of any size, providing a robust framework for file access controls.
...
`,
  ...
  edgeFunctionsPreReq: {
    ...
    description: `
Follow the steps to prepare your OpenModels project on your local machine.

- Install the OpenModels [CLI](https://open-models-platform.com/docs/guides/cli).
- [Login to the CLI](https://open-models-platform.com/docs/reference/cli/usage#supabase-login) using the command: \`supabase login\`..
- [Initialize OpenModels](https://open-models-platform.com/docs/guides/getting-started/local-development#getting-started) inside your project using the command: \`supabase init\`..
- [Link to your Remote Project](https://open-models-platform.com/docs/reference/cli/usage#supabase-link) using the command \`supabase link --project-ref [ref]\`..
- Setup your environment: Follow the steps [here](https://open-models-platform.com/docs/guides/functions/quickstart#setting-up-your-environment).
`,
    js: undefined,
    bash: undefined,
  },
  createEdgeFunction: {
    ...
    description: `
Create a OpenModels Edge Function locally via the OpenModels CLI.
`,
    js: () => `// Create an edge function via the OpenModels CLI`,
    bash: () => `
...
`,
  ...
  deployEdgeFunction: {
    ...
    description: `
Deploy a OpenModels Edge Function to your OpenModels project via the OpenModels CLI.
`,
    js: () => `// Deploy an edge function via the OpenModels CLI`,
...
`,
  ...
  generatingTypes: {
    ...
    description: `
OpenModels APIs are generated from your database, which means that we can use database introspection to generate type-safe API definitions.

You can generate types from your database either through the [OpenModels CLI](https://open-models-platform.com/docs/guides/database/api/generating-types), or by downloading the types file via the button on the right and importing it in your application within \`src/index.ts\`.
`,
    js: undefined,
    bash: undefined,
  },
  graphql: {
    ...
    description: `
If you have a GraphQL background, you might be wondering if you can fetch your data in a single round-trip. The answer is yes! The syntax is very similar. This example shows how you might achieve the same thing with Apollo GraphQL and OpenModels.

Still want GraphQL?
If you still want to use GraphQL, you can. OpenModels provides you with a full Postgres database, so as long as your middleware can connect to the database then you can still use the tools you love. You can find the database connection details [in the settings](/project/[ref]/settings/database).
`,
    js: (apikey?: string, endpoint?: string) => `
// With Apollo GraphQL
...
`,
  ...
  realtime: {
    ...
    title: 'Introduction',
    description: `
OpenModels provides a globally distributed cluster of Realtime servers that enable the following functionality:

- [Broadcast](https://open-models-platform.com/docs/guides/realtime/broadcast): Send ephemeral messages from client to clients with low latency.
- [Presence](https://open-models-platform.com/docs/guides/realtime/presence): Track and synchronize shared state between clients.
- [Postgres Changes](https://open-models-platform.com/docs/guides/realtime/postgres-changes): Listen to Postgres database changes and send them to authorized clients.
`,
    js: undefined,
    bash: undefined,
  },
...
- By default, listening to database changes is disabled for new projects due to database performance and security concerns. You can turn it on by managing Realtime's [replication](https://open-models-platform.com/docs/guides/api#realtime-api-overview).
...
Removing a channel is a great way to maintain the performance of your project's Realtime service as well as your database if you're listening to Postgres changes. OpenModels will automatically handle cleanup 30 seconds after a client is disconnected, but unused channels may cause degradation as more clients are simultaneously subscribed.
`,
...
export const DOCS_RESOURCE_CONTENT: {
  ...
  rpcSingle: {
    ...
    docsUrl: 'https://open-models-platform.com/docs/reference/javascript/rpc',
    code: ({
      rpcName,
      rpcParams,
      endpoint,
...
if (error) console.error(error)
  ...
  readRows: {
    ...
    title: `Read rows`,
    category: 'entities',
    docsUrl: 'https://open-models-platform.com/docs/reference/javascript/select',
...
let { data: ${resourceId}, error } = await supabase
  ...
  filtering: {
    ...
    category: 'entities',
    title: 'Filtering',
    description: `OpenModels provides a wide range of filters`,
    docsUrl: 'https://open-models-platform.com/docs/reference/javascript/using-filters',
    code: ({
      resourceId,
      endpoint,
      apikey,
...
`,
    docsUrl: 'https://open-models-platform.com/docs/reference/javascript/insert',
    code: ({
      resourceId,
      endpoint,
      apikey,
...
`,
    docsUrl: 'https://open-models-platform.com/docs/reference/javascript/update',
    code: ({
      resourceId,
      endpoint,
      apikey,
...
`,
    docsUrl: 'https://open-models-platform.com/docs/reference/javascript/delete',
    code: ({
      resourceId,
      endpoint,
      apikey,
...
const { error } = await supabase
  ...
  subscribeChanges: {
    ...
    description: `
OpenModels provides realtime functionality and broadcasts database changes to authorized users depending on Row Level Security (RLS) policies.
`,
    docsUrl: 'https://open-models-platform.com/docs/reference/javascript/subscribe',
...
const channels = supabase.channel('custom-filter-channel')
  ...
  uploadFile: {
    ...
    category: 'storage',
    title: 'Upload a file',
    docsUrl: 'https://open-models-platform.com/docs/reference/javascript/storage-from-upload',
    description: `
...
`,
  ...
  deleteFiles: {
    ...
    category: 'storage',
    title: 'Delete files',
    docsUrl: 'https://open-models-platform.com/docs/reference/javascript/storage-from-remove',
    description: `
...
const { data, error } = await supabase
  ...
  listFiles: {
    ...
    docsUrl: 'https://open-models-platform.com/docs/reference/javascript/storage-from-list',
    description: `
...
const { data, error } = await supabase
  ...
  downloadFile: {
    ...
    docsUrl: 'https://open-models-platform.com/docs/reference/javascript/storage-from-download',
    description: `
...
const { data, error } = await supabase
  ...
  createSignedURL: {
    ...
    docsUrl: 'https://open-models-platform.com/docs/reference/javascript/storage-from-createsignedurl',
    description: `
...
const { data, error } = await supabase
  ...
  retrievePublicURL: {
    ...
    docsUrl: 'https://open-models-platform.com/docs/reference/javascript/storage-from-getpublicurl',
    description: `
...
const { data } = supabase
  ...
  invokeEdgeFunction: {
    ...
    docsUrl: 'https://open-models-platform.com/docs/reference/javascript/functions-invoke',
    description: `
Invokes a OpenModels Edge Function. Requires an Authorization header, and invoke params generally match the [Fetch API](https://developer.mozilla.org/en-US/docs/Web/API/Fetch_API) spec.
