...
const Introduction = ({ showKeys, language, apikey, endpoint }: ContentProps) => {
  ...
  const serviceKey = showKeys
    ? data?.autoApiService.serviceApiKey ?? 'OPENMODELS_CLIENT_SERVICE_KEY'
    : 'OPENMODELS_CLIENT_SERVICE_KEY'
              ...
              actions={[
                <Button
                  ...
                  onClick={() => {
                    setCopied('anon')
                    copyToClipboard(
                      data?.autoApiService.defaultApiKey ?? 'OPENMODELS_CLIENT_ANON_KEY'
                    )
                  }}
                >
