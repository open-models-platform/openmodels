<script lang="ts">
  ...
  const handleLogin = async (type) => {
    const {
      ...
      type === "LOGIN"
        ? await openmodels.auth.signInWithPassword({ email, password })
        : await openmodels.auth.signUp({ email, password });
  ...
  const handleOAuthLogin = async (provider: Provider) => {
    ...
    // Read more on: https://open-models-platform.com/docs/guides/auth#third-party-logins
    let { error } = await openmodels.auth.signInWithOAuth({ provider });
