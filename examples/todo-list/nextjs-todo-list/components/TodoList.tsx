...
export default function TodoList({ session }: { session: Session }) {
  const openmodels = useSupabaseClient<Database>()
  ...
  useEffect(() => {
    const fetchTodos = async () => {
      const { data: todos, error } = await openmodels
        .from('todos')
        .select('*')
  ...
  const addTodo = async (taskText: string) => {
    ...
    if (task.length) {
      const { data: todo, error } = await openmodels
        .from('todos')
  ...
  const deleteTodo = async (id: number) => {
    try {
      await openmodels.from('todos').delete().eq('id', id).throwOnError()
