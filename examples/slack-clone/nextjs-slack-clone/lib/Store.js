...
export const openmodels = createPagesBrowserClient()
...
export const useStore = (props) => {
  ...
  useEffect(() => {
    ...
    const messageListener = openmodels
    ...
    const channelListener = openmodels
    ...
    // Cleanup on unmount
    return () => {
      openmodels.removeChannel(openmodels.channel(messageListener))
      openmodels.removeChannel(openmodels.channel(userListener))
      openmodels.removeChannel(openmodels.channel(channelListener))
    }
  }, [])
...
export const fetchChannels = async (setState) => {
  try {
    let { data } = await openmodels.from('channels').select('*')
...
export const fetchUser = async (userId, setState) => {
  try {
    let { data } = await openmodels.from('users').select(`*`).eq('id', userId)
    let user = data[0]
...
export const fetchUserRoles = async (setState) => {
  try {
    let { data } = await openmodels.from('user_roles').select(`*`)
    if (setState) setState(data)
    return data
  } catch (error) {
    console.log('error', error)
...
export const addChannel = async (slug, user_id) => {
  try {
    let { data } = await openmodels.from('channels').insert([{ slug, created_by: user_id }]).select()
    return data
  } catch (error) {
    console.log('error', error)
  }
...
export const addMessage = async (message, channel_id, user_id) => {
  try {
    let { data } = await openmodels.from('messages').insert([{ message, channel_id, user_id }]).select()
    return data
  } catch (error) {
    console.log('error', error)
  }
...
export const deleteChannel = async (channel_id) => {
  try {
    let { data } = await openmodels.from('channels').delete().match({ id: channel_id })
    return data
  } catch (error) {
    console.log('error', error)
  }
...
export const deleteMessage = async (message_id) => {
  try {
    let { data } = await openmodels.from('messages').delete().match({ id: message_id })
    return data
  } catch (error) {
    console.log('error', error)
  }
