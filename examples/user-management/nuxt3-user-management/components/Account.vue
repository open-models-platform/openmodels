...
<script setup>
const openmodels = useSupabaseClient()
...
async function updateProfile() {
    try {
        ...
        let { error } = await openmodels.from('profiles').upsert(updates)
...
const openmodelsAuth = useSupabaseAuthClient()
async function signOut() {
    try {
        loading.value = true
        let { error } = await openmodelsAuth.auth.signOut()
        if (error) throw error
    } catch (error) {
        alert(error.message)
    } finally {
