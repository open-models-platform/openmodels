...
const openmodels = createClient(
  process.env.NEXT_PUBLIC_OPENMODELS_URL ?? 'http://localhost:54321',
  process.env.OPENMODELS_SERVICE_ROLE_KEY ??
...
export default async function PostList() {
  const { data, error } = await openmodels.from('articles').select('*')
