        ...
        children: [
          ...
          TextFormField(
            ...
            onChanged: (value) async {
              ...
              try {
                final factorsResponse = await openmodels.auth.mfa.listFactors();
                final factor = factorsResponse.totp.first;
                final factorId = factor.id;

                final challenge =
                    await openmodels.auth.mfa.challenge(factorId: factorId);
                await openmodels.auth.mfa.verify(
                  factorId: factorId,
                  challengeId: challenge.id,
                  code: value,
                );
                await openmodels.auth.refreshSession();
                if (mounted) {
