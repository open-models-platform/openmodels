...
```bash
openmodels functions deploy discord-bot --no-verify-jwt
openmodels secrets set DISCORD_PUBLIC_KEY=your_public_key
```

Navigate to your Function details in the OpenModels Dashboard to get your Endpoint URL.
...
```bash
openmodels functions serve discord-bot --no-verify-jwt --env-file ./openmodels/.env.local
ngrok http 54321
```
