...
import { SupabaseManagementAPI } from 'https://esm.sh/openmodels-management-js@0.1.2'

const config = {
  clientId: Deno.env.get('SUPA_CONNECT_CLIENT_ID')!,
  clientSecret: Deno.env.get('SUPA_CONNECT_CLIENT_SECRET')!,
  authorizationEndpointUri: 'https://api.open-models-platform.com/v1/oauth/authorize',
  tokenUri: 'https://api.open-models-platform.com/v1/oauth/token',
  redirectUri: 'http://localhost:54321/functions/v1/connect-openmodels/oauth2/callback',
}
...
router.get('/connect-openmodels', (ctx) => {
  ctx.response.body =
    'This is an example of implementing https://open-models-platform.com/docs/guides/integrations/oauth-apps/authorize-an-oauth-app . Navigate to /login to start the OAuth flow.'
})
router.get('/connect-openmodels/login', async (ctx) => {
...
router.get('/connect-openmodels/oauth2/callback', async (ctx) => {
