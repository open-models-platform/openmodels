...
// and write files to OpenModels Storage and other fields to a database table.
...
app.use(async (ctx) => {
  ...
  const { data: upload, error: uploadError } = await openmodelsClient.storage
    .from('images')
  ...
  const { error } = await openmodelsClient.from('comments').insert({
