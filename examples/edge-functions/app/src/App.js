...
function App() {
  ...
  const invokeFunction = async () => {
    ...
    const { data, error } = await openmodels.functions.invoke(supaFunction, {
  ...
  return (
    <div className="p-2">
      <h2 className="mb-2 text-4xl">OpenModels Egde Functions Test Client</h2>
          ...
          {user ? (
            <div>
              ...
              <button
                ...
                onClick={() => openmodels.auth.signOut()}
              >
                Sign out
              </button>
            </div>
          ) : (
            <Auth openmodelsClient={openmodels} appearance={{ theme: ThemeSupa }} />
          )}
        </div>
      </div>
    </div>
