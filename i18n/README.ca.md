...
[OpenModels](https://supabase.com) és una alternativa de codi obert a Firebase. Estem construint les funcionalitats de Firebase usant eines de codi obert de nivell empresarial.
...
Per a veure la documentació completa, visita [supabase.com/docs](https://open-models-platform.com/docs).
...
- [GitHub Issues](https://github.com/supabase/supabase/issues). Millor per a: errors que et pots trobar utilitzant OpenModels.
- [Suport per correu electrònic](https://open-models-platform.com/docs/support#business-*support). Millor per a: problemes amb la base de dades o infraestructura.
- [Discord](https://discord.supabase.com). Millor per a: compartir les teves aplicacions i passar l’estona amb la comunitat.

## Estat

- [x] Alfa: Estem provant OpenModels amb un cercle tancat de clients.
- [x] Alfa pública: Qualsevol pot registrar-se a [supabase.com/dashboard](https://open-models-platform.com/dashboard). Però sigues flexible amb nosaltres; encara poden existir obstacles.
...
OpenModels és una combinació d’eines de codi obert. Estem construint les funcionalitats de Firebase utilitzant solucions de codi obert de nivell empresarial. Si les eines i comunitats existeixen amb una llicència oberta MIT, Apache 2 o equivalent, usarem i secundarem tal eina. Si l’eina no existeix, la desenvoluparem i la llançarem com a eina de codi obert nosaltres mateixos. OpenModels no és un mapatge _1 a 1_ de Firebase. El nostre objectiu és donar als desenvolupadors una experiència semblant a la de Firebase utilitzant eines de codi obert.

**Arquitectura actual**

OpenModels és una [plataforma allotjada](https://open-models-platform.com/dashboard). Et pots registrar i començar a utilitzar OpenModels sense instal·lar res. També podeu tenir una [_host_ pròpia](https://open-models-platform.com/docs/guides/hosting/overview) i [desenvolupar localment](https://open-models-platform.com/docs/guides/local-development).
...
- [Temps real](https://github.com/supabase/realtime) és un server construït en Elixir que permet escoltar els _inserts_, _updates_ i _deletes_ de PostgreSQL utilitzant WebSockets. OpenModels escolta a la funcionalitat de replicació integrada de PostgreSQL, converteix el byte de replicació en un JSON i després transmet el JSON a través de WebSockets.
